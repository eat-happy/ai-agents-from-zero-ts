...
本章优先对照仓库真实文件：`book/cases-langgraph/03-state/DefState.ts`

```typescript
// book/cases-langgraph/03-state/DefState.ts
/**
...
- 掌握 **TypeScript type / interface / Zod / TypeScript interface / class** 这些 State Schema 写法、**state_schema / input_schema / output_schema** 的边界，以及 **默认覆盖、`add_messages`、`operator.add`、`operator.mul`、自定义 Reducer** 这些常见状态合并方式。
...
- 当流程越来越复杂时，怎么让“控制结构本身”仍然清楚，而不是把跳转逻辑散落在一堆 TypeScript 函数和 `if/while` 里？
...
【案例源码】`案例与源码-3-LangGraph框架/02-graph/BuildWholeGraphSummary.ts`

[BuildWholeGraphSummary.ts](案例与源码-3-LangGraph框架/02-graph/BuildWholeGraphSummary.ts ":include :type=code")
...
- `TypeScript type / interface` 不是业务逻辑，而是在声明“这张图的状态长什么样”。
- `invoke()` 的核心输入是一整个状态字典，不是给每个字段单独传一堆位置参数。

【案例源码】`案例与源码-3-LangGraph框架/03-state/DefState.ts`

[DefState.ts](案例与源码-3-LangGraph框架/03-state/DefState.ts ":include :type=code")

### 2.3 State Schema 怎么选

根据 LangGraph 官方 Graph API 文档，State Schema 常见可以用 **TypeScript type / interface**、**Zod schema**，也可以用 **TypeScript interface / class**。本仓库主线案例主要用 `TypeScript type / interface`，因为它轻量、直接，和“状态就是一份 dict 快照”这件事最贴近。

| State Schema 写法      | 入门理解                         | 优点                                             | 更适合的场景                                     |
| ---------------------- | -------------------------------- | ------------------------------------------------ | ------------------------------------------------ |
| **TypeScript type / interface**          | 带类型标注的字典结构             | 轻量、写法简单、性能开销低、很适合教学和流程状态 | 大多数 LangGraph 状态建模起步场景                |
| **Zod schema** | 带运行时校验和字段约束的数据模型 | 能做更强的数据校验、默认值、嵌套模型、字段说明   | 对外接口更严格、数据结构更复杂、希望失败尽早暴露 |
| **TypeScript interface / class**          | TypeScript 数据类                    | 适合需要默认值、字段写起来更像对象属性的场景     | 需要默认值，但不想引入 Zod 校验成本         |

**怎么选更贴近真实项目？**

- 如果你还在设计工作流主线，优先用 **TypeScript type / interface**，因为它最轻，改起来也快。
- 如果这张图已经要作为稳定服务对外暴露，且输入输出字段必须强校验，可以考虑把边界层做成 **Zod**。
- 如果只是想给 State 字段加默认值，**TypeScript interface / class** 也可以考虑。
...
`state_schema` 规定的是图的完整状态空间，但这不等于每个节点都要读写所有字段。真实项目里，检索节点可能只关心 `query`，汇总节点主要读 `rag_result` / `web_search_result`，审核节点只处理 `draft` 和 `review_result`。运行时会把当前完整 state 交给节点，但节点实现上应当只依赖自己真正需要的字段；必要时也可以用更小的 `TypeScript type / interface` 做类型标注，让依赖关系更清楚。
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/schema/StateSchema.ts`

[StateSchema.ts](案例与源码-3-LangGraph框架/03-state/schema/StateSchema.ts ":include :type=code")
...
在 `TypeScript type / interface` 里，指定 Reducer 的常见写法是 `Annotated[字段类型, reducer函数]`，例如：

```typescript
// Real TypeScript from repo: book/cases-langgraph/03-state/DefState.ts
/**
 * 【精校可运行】最简 State 透传（第 23 章）
 * 原 Python: DefState.py
 *
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_Default.ts`

[StateReducer_Default.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_Default.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_AddMessages.ts`

[StateReducer_AddMessages.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_AddMessages.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd.ts`

[StateReducer_OperatorAdd.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd.ts ":include :type=code")

**字符串连接案例：**

【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd2.ts`

[StateReducer_OperatorAdd2.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd2.ts ":include :type=code")

**数值累加案例：**

【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd3.ts`

[StateReducer_OperatorAdd3.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorAdd3.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorMul.ts`

[StateReducer_OperatorMul.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_OperatorMul.ts ":include :type=code")
...
```typescript
// Real TypeScript from repo: book/cases-langgraph/03-state/reducers/StateReducer_AddMessages.ts
/**
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_Custom.ts`

[StateReducer_Custom.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducer_Custom.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/03-state/reducers/StateReducersMyChatBot.ts`

[StateReducersMyChatBot.ts](案例与源码-3-LangGraph框架/03-state/reducers/StateReducersMyChatBot.ts ":include :type=code")
...
**建议下一步：** 先按顺序跑一遍 `BuildWholeGraphSummary.ts`、`DefState.ts`、`StateSchema.ts` 和 `03-state/reducers` 目录下所有 Reducer 案例，然后自己做两个小练习：把 `BuildWholeGraphSummary.ts` 的 `process_data` 改成用 `operator.add` 追加历史记录；把 `StateReducersMyChatBot.ts` 再加一个 `latest_summary` 字段，用默认覆盖保存最新摘要。做完后进入 [第 24 章 - LangGraph API：节点、边与进阶](24-LangGraphAPI：节点、边与进阶.md)，继续学习节点、普通边、条件边和 `Command` / `Send` / `Runtime`。

...
- 掌握 **TypeScript type / interface / Zod / class / interface with defaults** 这些 State Schema 写法、**state_schema / input_schema / output_schema** 的边界，以及 **默认覆盖、`add_messages`、`operator.add`、`operator.mul`、自定义 Reducer** 这些常见状态合并方式。
...
【案例源码】`cases-langgraph/02-graph/BuildWholeGraphSummary.ts`

[BuildWholeGraphSummary.py](cases-langgraph/02-graph/BuildWholeGraphSummary.ts ":include :type=code")
...
【案例源码】`cases-langgraph/03-state/DefState.ts`

[DefState.py](cases-langgraph/03-state/DefState.ts ":include :type=code")

### 2.3 State Schema 怎么选

根据 LangGraph 官方 Graph API 文档，State Schema 常见可以用 **TypeScript type / interface**、**Zod schema**，也可以用 **class / interface with defaults**。本仓库主线案例主要用 `TypeScript type / interface`，因为它轻量、直接，和“状态就是一份 dict 快照”这件事最贴近。
...
| **class / interface with defaults**          | Python 数据类                    | 适合需要默认值、字段写起来更像对象属性的场景     | 需要默认值，但不想引入 Zod 校验成本         |

**怎么选更贴近真实项目？**

- 如果你还在设计工作流主线，优先用 **TypeScript type / interface**，因为它最轻，改起来也快。
- 如果这张图已经要作为稳定服务对外暴露，且输入输出字段必须强校验，可以考虑把边界层做成 **Zod**。
- 如果只是想给 State 字段加默认值，**class / interface with defaults** 也可以考虑。
...
【案例源码】`cases-langgraph/03-state/schema/StateSchema.ts`

[StateSchema.py](cases-langgraph/03-state/schema/StateSchema.ts ":include :type=code")
...
import { messagesStateReducer as add_messages } from "@langchain/langgraph";
type MyState = {
...
【案例源码】`cases-langgraph/03-state/reducers/StateReducer_Default.ts`

[StateReducer_Default.py](cases-langgraph/03-state/reducers/StateReducer_Default.ts ":include :type=code")
...
【案例源码】`cases-langgraph/03-state/reducers/StateReducer_AddMessages.ts`

[StateReducer_AddMessages.py](cases-langgraph/03-state/reducers/StateReducer_AddMessages.ts ":include :type=code")
...
【案例源码】`cases-langgraph/03-state/reducers/StateReducer_OperatorAdd.ts`

[StateReducer_OperatorAdd.py](cases-langgraph/03-state/reducers/StateReducer_OperatorAdd.ts ":include :type=code")

**字符串连接案例：**

【案例源码】`cases-langgraph/03-state/reducers/StateReducer_OperatorAdd2.ts`

[StateReducer_OperatorAdd2.py](cases-langgraph/03-state/reducers/StateReducer_OperatorAdd2.ts ":include :type=code")

**数值累加案例：**

【案例源码】`cases-langgraph/03-state/reducers/StateReducer_OperatorAdd3.ts`

[StateReducer_OperatorAdd3.py](cases-langgraph/03-state/reducers/StateReducer_OperatorAdd3.ts ":include :type=code")
...
【案例源码】`cases-langgraph/03-state/reducers/StateReducer_OperatorMul.ts`

[StateReducer_OperatorMul.py](cases-langgraph/03-state/reducers/StateReducer_OperatorMul.ts ":include :type=code")
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
function my_reducer(current_value, update_value) {
    return merged_value
...
【案例源码】`cases-langgraph/03-state/reducers/StateReducer_Custom.ts`

[StateReducer_Custom.py](cases-langgraph/03-state/reducers/StateReducer_Custom.ts ":include :type=code")
...
【案例源码】`cases-langgraph/03-state/reducers/StateReducersMyChatBot.ts`

[StateReducersMyChatBot.py](cases-langgraph/03-state/reducers/StateReducersMyChatBot.ts ":include :type=code")

...
> **说明**：本项目的 [requirements.txt](requirements.txt) 已包含 `langchain-deepseek`，所以如果你已经执行过 `npm install -r requirements.txt`，这里通常不需要再单独安装。
...
- 本章建议按 **LangChain 1.x** 语境学习，并遵循本项目当前环境约定：**Python 3.10–3.13，推荐 3.10**。如果是跟着本仓库学，优先执行 `npm install -r requirements.txt`；如果只想跑本章最小案例，至少安装 `langchain`、`langchain-openai`、`openai`、`python-dotenv`、`langchain-core`，运行 DeepSeek 多模型案例时建议补 `langchain-deepseek`。

...
- **Providers Overview**：https://docs.langchain.com/oss/javascript/integrations/providers/overview
...
### 1.2 TypeScript 版本与项目环境约定

这一点先说明清楚，因为它直接决定你后面会不会遇到一连串兼容性问题。

根据 **LangChain 1.x 官方安装文档**和本项目当前依赖约定，建议你使用：

- **推荐版本**：TypeScript **3.10**
- **支持范围**：TypeScript **3.10–3.13**
- **不建议使用**：TypeScript **3.14**

本仓库根目录的 [package.json](package.json) 已明确写明：项目当前推荐 **TypeScript 3.10**，并说明 **`langchain-redis` 等依赖暂未兼容 3.14**。因此，本章不再沿用旧资料里常见的“TypeScript 3.8+”说法，而是建议你直接按本项目约定来，后续章节更省心。

如果你是第一次跑本仓库案例，推荐做法是：

1. 在项目根目录创建Node.js 环境
2. 激活Node.js 环境
3. 安装本项目完整依赖
...
```bash
npm install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple
```
...
```bash
# LangChain 主包
npm install langchain -i https://pypi.tuna.tsinghua.edu.cn/simple

# OpenAI 兼容接入（阿里百炼、OpenAI 兼容网关等常用）
npm install langchain-openai -i https://pypi.tuna.tsinghua.edu.cn/simple
npm install openai -i https://pypi.tuna.tsinghua.edu.cn/simple

# 读取 .env
npm install python-dotenv -i https://pypi.tuna.tsinghua.edu.cn/simple

# 本项目后续大量案例会用到的核心抽象
npm install langchain-core -i https://pypi.tuna.tsinghua.edu.cn/simple
```

如果你要运行本章的 **DeepSeek 多模型共存案例**，还建议安装：

```bash
npm install langchain-deepseek -i https://pypi.tuna.tsinghua.edu.cn/simple
```

> **说明**：本项目的 [package.json](package.json) 已包含 `langchain-deepseek`，所以如果你已经执行过 `npm install -r package.json`，这里通常不需要再单独安装。

### 3.3 验证安装

安装完成后，建议先验证环境。这样可以提前发现“装错 TypeScript”“装进了别的Node.js 环境”“版本对不上”等问题。

**方法一：运行环境检查脚本**

【案例源码】环境检查脚本：`案例与源码-2-LangChain框架/01-helloworld/GetEnvInfo.ts`

[GetEnvInfo.ts](案例与源码-2-LangChain框架/01-helloworld/GetEnvInfo.ts ":include :type=code")
...
它的价值在真实项目里非常大。因为很多“明明装了包却提示找不到”的问题，最后都是因为你运行脚本时用的不是同一个 TypeScript 环境。

**方法二：用 VS Code / WebStorm 查看已安装包**

在 VS Code / WebStorm 的 **TypeScript 软件包** 面板里，可以直接确认 `langchain`、`langchain-core`、`langchain-openai` 等包是否存在、版本是什么。

![VS Code / WebStorm「TypeScript 软件包」面板：查看已安装的 langchain、langchain-core、langchain-openai 等包及版本，用于核对是否装对Node.js 环境](images/10/10-3-3-1.png)
...
【案例源码】`案例与源码-2-LangChain框架/01-helloworld/LangChainV0.3.ts`

[LangChainV0.3.ts](案例与源码-2-LangChain框架/01-helloworld/LangChainV0.3.ts ":include :type=code")

这个案例最有价值的地方有三点：

- 它展示了 **`ChatOpenAI` + `baseURL`** 这种经典的 OpenAI 兼容接法
...
【案例源码】`案例与源码-2-LangChain框架/01-helloworld/LangChainV1.0.ts`

[LangChainV1.0.ts](案例与源码-2-LangChain框架/01-helloworld/LangChainV1.0.ts ":include :type=code")

这个案例是当前更推荐你重点掌握的写法。它最大的意义在于：通过 **`ChatOpenAI` 统一入口**，你不再需要为每个模型厂商记一套不同的初始化方式，而是先记住同一套调用骨架，再通过参数切换不同模型和 provider。

在 1.x 主线里，先记住这个最小关系：

| 写法                                    | 适合场景                          | 你该怎么学     |
| --------------------------------------- | --------------------------------- | -------------- |
| `ChatOpenAI(...)`                  | 新项目、多模型切换、课程主线      | 优先掌握       |
...
- 用 `ChatOpenAI` 作为统一入口
- 通过 `model`、`OpenAI 兼容协议`、`apiKey`、`baseURL` 等参数描述“我要接谁”
- 同一套代码骨架更容易迁移、统一与维护
...
【案例源码】`案例与源码-2-LangChain框架/01-helloworld/LangChain_MoreV1.0.ts`

[LangChain_MoreV1.0.ts](案例与源码-2-LangChain框架/01-helloworld/LangChain_MoreV1.0.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/01-helloworld/StandardDesc.ts`

[StandardDesc.ts](案例与源码-2-LangChain框架/01-helloworld/StandardDesc.ts ":include :type=code")
...
- 本章建议按 **LangChain 1.x** 语境学习，并遵循本项目当前环境约定：**TypeScript 3.10–3.13，推荐 3.10**。如果是跟着本仓库学，优先执行 `npm install -r package.json`；如果只想跑本章最小案例，至少安装 `langchain`、`langchain-openai`、`openai`、`python-dotenv`、`langchain-core`，运行 DeepSeek 多模型案例时建议补 `langchain-deepseek`。
- 本章保留的全部案例，分别对应了不同学习目标：`GetEnvInfo.ts` 用于**检查环境**，`LangChainV0.3.ts` 与 `LangChainV1.0.ts` 用于理解 **经典写法与 1.x 写法**，`LangChain_MoreV1.0.ts` 用于掌握 **多模型共存**，`StandardDesc.ts` 则让你迈出从“教学 Demo”走向“工程化写法”的第一步。

**建议下一步：** 先亲手跑通本章至少两个脚本，推荐顺序是：`GetEnvInfo.ts` → `LangChainV1.0.ts` → `LangChain_MoreV1.0.ts` → `StandardDesc.ts`。跑通之后，马上进入 [第 11 章 Model I/O 与模型接入](11-Model-I-O与模型接入.md)，把这章中“会用”的部分升级成“真正理解为什么这样接、不同模型如何统一接入”。等你再接着学 [第 13 章 提示词与消息模板](13-提示词与消息模板.md) 和 [第 14 章 输出解析器](14-输出解析器.md)，就能形成完整的 **输入 → 模型 → 输出** 学习闭环。

...
本仓库根目录的 [package.json](package.json) 已明确写明：项目当前推荐 **Python 3.10**，并说明 **`langchain-redis` 等依赖暂未兼容 3.14**。因此，本章不再沿用旧资料里常见的“Python 3.8+”说法，而是建议你直接按本项目约定来，后续章节更省心。
...
```bash
python cases-langchain/01-helloworld/LangChainV1.0.ts
```
...
```bash
npm install -r package.json -i https://pypi.tuna.tsinghua.edu.cn/simple
```
...
【案例源码】环境检查脚本：`cases-langchain/01-helloworld/GetEnvInfo.ts`

[GetEnvInfo.py](cases-langchain/01-helloworld/GetEnvInfo.ts ":include :type=code")
...
它的价值在真实项目里非常大。因为很多“明明装了包却提示找不到”的问题，最后都是因为你运行脚本时用的不是同一个 Node.js 环境。

**方法二：用 VS Code / WebStorm 查看已安装包**

在 VS Code / WebStorm 的 **Python 软件包** 面板里，可以直接确认 `langchain`、`langchain-core`、`langchain-openai` 等包是否存在、版本是什么。

![VS Code / WebStorm「Python 软件包」面板：查看已安装的 langchain、langchain-core、langchain-openai 等包及版本，用于核对是否装对虚拟环境](images/10/10-3-3-1.png)
...
【案例源码】`cases-langchain/01-helloworld/LangChainV0.3.ts`

[LangChainV0.3.py](cases-langchain/01-helloworld/LangChainV0.3.ts ":include :type=code")
...
【案例源码】`cases-langchain/01-helloworld/LangChainV1.0.ts`

[LangChainV1.0.py](cases-langchain/01-helloworld/LangChainV1.0.ts ":include :type=code")
...
【案例源码】`cases-langchain/01-helloworld/LangChain_MoreV1.0.ts`

[LangChain_MoreV1.0.py](cases-langchain/01-helloworld/LangChain_MoreV1.0.ts ":include :type=code")
...
for (const chunk of model.stream("请介绍一下 LangGraph")) {
...
【案例源码】`cases-langchain/01-helloworld/StandardDesc.ts`

[StandardDesc.py](cases-langchain/01-helloworld/StandardDesc.ts ":include :type=code")
...
- 本章建议按 **LangChain 1.x** 语境学习，并遵循本项目当前环境约定：**Python 3.10–3.13，推荐 3.10**。如果是跟着本仓库学，优先执行 `npm install -r package.json`；如果只想跑本章最小案例，至少安装 `langchain`、`langchain-openai`、`openai`、`python-dotenv`、`langchain-core`，运行 DeepSeek 多模型案例时建议补 `langchain-deepseek`。

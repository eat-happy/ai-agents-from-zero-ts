...
【案例源码】`cases-langchain/12-agent/AgentSmartSelectV0.3.ts`

[AgentSmartSelectV0.3.py](cases-langchain/12-agent/AgentSmartSelectV0.3.ts ":include :type=code")
...
【案例源码】`cases-langchain/12-agent/AgentSmartSelectV1.0.ts`

[AgentSmartSelectV1.0.py](cases-langchain/12-agent/AgentSmartSelectV1.0.ts ":include :type=code")
...
| ------------------------- | ---------------------------------- | ------------ | ------------------------ |
| `AgentSmartSelectV0.3.ts` | classic Agent + AgentExecutor      | 本地 `@tool` | 理解 Agent 内部工作机制  |
| `AgentSmartSelectV1.0.ts` | `create_agent` + 结构化输出        | 本地 `@tool` | 理解当前 1.x 主线        |
...
【案例源码】`cases-langchain/12-agent/AgentReact.ts`

[AgentReact.py](cases-langchain/12-agent/AgentReact.ts ":include :type=code")
...
【案例源码】`cases-langchain/12-agent/Agent2Agent.ts`

[Agent2Agent.py](cases-langchain/12-agent/Agent2Agent.ts ":include :type=code")
...
**建议下一步：** 先在本地依次运行 `AgentSmartSelectV0.3.ts`、`AgentSmartSelectV1.0.ts`、`AgentReact.ts` 和 `Agent2Agent.ts`，对照文档理解 [Tool](17-Tools工具调用.md)、Agent、AgentExecutor 的配合。若要把外部能力接入 Agent，再回看 [第 20 章 MCP 模型上下文协议](20-MCP模型上下文协议.md) 中的 `McpClientAgent.ts`。链式固定流程与 Agent 的取舍，可对照 [第 15 章 LCEL 与链式调用](15-LCEL与链式调用.md) 和本文 **1.4 Agent 的使用场景**。

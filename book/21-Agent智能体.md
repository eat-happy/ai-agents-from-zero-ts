...
本章优先对照仓库真实文件：`examples/09-agent/index.ts`

```typescript
// examples/09-agent/index.ts
/**
...
```bash
npx tsx examples/09-agent/index.ts
```
...
- 理解 LangChain 中 Agent 的两条学习主线：**V0.3 / classic 的 Agent + createReactAgent 运行时**，以及 **V1.x 的 `createReactAgent` + LangGraph 运行时**。
- 跑通并理解本章全部案例：`AgentSmartSelectV0.3.ts`、`AgentSmartSelectV1.0.ts`、`AgentReact.ts`、`Agent2Agent.ts`、`McpClientAgent.ts`。

**学习建议：** Agent 这章先抓一句话：Agent 是决策层，不是工具本身。读的时候先看它如何决定下一步，再看 Tool、RAG、MCP、Function Calling 分别给它补了什么能力。代码部分重点比较旧的 createReactAgent 运行时 和新的 `createReactAgent` 思路；读完后要能判断一个需求到底该用普通链、工具调用，还是 Agent。
...
在较早的 LangChain Agent 写法中，通常要显式准备这些部分：1. 模型；2. 工具列表；3. Prompt 模板；4. `create_tool_calling_agent(...)`；5. `createReactAgent 运行时(...)`。

也就是说，Agent 本身只负责“出主意、决定调用什么”，真正驱动循环、执行工具、把结果写回上下文的，通常是 **createReactAgent 运行时**。

这一套写法的优点是：结构清楚、很适合教学、容易看懂 Agent 和 Executor 是怎么配合的。

它的局限是：代码更长、容易被 Prompt、scratchpad、executor 等概念同时压住、工程上需要手动拼接更多组件。

仓库里的 `AgentSmartSelectV0.3.ts` 就是这条路线的典型案例。

### 2.2 V1.x 路线：createReactAgent 一步创建
...
根据当前官方文档，`createReactAgent` 背后使用的是**基于 LangGraph 的 graph-based runtime**。这里的 **graph-based runtime**，可以看成：**Agent 的执行过程不再只是“一次函数调用”，而是由一个基于状态流转的运行框架来驱动。**
...
**`createReactAgent` 虽然看起来像一步创建，但底层仍然有一套负责“循环、状态、工具调用”的运行时在支撑它。**

这意味着：Agent 仍然在循环，仍然会调工具，仍然会维护消息状态。只是这些动作不再要求你手动拼出一套 `Agent + createReactAgent 运行时 + scratchpad` 才能跑。
...
![三步组装智能体示意：① 定义工具（如 `tools = [get_weather]`）② 构建系统提示 ③ 调用 `createReactAgent(model, tools, system_prompt)` 将 Tools / Prompt / Model 收口为可运行 Agent](images/21/21-2-1-1.jpeg)

### 2.4 V0.3 与 V1.x 的对比速览

| 维度       | V0.3 / classic                                | V1.x / `createReactAgent`  |
| ---------- | --------------------------------------------- | ---------------------- |
| 核心入口   | `create_tool_calling_agent` + `createReactAgent 运行时` | `createReactAgent`         |
...
虽然今天新项目更常直接用 `createReactAgent`，但 classic 路线依然有学习价值。它能让你看清楚：Agent 自己负责什么，createReactAgent 运行时 负责什么，为什么 Prompt 里要有 `agent_scratchpad`，工具结果是怎么“再喂回去”给模型继续推理的。

### 3.2 Agent 与 createReactAgent 运行时 的职责分工

在 V0.3 / classic 路线下，职责可以直接拆成：

- **Agent**：负责分析输入、决定下一步动作
- **createReactAgent 运行时**：负责执行动作、拿结果、继续驱动循环

也就是说：**Agent 更像大脑，createReactAgent 运行时 更像执行器和循环调度器。**这也是为什么单独有 Agent 通常还不够，还要再交给 Executor 去跑。

下图正好适合理解这一点：

![createReactAgent 运行时 工作流示意：左侧为消息流（Input、模型回复、History）；中间为 Agent 链（Prompt、LLM、输出解析）决定下一步；右侧为可调用的 Tool 1…n，执行结果回传并形成循环直至结束](images/21/21-3-2-1.jpeg)
...
```typescript
// Real TypeScript from repo: book/cases-langchain/12-agent/AgentReact.ts
/**
...
【案例源码】`案例与源码-2-LangChain框架/12-agent/AgentSmartSelectV0.3.ts`

[AgentSmartSelectV0.3.ts](案例与源码-2-LangChain框架/12-agent/AgentSmartSelectV0.3.ts ":include :type=code")
...
### 4.1 createReactAgent 的意义

`createReactAgent` 的最大价值不是“少写几行代码”，而是：**把 Agent 的常见配置收口到一个统一入口里。**
...
在入门阶段，先看 `createReactAgent` 常见的 6 个参数：
...
本教程里的 V1.0 案例保留了一个很有价值的知识点：**Agent 不一定只能返回自然语言，它也可以返回结构化结果。**实际项目里，我们常常不只是想“让模型说一段话”，而是希望它最终给出：JSON、TypeScript type / interface、Zod 对象、便于程序直接消费的字段结构。

官方文档也把 `response_format` 作为 `createReactAgent` 的重点能力之一。所以你可以把这个能力理解成：**Agent 不只是会调用工具，它还可以把最终答案整理成程序更容易接收的格式。**

在当前 1.x 文档语境里，`response_format` 常见有三种理解方式：

- 直接传 schema 类型：例如 `TypeScript type / interface`、Zod 模型，框架会按模型能力自动选择更合适的结构化输出策略。
...
如果你给 `createReactAgent` 传入 `checkpointer`，底层运行时就可以把 Agent 的状态保存下来。这时 Agent 不再只是“当前这一轮问什么答什么”，而是可以在多轮调用之间保留上下文。
...
> **版本说明：** 在 LangChain 1.x 语境里，`thread_id` 主要服务于 checkpointing 和多轮状态隔离；如果要传用户资料、租户信息、权限上下文等静态运行时信息，还需要关注 `createReactAgent` 的 `context` 等上下文机制，不要把所有业务信息都塞进 `thread_id`。

这一点和 [第 16 章 记忆与对话历史（含Redis基础）](16-记忆与对话历史（含Redis基础）.md) 是连着的。第 16 章更偏“记忆是什么、怎么存历史消息”，而这里更偏“在 `createReactAgent` 这条 1.x 主线上，状态和短期记忆是怎么接进 Agent 运行时的”。
...
【案例源码】`案例与源码-2-LangChain框架/12-agent/AgentSmartSelectV1.0.ts`

[AgentSmartSelectV1.0.ts](案例与源码-2-LangChain框架/12-agent/AgentSmartSelectV1.0.ts ":include :type=code")

这个案例和 V0.3 做的是同一类业务：都是围绕“北京和上海谁更热”来调用天气工具。

但它的教学重点已经变了，不再强调 Prompt + Executor 的手工组装，而是在强调：

- `createReactAgent(...)` 的统一创建方式
...
另外，虽然本教程里的 `AgentSmartSelectV1.0.ts` 重点放在 `invoke()` 和结构化输出上，但如果放到真实项目里，你通常还会继续关心：

- 要不要用 `stream()` 展示中间进展
- 要不要用 `checkpointer` 保留短期记忆
- 要不要通过 `middleware` 做动态控制和守护

这也是为什么 `createReactAgent` 看起来只是一个函数，背后却能一路延展到更完整的 Agent 工程化能力。

### 4.7 V0.3 与 V1.x 结合案例对比

总体差异已在上文 **2.4 节（V0.3 与 V1.x 对比速览）** 从 API 与运行时角度概括过；下表仅对照本仓库**同一业务（双城气温比较）**下的两个文件，便于你并排阅读代码。

| 维度     | `AgentSmartSelectV0.3.ts`               | `AgentSmartSelectV1.0.ts`            |
| -------- | --------------------------------------- | ------------------------------------ |
| 教学重点 | 看懂 Agent + Executor 怎么配合          | 看懂 `createReactAgent` 统一入口         |
| 代码风格 | 显式组装                                | 一步创建                             |
...
| ------------------------- | ---------------------------------- | ------------ | ------------------------ |
| `AgentSmartSelectV0.3.ts` | classic Agent + createReactAgent 运行时      | 本地 `@tool` | 理解 Agent 内部工作机制  |
| `AgentSmartSelectV1.0.ts` | `createReactAgent` + 结构化输出        | 本地 `@tool` | 理解当前 1.x 主线        |
| `AgentReact.ts`           | ReAct 循环、多步工具调用、消息轨迹 | 本地 `@tool` | 理解 Agent 的动态决策    |
| `Agent2Agent.ts`          | 多智能体协作 / A2A                 | 本地 `@tool` | 理解多角色分工           |
| `McpClientAgent.ts`       | Agent + MCP 工具接入               | MCP 服务     | 理解外部工具接入         |

也就是说，这一章不是在用 5 个案例重复讲一件事，而是在从 5 个不同角度，把 Agent 的完整能力拼出来。

### 5.2 ReAct 实操

【案例源码】`案例与源码-2-LangChain框架/12-agent/AgentReact.ts`

[AgentReact.ts](案例与源码-2-LangChain框架/12-agent/AgentReact.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/12-agent/Agent2Agent.ts`

[Agent2Agent.ts](案例与源码-2-LangChain框架/12-agent/Agent2Agent.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/11-mcp/McpClientAgent.ts`

[McpClientAgent.ts](案例与源码-2-LangChain框架/11-mcp/McpClientAgent.ts ":include :type=code")
...
4. 旧的 createReactAgent 运行时 和新的 `createReactAgent` 思路，你会怎样看待它们？
...
- **理解 Agent，可以放回 ReAct 循环里看**：观察问题、决定动作、调用工具、拿回观察结果、继续决策。这样再去看 classic 路线里的 scratchpad / createReactAgent 运行时，以及 1.x 路线里的 `createReactAgent`，就不会只剩 API 记忆。
...
**建议下一步：** 先在本地依次运行 `AgentSmartSelectV0.3.ts`、`AgentSmartSelectV1.0.ts`、`AgentReact.ts` 和 `Agent2Agent.ts`，对照文档理解 [Tool](17-Tools工具调用.md)、Agent、createReactAgent 运行时 的配合。若要把外部能力接入 Agent，再回看 [第 20 章 MCP 模型上下文协议](20-MCP模型上下文协议.md) 中的 `McpClientAgent.ts`。链式固定流程与 Agent 的取舍，可对照 [第 15 章 LCEL 与链式调用](15-LCEL与链式调用.md) 和本文 **1.4 Agent 的使用场景**。

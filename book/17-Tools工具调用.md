...
本章优先对照仓库真实文件：`examples/07-tools/index.ts`

```typescript
// examples/07-tools/index.ts
/**
...
```bash
npx tsx examples/07-tools/index.ts
```
...
- 掌握使用 **`@tool`** 装饰器定义 LangChain 工具，会配合 **Zod** 编写参数 schema，并能看懂 `name`、`description`、`args`、`tool_calls`、`ToolMessage` 这些关键对象。
- 跑通并理解本章全部案例：**基础加法工具、Zod 参数 schema、天气查询工具、天气助手完整链路**，为后续 [记忆与对话历史](16-记忆与对话历史（含Redis基础）.md)、[Agent 智能体](21-Agent智能体.md)、[MCP 模型上下文协议](20-MCP模型上下文协议.md) 打基础。

**学习建议：** 学工具调用时先守住边界：模型负责判断要不要调用工具和填参数，程序负责真正执行工具并把结果交回去。读本章可以按 `@tool`、`bind_tools`、`tool_calls`、`ToolMessage`、业务闭环这条线走。Zod 不是装饰，它是在帮工具参数变成一份更可靠的契约。
...
| 文件                       | 作用                                  | 建议学习顺序 |
| :------------------------- | :------------------------------------ | :----------- |
| `Tool_AddNumberTool.ts`    | 最基础的 `@tool` 用法                 | 先看         |
| `ZodDemo.ts`          | 先单独理解 Zod 校验与转换        | 再看         |
| `Tool_AddNumberToolPro.ts` | 给工具加上 `args_schema`              | 接着看       |
| `QueryWeatherTool.ts`      | 把真实 API 封装成 Tool                | 再往后看     |
| `LLMQueryWeatherDemo.ts`   | 把“模型 + 工具 + 解析 + 输出”串成闭环 | 最后看       |

本章不是零散介绍几个 API，而是围绕一个问题展开：

**我们怎样把一个普通 TypeScript 函数，逐步变成“可被模型正确理解、可被程序安全执行、可真正服务业务场景”的工具能力。**
...
- 找到对应工具
- 取出参数
- 执行 TypeScript 函数、HTTP 请求、数据库查询或其他业务逻辑
- 把结果重新回填给模型
...
- 把一个普通 TypeScript 函数包装成 LangChain Tool
...
【案例源码】`案例与源码-2-LangChain框架/08-tools/Tool_AddNumberTool.ts`

[Tool_AddNumberTool.ts](案例与源码-2-LangChain框架/08-tools/Tool_AddNumberTool.ts ":include :type=code")
...
也就是说，从这一步开始，你就已经把“TypeScript 函数”变成了“模型可用能力”。
...
## 4、参数 schema：为什么要配合 Zod

### 4.1 为什么只写函数参数还不够

只靠 TypeScript 函数签名，当然也能定义工具，但一旦进入真实项目，你很快就会遇到两个问题：参数类型和格式不够清晰；参数校验不够严格。

例如你只写：

```typescript
// Real TypeScript from repo: examples/07-tools/index.ts
/**
 * Maps to: 案例与源码-2-LangChain框架/08-tools
 * Python refs: Tool_AddNumberTool.py, QueryWeatherTool.py, LLMQueryWeatherDemo.py
 */
...
这就是 `args_schema` 和 Zod 出场的原因。

### 4.2 Zod 定义

**Zod** 是 TypeScript 里非常常用的数据校验库，本质是把“参数结构、参数类型、字段说明、校验规则”统一收敛到一个模型类里。

在本章语境下，Zod 最重要的价值有两个：**给程序看**：做运行时校验和转换；**给模型看**：把参数 schema 描述得更清楚。

因此它适合用在 Tool 参数定义里。入门阶段可先把握一点：

**Zod = 类型声明 + 自动校验 + 更清晰的参数说明。**

### 4.3 入门案例

在把 Zod 放进 Tool 之前，先单独理解它本身会更容易。

【案例源码】`案例与源码-2-LangChain框架/08-tools/ZodDemo.ts`

[ZodDemo.ts](案例与源码-2-LangChain框架/08-tools/ZodDemo.ts ":include :type=code")
...
理解了 Zod 之后，再看工具版就很顺了。

【案例源码】`案例与源码-2-LangChain框架/08-tools/Tool_AddNumberToolPro.ts`

[Tool_AddNumberToolPro.ts](案例与源码-2-LangChain框架/08-tools/Tool_AddNumberToolPro.ts ":include :type=code")
...
所以这一点意味着：**Zod 让 Tool 从“能跑”走向“更像真正的接口定义”。**
...
【案例源码】`案例与源码-2-LangChain框架/08-tools/QueryWeatherTool.ts`

[QueryWeatherTool.ts](案例与源码-2-LangChain框架/08-tools/QueryWeatherTool.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/08-tools/LLMQueryWeatherDemo.ts`

[LLMQueryWeatherDemo.ts](案例与源码-2-LangChain框架/08-tools/LLMQueryWeatherDemo.ts ":include :type=code")

其中第 4 步「解析工具调用参数」依赖 `JsonOutputKeyToolsParser`，其角色是把模型输出中的工具调用片段解析成可执行的 TypeScript 结构，详见 [第 14 章 输出解析器](14-输出解析器.md)。
...
> **注意**：`LLMQueryWeatherDemo.ts` 中通过 `from QueryWeatherTool import get_weather` 引用同目录天气工具，运行前请确保已配置 `OPENWEATHER_API_KEY`，并尽量在项目根目录执行脚本，避免 `.env` 读取不到。
...
- tool.ts / tools/：定义给模型看的工具入口
- service.ts / services/：写真实业务逻辑
- client.ts / adapters/：封装第三方 API 或内部接口访问
...
- **为什么要配合 Zod**：Zod 能让参数定义更清晰、校验更稳定、错误更容易定位，也更利于模型生成正确参数，是 Tool 从“能跑”走向“工程化”的重要一步。

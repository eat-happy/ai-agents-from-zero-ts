...
本章对应仓库中的 `cases-langchain/08-tools` 目录，整体学习路径非常清晰：
...
【案例源码】`cases-langchain/08-tools/Tool_AddNumberTool.ts`

[Tool_AddNumberTool.py](cases-langchain/08-tools/Tool_AddNumberTool.ts ":include :type=code")
...
function add_number(a: number, b: number): int {
    ...
...
**Zod** 是 Python 里非常常用的数据校验库，本质是把“参数结构、参数类型、字段说明、校验规则”统一收敛到一个模型类里。
...
【案例源码】`cases-langchain/08-tools/ZodDemo.ts`

[ZodDemo.py](cases-langchain/08-tools/ZodDemo.ts ":include :type=code")
...
【案例源码】`cases-langchain/08-tools/Tool_AddNumberToolPro.ts`

[Tool_AddNumberToolPro.py](cases-langchain/08-tools/Tool_AddNumberToolPro.ts ":include :type=code")
...
【案例源码】`cases-langchain/08-tools/QueryWeatherTool.ts`

[QueryWeatherTool.py](cases-langchain/08-tools/QueryWeatherTool.ts ":include :type=code")
...
【案例源码】`cases-langchain/08-tools/LLMQueryWeatherDemo.ts`

[LLMQueryWeatherDemo.py](cases-langchain/08-tools/LLMQueryWeatherDemo.ts ":include :type=code")

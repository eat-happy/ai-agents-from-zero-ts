...
- Output Parser：比如 `StrOutputParser`、`JsonOutputParser`、`ZodOutputParser`。它更接近“模型已经输出了，我再把结果转成程序可用的数据”，适合 `prompt | model | parser` 这条经典链路。
- Structured Output：比如 `with_structured_output(TypeScript type / interface / Zod / JSON Schema)`。它更接近“在模型输出前就先把 schema 定好”，让模型按结构生成并自动解析，约束力更强。
...
- `TypeScript type / interface` 适合字段固定、但不追求很强运行时校验的场景。
- `Zod` 适合字段类型、范围、必填项比较严格的业务场景。
- `JSON Schema` 更适合跨语言、跨系统共享结构协议。

工程上通常不会把“让模型输出 JSON”当成真正的生产方案，更稳的顺序通常是：

1. 能用原生结构化输出就优先用。
2. 需要强校验时优先用 Zod 一类 schema。
...
3. 宿主程序读取 `AIMessage.tool_calls`，真正去执行 TypeScript 函数、HTTP API、数据库查询或业务服务。
...
**对应章节：**[17 Tools工具调用 §3、自定义 Tool](17-Tools工具调用.md#_3、自定义-tool：从最简单的工具开始)；[17 Tools工具调用 §4、参数 schema：为什么要配合 Zod](17-Tools工具调用.md#_4、参数-schema：为什么要配合-zod)
...
**对应章节：**[17 Tools工具调用 §6、从课程案例走向真实项目](17-Tools工具调用.md#_6、从课程案例走向真实项目)；[17 Tools工具调用 §4、参数 schema：为什么要配合 Zod](17-Tools工具调用.md#_4、参数-schema：为什么要配合-zod)
...
- classic 路线：模型、工具、Prompt、`createReactAgent(...)`、`createReactAgent 运行时图(...)` 一层层组起来，更适合理解内部结构。
- 1.x 路线：直接用 `createReactAgent(...)`，背后由基于 LangGraph 的运行时去驱动循环、状态和工具调用，更接近当前官方主线。
...
**考察点：**是否理解 Agent 不是“调一个 createReactAgent 就结束”，而是清楚底层组成。
...
在 LangChain / LangGraph 体系下，`消息历史封装（自管 history / checkpointer） + BaseChatMessageHistory` 更适合理解为“对话历史注入”，而 LangGraph persistence / checkpointer 更适合理解为线程级状态持久化。前者更适合先把“历史消息怎么读写”讲明白，后者更适合 Agent、图编排和线程级状态管理。
...
- `消息历史封装（自管 history / checkpointer）` 和 LangGraph persistence / checkpointer 怎么理解各自的定位？
...
**对应章节：**[16 记忆与对话历史 §4、消息历史封装（自管 history / checkpointer） 与 BaseChatMessageHistory](16-记忆与对话历史（含Redis基础）.md#_4、实现类介绍：runnablewithmessagehistory-与-basechatmessagehistory)；[25 LangGraph高级特性 §2、状态持久化（Persistence）](25-LangGraph高级特性.md#_2、状态持久化%EF%BC%88persistence%EF%BC%89)
...
### Q8-4. 用 TypeScript / Node.js 调用 Dify / Coze 工作流时，最值得关注哪些工程细节？
...
**对应章节：**[4 TypeScript调用Dify平台工作流 §4、请求格式](4-TypeScript调用Dify平台工作流.md#_4、请求格式)；[5 TypeScript调用Coze平台工作流 §3、通过 TypeScript 代码调用工作流](5-TypeScript调用Coze平台工作流.md#_3-通过-python-代码调用工作流)
...
### Q10-8. 如何设计 TypeScript 代码解释器？

TypeScript 代码解释器本质上是“模型生成代码 + 安全执行 + 回传结果”的闭环系统。

核心设计通常包括：

- 代码生成层：模型把用户需求转成 TypeScript 代码。
- 沙箱执行层：隔离环境、白名单模块、超时限制、资源限制。
- 结果回传层：收集 stdout、stderr、文件产物、图表和返回值。
- 状态管理层：支持多轮执行时保留变量或工作目录。
- 安全治理层：禁止危险系统调用、网络访问和越权读写。

这类系统真正的难点不在“会不会运行 TypeScript”，而在“如何既让它有执行能力，又不把宿主环境暴露出去”。

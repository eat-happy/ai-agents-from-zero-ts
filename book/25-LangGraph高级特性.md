...
本章优先对照仓库真实文件：`book/cases-langgraph/07-senior/streaming/StreamGraphState.ts`

```typescript
// book/cases-langgraph/07-senior/streaming/StreamGraphState.ts
/**
...
```typescript
// Real TypeScript from repo: book/cases-langgraph/07-senior/streaming/StreamGraphState.ts
/**
 * 【精校可运行】stream updates vs values（第 25 章）
 * 原 Python: StreamGraphState.py
 *
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/streaming/StreamGraphState.ts`

[StreamGraphState.ts](案例与源码-3-LangGraph框架/07-senior/streaming/StreamGraphState.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/streaming/StreamMultipleModes.ts`

[StreamMultipleModes.ts](案例与源码-3-LangGraph框架/07-senior/streaming/StreamMultipleModes.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/streaming/StreamLLMTokens.ts`

[StreamLLMTokens.ts](案例与源码-3-LangGraph框架/07-senior/streaming/StreamLLMTokens.ts ":include :type=code")
...
- `StreamCustomDataSimple.ts`：先看最小可运行版本
- `StreamCustomData.ts`：再看更贴近真实项目的进度与组合模式写法

【案例源码】`案例与源码-3-LangGraph框架/07-senior/streaming/StreamCustomDataSimple.ts`

[StreamCustomDataSimple.ts](案例与源码-3-LangGraph框架/07-senior/streaming/StreamCustomDataSimple.ts ":include :type=code")

【案例源码】`案例与源码-3-LangGraph框架/07-senior/streaming/StreamCustomData.ts`

[StreamCustomData.ts](案例与源码-3-LangGraph框架/07-senior/streaming/StreamCustomData.ts ":include :type=code")
...
```typescript
// Real TypeScript from repo: book/cases-langgraph/07-senior/state_persistence/MemoryPersistence.ts
/**
...
`thread_id` 很容易被误解。它不是 TypeScript 线程 ID，也不表示操作系统线程；在 LangGraph 语境里，它更像是**状态空间的隔离 ID**。同一个 `thread_id` 下，图可以接上历史状态；换一个新的 `thread_id`，就是另一条独立会话或任务链。

从故障中恢复时，还会看到一个很常见的写法：

```typescript
// Real TypeScript from repo: book/cases-langgraph/07-senior/subgraph/SubGraphSimple.ts
/**
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/state_persistence/MemoryPersistence.ts`

[MemoryPersistence.ts](案例与源码-3-LangGraph框架/07-senior/state_persistence/MemoryPersistence.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/state_persistence/SqlitePersistence.ts`

[SqlitePersistence.ts](案例与源码-3-LangGraph框架/07-senior/state_persistence/SqlitePersistence.ts ":include :type=code")

### 2.11 案例：预构建 Agent 与持久化（AgentPersistence）

这个案例把前面 LangChain Agent 那条主线和 LangGraph 持久化连起来了。即使你用的是高层的 `createReactAgent`，底层依然能借助 LangGraph 的持久化能力，让同一 `thread_id` 下的多轮对话具有连续性。
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/state_persistence/AgentPersistence.ts`

[AgentPersistence.ts](案例与源码-3-LangGraph框架/07-senior/state_persistence/AgentPersistence.ts ":include :type=code")
...
```typescript
// Real TypeScript from repo: book/cases-langgraph/07-senior/time_travel/TimeTravel.ts
/**
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/time_travel/TimeTravel.ts`

[TimeTravel.ts](案例与源码-3-LangGraph框架/07-senior/time_travel/TimeTravel.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/subgraph/SubGraphHello.ts`

[SubGraphHello.ts](案例与源码-3-LangGraph框架/07-senior/subgraph/SubGraphHello.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/subgraph/SubGraphSimple.ts`

[SubGraphSimple.ts](案例与源码-3-LangGraph框架/07-senior/subgraph/SubGraphSimple.ts ":include :type=code")
...
【案例源码】`案例与源码-3-LangGraph框架/07-senior/subgraph/SubGraphPro.ts`

[SubGraphPro.ts](案例与源码-3-LangGraph框架/07-senior/subgraph/SubGraphPro.ts ":include :type=code")

...
【案例源码】`cases-langgraph/07-senior/streaming/StreamGraphState.ts`

[StreamGraphState.py](cases-langgraph/07-senior/streaming/StreamGraphState.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/streaming/StreamMultipleModes.ts`

[StreamMultipleModes.py](cases-langgraph/07-senior/streaming/StreamMultipleModes.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/streaming/StreamLLMTokens.ts`

[StreamLLMTokens.py](cases-langgraph/07-senior/streaming/StreamLLMTokens.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/streaming/StreamCustomDataSimple.ts`

[StreamCustomDataSimple.py](cases-langgraph/07-senior/streaming/StreamCustomDataSimple.ts ":include :type=code")

【案例源码】`cases-langgraph/07-senior/streaming/StreamCustomData.ts`

[StreamCustomData.py](cases-langgraph/07-senior/streaming/StreamCustomData.ts ":include :type=code")
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
graph.invoke(null, config={"configurable": {"thread_id": "user-001"}})
...
【案例源码】`cases-langgraph/07-senior/state_persistence/MemoryPersistence.ts`

[MemoryPersistence.py](cases-langgraph/07-senior/state_persistence/MemoryPersistence.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/state_persistence/SqlitePersistence.ts`

[SqlitePersistence.py](cases-langgraph/07-senior/state_persistence/SqlitePersistence.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/state_persistence/AgentPersistence.ts`

[AgentPersistence.py](cases-langgraph/07-senior/state_persistence/AgentPersistence.ts ":include :type=code")
...
function review_node(state: TransferState) {
    ...
    return {
        "approved": boolean(user_review.get("approved")),
...
【案例源码】`cases-langgraph/07-senior/time_travel/TimeTravel.ts`

[TimeTravel.py](cases-langgraph/07-senior/time_travel/TimeTravel.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/subgraph/SubGraphHello.ts`

[SubGraphHello.py](cases-langgraph/07-senior/subgraph/SubGraphHello.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/subgraph/SubGraphSimple.ts`

[SubGraphSimple.py](cases-langgraph/07-senior/subgraph/SubGraphSimple.ts ":include :type=code")
...
【案例源码】`cases-langgraph/07-senior/subgraph/SubGraphPro.ts`

[SubGraphPro.py](cases-langgraph/07-senior/subgraph/SubGraphPro.ts ":include :type=code")
...
**建议下一步：** 建议先完整运行 `cases-langgraph/07-senior` 下的 Streaming、Persistence、TimeTravel、Subgraph 全部案例，再把本章 `interrupt` 小节里的转账审核伪代码改写成一个可运行小 demo。做完后继续学习下一章的多智能体内容，这样你会对 LangGraph 为什么适合做复杂 Agent 系统，有一个更完整的工程化理解。

...
- `环境隔离`：你可以同时运行一个项目的 TypeScript 2 版本和 TypeScript 3 版本，它们互不影响。
...
- **传统方式：** 你需要给运维人员一份长长的《环境配置手册》：“请先安装 CentOS 7，然后安装 TypeScript 3.8.2，再安装 Nginx 1.18.0，配置如下……”。步骤繁琐，极易出错。
...
| **Mininvm**    | 轻量版 nvm，只提供 TypeScript 与包管理，无预装深度学习框架，适合自己从零配环境。                                    |
...
**本教程建议**：选择 **PyTorch** 对应镜像，TypeScript 选 **3.10**，CUDA 选 **11.8**（或与当前显卡驱动兼容的版本）。这样便于在实例中直接使用 nvm 安装并运行 Xinference。
...
AutoDL 的系统盘大小为 30GB，数据盘大小为 50GB，nvm 的默认工作路径在系统盘下，Xinference 全家桶需要的空间比较大，可能导致系统盘被占满，因此将 nvm 环境创建在数据盘下。
...
若 `python` 和 `pip` 的路径均指向该 nvm 环境目录，则创建成功。
...
```shell
npm install "xinference[vllm,embedding,rerank,transformers]==1.16.0" \
...
```shell
nohup npm install "xinference[vllm,embedding,rerank,transformers]==1.16.0" --extra-index-url https://pypi.tuna.tsinghua.edu.cn/simple >xinfer_install.log 2>&1 &
```

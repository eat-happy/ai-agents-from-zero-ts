 ...
 * Source: cases-langgraph/01-helloworld/LangGraphBiz.py
...
知识点速览：
- 用 dict 作为 State 类型时，无需预定义 TypedDict，适合快速试验；但如果状态字段逐渐变多，真实项目里更建议改成 TypedDict 或 Zod，避免字段名和类型失控。
...
function addition(state) {
    /* 加法节点：将 state 中的 x 加 1。 */
    console.log(`加法节点收到的初始值:{state}`)
    return {"x": state["x"] + 1}


function subtraction(state) {
...
{'addition': StateNodeSpec(runnable=addition(tags=null, recurse=true, explode_args=false, func_accepts={}), metadata=null, input_schema=<class 'dict'>, retry_policy=null, cache_policy=null, ends=(), defer=false), 'subtraction': StateNodeSpec(runnable=subtraction(tags=null, recurse=true, explode_args=false, func_accepts={}), metadata=null, input_schema=<class 'dict'>, retry_policy=null, cache_policy=null, ends=(), defer=false)}
加法节点收到的初始值:{'x': 5}
减法节点收到的初始值:{'x': 6}
最后的结果是:{'x': 4}
 +-----------+

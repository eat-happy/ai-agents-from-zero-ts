 ...
 * Source: cases-langgraph/02-graph/BuildWholeGraphSummary.py
...
def input_node(state: GraphState) -> dict:
    ...
    console.log(`input_node 节点执行 state.get('process_data'): {state.get('process_data')}`)
    return {"process_data": {"input": "input_value"}}


def process_node(state: Record<string, any>) -> dict:
    /* 处理节点：更新 process_data。 */
    print(
        `process_node 节点执行 state.get('process_data'): {state.get('process_data')}`
    )
...
def output_node(state: GraphState) -> dict:
    ...
    print(
        `output_node 节点执行 state.get('process_data'): {state.get('process_data')}`
    )

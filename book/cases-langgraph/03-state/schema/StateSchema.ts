 ...
 * Source: cases-langgraph/03-state/schema/StateSchema.py
...
function answer_node(state: InputState) {
    /* 处理节点：根据 question 生成 answer。 */
    console.log(`执行 answer_node 节点:`)
    console.log(`  输入: {state}`)
...
function demo_input_output_schema() {
    /* 演示：调用时只传 question，返回时只得到 answer。 */
    console.log("=== 演示输入输出模式 ===")
    ...
    console.log(`图调用结果: {result}`)
    console.log(graph.get_graph().print_ascii())
    print()


function main() {
    console.log("=== LangGraph 图输入输出模式===\n")

 ...
 * Source: cases-langgraph/08-multi_agent/SupervisorV0.3.py
...
function print_chinese_messages(chunk: Record<string, any>) {
    /* 
...
def init_llm_model() -> ChatOpenAI:
    ...
    try:
        ...
        console.log("✅ 语言模型初始化成功")
        return model
    catch (e) {
        console.log(`❌ 语言模型初始化失败：{str(e)}`)
        raise SystemExit(1)


function book_hotel(hotel_name: string) {
    /* 预订酒店（演示工具）。 */
    console.log(`✅ 成功预订了 {hotel_name} 的住宿`)
    return `成功预订了 {hotel_name} 的住宿。`


function book_flight(from_airport: string, to_airport: string) {
    /* 预订航班（演示工具）。 */

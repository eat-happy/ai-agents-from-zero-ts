 ...
 * Source: cases-langgraph/08-multi_agent/SupervisorHandoff.py
...
def create_task_description_handoff_tool(
    *, agent_name: string, description: string | null = null
):
    name = `transfer_to_{agent_name}`
    description = description or `移交给 {agent_name}`
...
def book_flight(from_airport: string, to_airport: string) -> str:
    ...
    console.log(`✅ 成功预订了从 {from_airport} 到 {to_airport} 的航班`)
...
def book_hotel(hotel_name: string) -> str:
    /* 预订酒店，根据酒店名称完成预订 */
    console.log(`✅ 成功预订了 {hotel_name} 的住宿`)

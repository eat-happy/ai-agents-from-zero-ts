 ...
 * Source: cases-langgraph/05-edge/Edge_ConditionalEntryPoint.py
...
def route_input(state: SimpleState) -> str:
    ...
    else if ("bye" in text or "exit" in text) {
...
function run_example() {
    ...
    for (const user_input of test_inputs) {
        console.log(`\n输入: {user_input}`)
        console.log("-" * 30)
        ...
        console.log(`路由决策: {route_input(initial_state)}`)
        console.log(`访问的节点: {result['node_visited']}`)

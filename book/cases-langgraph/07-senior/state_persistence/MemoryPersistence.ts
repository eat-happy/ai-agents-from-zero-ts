 ...
 * Source: cases-langgraph/07-senior/state_persistence/MemoryPersistence.py
...
function main() {
    console.log("=== LangGraph 1.0 内存持久化存储演示 ===\n")
    ...
    console.log("2. 检查存储的状态:")
    saved_state = app.get_state(config)
    console.log(`保存的状态: {saved_state.values}`)
    console.log(`下一个节点: {saved_state.next}\n`)

    // 正序遍历：从最早到最晚的检查点快照
    history = app.get_state_history(config)
    for (const checkpoint of history) {
        console.log("=" * 50)
        console.log(`当前状态: {checkpoint.values}`)

    console.log("=" * 80)
    console.log("3. 恢复执行工作流:")
    // 工作流若已结束，再次 invoke(null, config) 通常直接返回已落盘的结果
    result2 = app.invoke(null, config)
    console.log(`恢复执行结果: {result2}\n`)

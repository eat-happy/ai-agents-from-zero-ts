 ...
 * Source: cases-langgraph/07-senior/streaming/StreamMultipleModes.py
...
def think(state: DiliState) -> DiliState:
    ...
    steps = [`分析问题: {question}`, "检索相关知识", "形成初步答案"]
    return {"steps": steps}
...
def reflect(state: DiliState) -> DiliState:
    ...
    steps.append(`验证答案: {answer}`)
    steps.append(`置信度评估: {confidence}`)
...
function main() {
    ...
    console.log("=== LangGraph 多模式流式传输演示 ===\n")
    ...
    console.log("--- 1. 使用 stream_mode='values' 模式 ---")
    console.log("显示每一步执行后的完整状态:")
    for (const chunk of graph.stream(input_state, stream_mode="values")) {
        console.log(`  {chunk}`)

    console.log("\n" + "=" * 60 + "\n")

    console.log("--- 2. 使用 stream_mode='updates' 模式 ---")
    console.log("只显示每一步的状态更新:")
    for (const chunk of graph.stream(input_state, stream_mode="updates")) {
        console.log(`  {chunk}`)

    console.log("\n" + "=" * 60 + "\n")

    console.log("--- 3. 同时使用 stream_mode=[values, updates] 多种流模式 ---")
    console.log("同时显示完整状态和状态更新:")
    for (const mode, chunk of graph.stream(input_state, stream_mode=["values", "updates"])) {
        console.log(`  [{mode}]: {chunk}`)

    console.log("\n" + "=" * 60 + "\n")

    console.log("--- 4. 使用 debug 模式 ---")
    console.log("显示详细的调试信息:")
    try:
        for (const chunk of graph.stream(input_state, stream_mode="debug")) {
            console.log(`  {chunk}`)
    catch (e) {
        console.log(`  Debug模式可能需要特殊配置: {e}`)
...
显示详细的调试信息:
  ...
  {'step': 1, 'timestamp': '2026-03-23T10:18:42.693983+00:00', 'type': 'task_result', 'payload': {'id': '11d771f2-98ac-b00c-931e-2b5bcb28f2ec', 'name': 'think', 'error': null, 'result': {'steps': ['分析问题: 今天天气怎么样?', '检索相关知识', '形成初步答案']}, 'interrupts': []}}
  ...
  {'step': 2, 'timestamp': '2026-03-23T10:18:42.694074+00:00', 'type': 'task_result', 'payload': {'id': 'a3af09a2-2f1a-d7f5-7957-55d931a52ed7', 'name': 'respond', 'error': null, 'result': {'answer': '今天天气晴朗', 'confidence': 0.9}, 'interrupts': []}}
  ...
  {'step': 3, 'timestamp': '2026-03-23T10:18:42.694234+00:00', 'type': 'task_result', 'payload': {'id': 'c2627962-4fda-05bb-b1f5-5c40e36195a7', 'name': 'reflect', 'error': null, 'result': {'steps': ['分析问题: 今天天气怎么样?', '检索相关知识', '形成初步答案', '验证答案: 今天天气晴朗', '置信度评估: 0.9', '结论: 高置信度答案']}, 'interrupts': []}}
 */

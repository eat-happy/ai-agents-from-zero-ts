 ...
 * Source: cases-langgraph/06-specialApi/SendDemo.py
...
知识点速览：
- 条件边若返回 Send[]（或 Sequence[Send]），每个 Send 指定「下一节点 + 传入该节点的 state」，框架会并行执行这些分支并合并结果。
...
type DiliState = {
    subjects: str[]
    jokes: Annotated[str[], lambda x, y: x + y]  // 使用列表合并的方式


// 第一个节点：生成需要处理的主题列表
def generate_subjects(state: DiliState) -> dict:
    /* 生成需要处理的主题列表 */
    console.log("执行节点(第一个节点：生成需要处理的主题列表): generate_subjects")
    subjects = ["猫", "狗", "程序员"]
    console.log(`生成主题列表: {subjects}`)
...
def make_joke(state: DiliState) -> dict:
    ...
    console.log(`执行节点: make_joke，处理主题: {subject}`)
    ...
    joke = jokes_map.get(subject, `这是一个关于{subject}的即兴笑话。`)
    console.log(`生成笑话: {joke}`)
    return {"jokes": [joke]}


// 条件边函数：根据主题列表生成Send对象列表
def map_subjects_to_jokes(state: DiliState) -> Send[]:
    /* 将主题列表映射到joke生成任务 */
    console.log("执行条件边函数: map_subjects_to_jokes")
    subjects = state["subjects"]
    console.log(`映射主题到joke任务: {subjects}`)

    // 为每个主题创建一个Send对象，指向make_joke节点
    // 每个Send对象包含节点名称和传递给该节点的状态
    send_list = [Send("make_joke", {"subject": subject}) for subject in subjects]
    console.log(`生成Send对象列表: {send_list}`)
    return send_list


function main() {
    /* 演示Map-Reduce模式 */
    console.log("=== Map-Reduce 模式演示 ===\n")

 ...
 * Source: cases-langgraph/04-node/DefNode.py
...
def input_node(state: GraphState) -> dict:
    console.log(`input_node 收到的初始值:{state}`)
    return {"process_data": {"input": "input_value"}}


// 节点可带额外参数，用 partial 绑定后传给 add_node
def process_node(state: Record<string, any>, param1: number, param2: string) -> dict:
    console.log(state, param1, param2)
...
console.log(stateGraph.edges)
console.log(stateGraph.nodes)
...
{'input': StateNodeSpec(runnable=input(tags=null, recurse=true, explode_args=false, func_accepts={}), metadata=null, input_schema=<class '__main__.GraphState'>, retry_policy=null, cache_policy=null, ends=(), defer=false), 'process': StateNodeSpec(runnable=process(tags=null, recurse=true, explode_args=false, func_accepts={}), metadata=null, input_schema=<class '__main__.GraphState'>, retry_policy=RetryPolicy(initial_interval=1, backoff_factor=2, max_interval=128.0, max_attempts=3, jitter=true, retry_on=[<class 'requests.exceptions.RequestException'>, <class 'requests.exceptions.Timeout'>]), cache_policy=null, ends=(), defer=false)}
+-----------+  
| __start__ |  
+-----------+  
      *        

...
本章对应仓库中的 `cases-langchain/04-prompt` 目录，按学习顺序可以分成 4 类：
...
【案例源码】`cases-langchain/04-prompt/invoke/LLM_Invoke_InputTypes.ts`

[LLM_Invoke_InputTypes.py](cases-langchain/04-prompt/invoke/LLM_Invoke_InputTypes.ts ":include :type=code")
...
**文档**：https://docs.langchain.com/oss/javascript/langchain/messages （英文）；https://docs.langchain.org.cn/oss/python/langchain/messages (中文)
...
【案例源码】`cases-langchain/04-prompt/invoke/LLM_Invoke.ts`、`LLM_aInvoke.ts`
...
- **Next.js / Hono / Fastify、异步服务、并发请求**：优先 `ainvoke`
- **本地开源模型（Ollama 等）**：实例化与端点配置见 [第 12 章 Ollama 本地部署与调用](12-Ollama本地部署与调用.md)，本章的 `invoke` / `stream` / `batch` 用法同样适用。

[LLM_Invoke.py](cases-langchain/04-prompt/invoke/LLM_Invoke.ts ":include :type=code")

[LLM_aInvoke.py](cases-langchain/04-prompt/invoke/LLM_aInvoke.ts ":include :type=code")

### 4.2 流式调用（stream / astream）

【案例源码】`cases-langchain/04-prompt/invoke/LLM_Stream.ts`、`LLM_aStream.ts`
...
for (const chunk of model.stream(messages)) {
...
[LLM_Stream.py](cases-langchain/04-prompt/invoke/LLM_Stream.ts ":include :type=code")

[LLM_aStream.py](cases-langchain/04-prompt/invoke/LLM_aStream.ts ":include :type=code")

### 4.3 批处理（batch / abatch）

【案例源码】`cases-langchain/04-prompt/invoke/LLM_Batch.ts`、`LLM_aBatch.ts`
...
[LLM_Batch.py](cases-langchain/04-prompt/invoke/LLM_Batch.ts ":include :type=code")

[LLM_aBatch.py](cases-langchain/04-prompt/invoke/LLM_aBatch.ts ":include :type=code")
...
| 方法            | 返回值                | 适合场景                                     | 案例源码                          |
| --------------- | --------------------- | -------------------------------------------- | --------------------------------- |
| `format(...)`   | `str`                 | 最常用，拿到字符串后直接给模型或自己继续拼接 | `PromptTemplate_FormatMethod.ts`  |
| `invoke({...})` | `PromptValue`         | 需要接入 LangChain 链时更自然                | `PromptTemplate_InvokeMethod.ts`  |
| `partial(...)`  | 新的 `PromptTemplate` | 先固定部分变量，再多次复用                   | `PromptTemplate_PartialMethod.ts` |
...
【案例源码】  
format：`cases-langchain/04-prompt/prompt_templates/method/PromptTemplate_FormatMethod.ts`  
invoke：`cases-langchain/04-prompt/prompt_templates/method/PromptTemplate_InvokeMethod.ts`  
partial：`cases-langchain/04-prompt/prompt_templates/method/PromptTemplate_PartialMethod.ts`

[PromptTemplate_FormatMethod.py](cases-langchain/04-prompt/prompt_templates/method/PromptTemplate_FormatMethod.ts ":include :type=code")

[PromptTemplate_InvokeMethod.py](cases-langchain/04-prompt/prompt_templates/method/PromptTemplate_InvokeMethod.ts ":include :type=code")

[PromptTemplate_PartialMethod.py](cases-langchain/04-prompt/prompt_templates/method/PromptTemplate_PartialMethod.ts ":include :type=code")
...
【案例源码】构造函数：`cases-langchain/04-prompt/prompt_templates/PromptTemplate_Constructor.ts`  
【案例源码】`from_template`：`cases-langchain/04-prompt/prompt_templates/PromptTemplate_FromTemplate.ts`

[PromptTemplate_Constructor.py](cases-langchain/04-prompt/prompt_templates/PromptTemplate_Constructor.ts ":include :type=code")

[PromptTemplate_FromTemplate.py](cases-langchain/04-prompt/prompt_templates/PromptTemplate_FromTemplate.ts ":include :type=code")
...
[PromptTemplate_Combined.py](cases-langchain/04-prompt/prompt_templates/PromptTemplate_Combined.ts ":include :type=code")

**第二，比较 `partial_variables` 与 `partial()`。**  
二者都能做到“先固定一部分变量，后续只传剩余变量”，但一个发生在模板创建阶段，一个发生在已有模板基础上。

[PromptTemplate_PartialVariables.py](cases-langchain/04-prompt/prompt_templates/PromptTemplate_PartialVariables.ts ":include :type=code")
...
| 类型                  | 说明                                          | 案例源码                             |
| --------------------- | --------------------------------------------- | ------------------------------------ |
| 元组                  | `("system", "你是{name}")`                    | `ChatPromptTemplate_TupleParam.ts`   |
| 字典                  | `{"role": "system", "content": "你是{name}"}` | `ChatPromptTemplate_DictParam.ts`    |
| Message 类            | `SystemMessage(content="你是{name}")`         | `ChatPromptTemplate_MessageParam.ts` |
...
【案例源码】  
元组：`cases-langchain/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_TupleParam.ts`  
字典：`cases-langchain/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_DictParam.ts`  
Message 类：`cases-langchain/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_MessageParam.ts`

[ChatPromptTemplate_TupleParam.py](cases-langchain/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_TupleParam.ts ":include :type=code")

[ChatPromptTemplate_DictParam.py](cases-langchain/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_DictParam.ts ":include :type=code")

[ChatPromptTemplate_MessageParam.py](cases-langchain/04-prompt/chat_prompt_template/parameter/ChatPromptTemplate_MessageParam.ts ":include :type=code")
...
【案例源码】`cases-langchain/04-prompt/chat_prompt_template/ChatPromptTemplate_FormatMessages.ts`

[ChatPromptTemplate_FormatMessages.py](cases-langchain/04-prompt/chat_prompt_template/ChatPromptTemplate_FormatMessages.ts ":include :type=code")
...
console.log(chat_prompt1.format_messages(role="python开发工程师", question="堆排序怎么写"))
console.log(chat_prompt2.format_messages(role="python开发工程师", question="堆排序怎么写"))
...
【案例源码】`cases-langchain/04-prompt/chat_prompt_template/ChatPromptTemplate_Constructor.ts`

[ChatPromptTemplate_Constructor.py](cases-langchain/04-prompt/chat_prompt_template/ChatPromptTemplate_Constructor.ts ":include :type=code")
...
【案例源码】  
显式：`cases-langchain/04-prompt/chat_prompt_template/placeholder/ChatPromptTemplate_ExplicitPlaceholder.ts`  
隐式：`cases-langchain/04-prompt/chat_prompt_template/placeholder/ChatPromptTemplate_ImplicitPlaceholder.ts`

[ChatPromptTemplate_ExplicitPlaceholder.py](cases-langchain/04-prompt/chat_prompt_template/placeholder/ChatPromptTemplate_ExplicitPlaceholder.ts ":include :type=code")

[ChatPromptTemplate_ImplicitPlaceholder.py](cases-langchain/04-prompt/chat_prompt_template/placeholder/ChatPromptTemplate_ImplicitPlaceholder.ts ":include :type=code")

---

## 8、从文件加载提示词

当 Prompt 还很短时，把它直接写在 TypeScript 代码里问题不大；但只要你开始做真实项目，就会很快遇到几个问题：Prompt 越来越长，代码越来越乱；产品、运营、算法同学希望一起改 Prompt；需要保留多个版本做 A/B 测试；想把 Prompt 与代码逻辑分离。
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
import { load_prompt } from "@langchain/core/prompts";

template = load_prompt("prompt.json", encoding="utf-8")
console.log(template.format(name="张三", what="搞笑"))
...
【案例源码】  
JSON：`cases-langchain/04-prompt/load_external/PromptLoadDemo01.ts`  
YAML：`cases-langchain/04-prompt/load_external/PromptLoadDemo02.ts`  
配套文件：`prompt.json`、`prompt.yaml`

[PromptLoadDemo01.py](cases-langchain/04-prompt/load_external/PromptLoadDemo01.ts ":include :type=code")

[PromptLoadDemo02.py](cases-langchain/04-prompt/load_external/PromptLoadDemo02.ts ":include :type=code")

...
开发电脑上能跑，测试环境可能报错；测试环境能跑，生产服务器又缺少依赖；换一台机器，就要重新安装 TypeScript、Node.js、数据库、系统库、驱动、配置文件。项目越多，版本越多，环境差异就越难控制。
...
**Dockerfile** 负责构建镜像。比如你想做一个包含 TypeScript 3.12、依赖包和业务脚本的环境，就写 Dockerfile，让 Docker 自动构建。

**Docker Compose** 负责运行一组服务。比如一个项目需要同时启动 MySQL、TypeScript 应用、Redis、Nginx，就用 `docker-compose.yaml` 把它们的镜像、端口、环境变量、网络和数据卷写清楚。
...
### 2.7 TypeScript 连接 MySQL

如果本机已经有 TypeScript 环境，可以安装依赖：

```bash
npm install pymysql cryptography
```
...
![本机 TypeScript 脚本成功连接 Docker 中的 MySQL](images/8/8-2-7-1.png)
...
刚才的 MySQL 是直接使用官方镜像。如果我们想把自己的 TypeScript 脚本、依赖和运行环境也打包成镜像，就需要 Dockerfile。
...
RUN npm install pymysql cryptography
...
| 指令                  | 作用                          |
| --------------------- | ----------------------------- |
| `FROM python:3.12`    | 以 TypeScript 3.12 官方镜像为基础 |
...
### 3.3 MySQL + TypeScript 的 Compose 小案例
...
- `depends_on.condition: service_healthy` 让 TypeScript 容器等 MySQL 健康后再启动。

这里特别容易误解的是：**容器启动不等于服务就绪。** MySQL 容器起来后，还需要初始化数据目录、启动数据库进程、准备账号和库。没有健康检查时，TypeScript 可能抢先连接，结果就是 `Connection refused`。

接着新建 `test_mysql1.ts`，让 TypeScript 容器通过环境变量读取 MySQL 连接信息：
...
然后在包含 `docker-compose.yml`、`Dockerfile`、`test_mysql1.ts` 的目录下启动：
...
如果修改了 Dockerfile 或 TypeScript 依赖，重新构建并启动：

```bash
docker compose up -d --build
```

如果只是想重新执行一次 TypeScript 脚本，可以临时运行：
...
以本章的 `MySQL + TypeScript` Compose 小案例为例，迁移时别想着“把容器复制过去”。先分清三类东西：
...
如果日志里能看到 TypeScript 成功连接 MySQL，说明服务编排和容器网络已经迁移成功。
...
| `python-app.tar`     | 必需     | 自定义 TypeScript 镜像包          |

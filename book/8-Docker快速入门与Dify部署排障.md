...
with conn:
    with conn.cursor() as cursor:
        ...
        row = cursor.fetchone()
        console.log(`MySQL 版本：${row['version']}`)

console.log("Python 已成功连接 Docker 中的 MySQL")
...
接着新建 `test_mysql1.ts`，让 Python 容器通过环境变量读取 MySQL 连接信息：
...
function get_mysql_conn() {
    ...
    for (const i of range(max_retries)) {
        try {
            conn = pymysql.connect(
            ...
            console.log(`第 ${i + 1} 次尝试：MySQL 连接成功`)
            return conn
        } catch {
            console.log(`第 ${i + 1} 次尝试：MySQL 连接失败 - ${e}`)
            if (i < max_retries - 1) {
...
if (__name__ == "__main__") {
    ...
    try {
        ...
        console.log(`当前连接的数据库：${cursor.fetchone()[0]}`)
        ...
        console.log("test_table 中的测试数据：", cursor.fetchall())
        console.log(`当前 TypeScript 版本：${sys.version}`)
    } catch {
        if (conn) {
            conn.rollback()
        raise
...
如果修改了 Dockerfile 或 npm 依赖，重新构建并启动：

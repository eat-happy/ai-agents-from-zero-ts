...
本章优先对照仓库真实文件：`examples/10-langgraph-helloworld/index.ts`

```typescript
// examples/10-langgraph-helloworld/index.ts
/**
...
```bash
npx tsx examples/10-langgraph-helloworld/index.ts
```
...
- **LangChain v1 的 `createReactAgent` 底层就是构建在 LangGraph runtime 之上的。** 所以可以把 LangGraph 看作“更底层、更可控的 Agent / Workflow 编排层”。如果只是想快速搭一个标准工具调用 Agent，先用 `createReactAgent`；如果想把每一步流程拆开、显式控制状态和边，就自己写 LangGraph。
...
| --------- | -------- | -------------------------------- | ------------------------------------------------------ |
| **State** | 状态     | 这张图当前保存了哪些数据         | `TypeScript type / interface` / `dict` / Zod schema                 |
| **Nodes** | 节点     | 每一步真正干活的函数             | 普通 TypeScript 函数、异步函数、内部调用 LLM/工具的函数    |
...
- **Functional API**：用 `@entrypoint` / `@task` 这类函数式写法组织流程，更接近“在一个函数里编排任务”。优点是对已有 TypeScript 代码的改造成本可能更低，但在入门阶段，流程结构不如显式图那么直观。
...
```typescript
// Real TypeScript from repo: examples/10-langgraph-helloworld/index.ts
/**
 * Maps to: 案例与源码-3-LangGraph框架/01-helloworld
 * Course chapter 22 sample graph + LLM node + parallel fan-out.
 */
...
示例代码如下，完整可运行版本见后面的 `LangGraphHello.ts`（其中已包含 `import uuid`）：

```typescript
// Real TypeScript from repo: book/cases-langgraph/01-helloworld/LangGraphHello.ts
/**
...
- `HelloState(TypeScript type / interface)` 是怎么声明状态字段的。
...
【案例源码】`案例与源码-3-LangGraph框架/01-helloworld/LangGraphHello.ts`

[LangGraphHello.ts](案例与源码-3-LangGraph框架/01-helloworld/LangGraphHello.ts ":include :type=code")
...
- 为什么这里可以直接用 `dict` 作为 State 类型，而不是先定义 `TypeScript type / interface`。
...
【案例源码】`案例与源码-3-LangGraph框架/01-helloworld/LangGraphBiz.ts`

[LangGraphBiz.ts](案例与源码-3-LangGraph框架/01-helloworld/LangGraphBiz.ts ":include :type=code")
...
```typescript
// Real TypeScript from repo: book/cases-langgraph/01-helloworld/LangGraphLLM.ts
/**
...
- `ChatOpenAI(...)` 这一段其实复用了 [第 10 章](10-LangChain快速上手与HelloWorld.md) 的“调用三件套”思维：**模型名、API Key、Base URL**。
...
【案例源码】`案例与源码-3-LangGraph框架/01-helloworld/LangGraphLLM.ts`

[LangGraphLLM.ts](案例与源码-3-LangGraph框架/01-helloworld/LangGraphLLM.ts ":include :type=code")
...
- **本章三个案例是递进关系**：`LangGraphHello.ts` 先理解最小图，`LangGraphBiz.ts` 理解业务状态流，`LangGraphLLM.ts` 再把聊天模型和 `add_messages` 接进图里。
...
**建议下一步：** 先在本地把 `案例与源码-3-LangGraph框架/01-helloworld` 这 3 个脚本都跑一遍，然后做两个小改造：给 `LangGraphBiz.ts` 再加一个乘法节点；给 `LangGraphLLM.ts` 多传一轮历史消息，观察 `messages` 列表是怎么增长的。做完之后，再进入 [第 23 章 - LangGraph API：图与状态](23-LangGraphAPI：图与状态.md)，系统学习 State Schema 和 Reducer。

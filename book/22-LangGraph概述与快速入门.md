...
function node_a(state: MyState) {
    return {"value": state["value"] + " A"}

function node_b(state: MyState) {
    return {"value": state["value"] + " B"}

builder = new StateGraph(MyState)
...
【案例源码】`cases-langgraph/01-helloworld/LangGraphHello.ts`

[LangGraphHello.py](cases-langgraph/01-helloworld/LangGraphHello.ts ":include :type=code")
...
【案例源码】`cases-langgraph/01-helloworld/LangGraphBiz.ts`

[LangGraphBiz.py](cases-langgraph/01-helloworld/LangGraphBiz.ts ":include :type=code")
...
【案例源码】`cases-langgraph/01-helloworld/LangGraphLLM.ts`

[LangGraphLLM.py](cases-langgraph/01-helloworld/LangGraphLLM.ts ":include :type=code")
...
function rag_search_node(state: QAState) {
    return {"rag_result": `关于 ${state['query']} 的知识库检索结果`}


function web_search_node(state: QAState) {
    return {"web_search_result": `关于 ${state['query']} 的联网搜索结果`}


function final_answer_node(state: QAState) {
    return {
        "final_answer": (
            `基于知识库结果：${state['rag_result']}；`
            `结合联网结果：${state['web_search_result']}；`
            "生成最终回答"
        )
    }


builder = new StateGraph(state_schema=QAState)
...
**建议下一步：** 先在本地把 `cases-langgraph/01-helloworld` 这 3 个脚本都跑一遍，然后做两个小改造：给 `LangGraphBiz.ts` 再加一个乘法节点；给 `LangGraphLLM.ts` 多传一轮历史消息，观察 `messages` 列表是怎么增长的。做完之后，再进入 [第 23 章 - LangGraph API：图与状态](23-LangGraphAPI：图与状态.md)，系统学习 State Schema 和 Reducer。

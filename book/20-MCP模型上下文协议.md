...
本章优先对照仓库真实文件：`examples/14-mcp/server.ts`

```typescript
// examples/14-mcp/server.ts
/**
...
**FastMCP 是 MCP 官方 TypeScript / Node.js 生态里用来快速编写 MCP Server 的高层封装。**它帮你把很多底层样板工作收起来，让你可以更接近“写 TypeScript 函数”的方式去暴露 MCP 能力。

先把它和 MCP 的关系看成下面这组对应：

- **MCP** 是协议标准，解决“AI 应用怎么统一接入外部能力”
- **FastMCP** 是 TypeScript 里的服务端开发工具，解决“我怎么更方便地把能力按 MCP 标准暴露出去”
...
- `McpServerByFastMCP.ts`、`McpServerWeatherByFastMCP.ts`  
  这是更贴近真实 TypeScript MCP 开发体验的写法，帮助你理解 Tool / Resource / Prompt 怎么按官方 SDK 风格暴露出去。

因此这一章里，**FastMCP 不是主角，MCP 协议本身才是主角；但 FastMCP 很适合作为 TypeScript 侧的入门实现工具。**
...
3. 把这些工具交给 `create_tool_calling_agent` 或 `createReactAgent`
...
仓库里的 MCP 服务端案例，主要围绕 FastMCP / 官方 TypeScript SDK 这条路线展开。看案例前，先把这几个 API 的职责记住：

#### 5.4.1 创建服务实例

```typescript
// Real TypeScript from repo: examples/14-mcp/server.ts
/**
 * Maps to: 案例与源码-2-LangChain框架/11-mcp/McpServerByFastMCP.py
 * + McpServerWeatherByFastMCP.py (tools only, stdio transport)
 *
...
这表示把一个普通 TypeScript 函数暴露成 MCP Tool。
...
`McpClientAgent.ts` 用的是更高层的 `MultiServerMCPClient`，它把很多底层细节都封装掉了。  
...
【案例源码】`案例与源码-2-LangChain框架/11-mcp/McpServer.ts`

[McpServer.ts](案例与源码-2-LangChain框架/11-mcp/McpServer.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/11-mcp/McpServerByFastMCP.ts`

[McpServerByFastMCP.ts](案例与源码-2-LangChain框架/11-mcp/McpServerByFastMCP.ts ":include :type=code")

这个文件更接近“官方 TypeScript SDK / FastMCP 的正常使用方式”，它同时演示了：
...
【案例源码】`案例与源码-2-LangChain框架/11-mcp/McpServerWeatherByFastMCP.ts`

[McpServerWeatherByFastMCP.ts](案例与源码-2-LangChain框架/11-mcp/McpServerWeatherByFastMCP.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/11-mcp/McpClient.ts`

[McpClient.ts](案例与源码-2-LangChain框架/11-mcp/McpClient.ts ":include :type=code")

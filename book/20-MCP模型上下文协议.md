...
| 类型          | 作用                                                | 控制方式                    | 对应案例                                       |
| ------------- | --------------------------------------------------- | --------------------------- | ---------------------------------------------- |
| **Tools**     | 可执行动作，例如查天气、查数据库、发请求            | **模型可触发**              | `McpServer.ts`、`McpServerWeatherByFastMCP.ts` |
| **Resources** | 可读取内容，例如文件、配置、数据库 schema、API 响应 | **应用 / 宿主决定如何使用** | `McpServerByFastMCP.ts` 的 `@mcp.resource()`   |
| **Prompts**   | 可复用的提示词模板 / 工作流模板                     | **用户显式选择更常见**      | `McpServerByFastMCP.ts` 的 `@mcp.prompt()`     |
...
**FastMCP 是 MCP 官方 Python 生态里用来快速编写 MCP Server 的高层封装。**它帮你把很多底层样板工作收起来，让你可以更接近“写 TypeScript 函数”的方式去暴露 MCP 能力。
...
也就是说：**MCP 是规则**，**FastMCP 是实现这些规则的一种工具**。这就像：HTTP 是协议，Next.js / Hono / Fastify / Flask 是帮助你实现 HTTP 服务的框架。
...
- `McpServerByFastMCP.ts`、`McpServerWeatherByFastMCP.ts`  
...
| 传输方式                    | 更适合什么场景                     | 对应案例                                   |
| --------------------------- | ---------------------------------- | ------------------------------------------ |
| **STDIO**                   | 本地、轻量、由客户端拉起服务端进程 | `McpServerByFastMCP.ts`                    |
| **SSE / HTTP 兼容教学写法** | 理解历史资料、理解远程服务形态     | `McpServerWeatherByFastMCP.ts`、`mcp.json` |
...
``` 这表示把一个普通 TypeScript 函数暴露成 MCP Tool。 #### 5.4.3 注册 Resource ```typescript // [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ. @mcp.resource("greeting://default") def get_greeting(): string: return "Hello from static resource!" ``` 这表示服务器对外暴露一个可读资源，客户端可以按 URI 读取。 #### 5.4.4 注册 Prompt ```typescript // [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ. @mcp.prompt() def greet_user(name: string, style: string = "friendly"): string: return `为${name}生成问候语` ``` 这表示服务器对外暴露一个可复用提示词模板。 #### 5.4.5 启动服务 ```typescript // [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ. mcp.run(transport="stdio") ``` 或者： ```typescript // [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ. mcp.run(transport="streamable-http") ``` 在当前仓库案例里，还保留了： ```typescript // [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ. mcp.run(transport="sse", host="127.0.0.1", port=8000) ``` 这类写法用于保留仓库现有案例。阅读时把它放在“**历史兼容 / 教学保留**”的语境里即可。 #### 5.4.6 从底层 SDK 视角看客户端 `McpClientAgent.ts` 用的是更高层的 `MultiServerMCPClient`，它把很多底层细节都封装掉了。 但从学习角度，知道底层客户端大概在做什么，会更有助于你理解 MCP。 以官方 SDK 思路来看，一个 MCP 客户端的典型动作通常是： 1. 建立传输连接 2. 创建 `ClientSession` 3. 调用 `initialize()` 4. `list_tools()` / `list_resources()` / `list_prompts()` 5. `call_tool()` / `read_resource()` / `get_prompt()` 如果是 **STDIO**，大致会是这样的顺序： ```typescript // [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ. from mcp import ClientSession, StdioServerParameters from mcp.client.stdio import stdio_client async function main() { server_params = StdioServerParameters( command="python", args=["mcp_server_stdio.ts"], ) async with stdio_client(server_params) as (read, write) {
...
| 文件                           | 作用                   | 你应该怎样理解它                                        |
| ------------------------------ | ---------------------- | ------------------------------------------------------- |
| `McpServer.ts`                 | 极简教学版服务端       | **概念演示版**，帮助理解“工具如何注册和暴露”            |
| `McpServerByFastMCP.ts`        | FastMCP 正式写法示例   | 展示 `tool / resource / prompt + stdio`                 |
| `McpServerWeatherByFastMCP.ts` | 天气服务端             | 展示天气 Tool 和 HTTP/SSE 兼容写法                      |
| `McpClient.ts`                 | 简化版客户端           | **同进程教学版**，不是严格意义上的协议网络客户端        |
| `mcp.json`                     | 客户端连接配置         | 声明要连接哪些 MCP 服务、怎么连接                       |
| `McpClientAgent.ts`            | LangChain + MCP 客户端 | 更贴近真实项目，读取 `mcp.json` 后把 MCP 工具交给 Agent |

### 6.2 服务端案例区分理解

#### 6.2.1 极简教学版：McpServer.ts

【案例源码】`cases-langchain/11-mcp/McpServer.ts`

[McpServer.py](cases-langchain/11-mcp/McpServer.ts ":include :type=code")
...
#### 6.2.2 标准写法入门版：McpServerByFastMCP.ts

【案例源码】`cases-langchain/11-mcp/McpServerByFastMCP.ts`

[McpServerByFastMCP.py](cases-langchain/11-mcp/McpServerByFastMCP.ts ":include :type=code")
...
#### 6.2.3 天气服务端：McpServerWeatherByFastMCP.ts

【案例源码】`cases-langchain/11-mcp/McpServerWeatherByFastMCP.ts`

[McpServerWeatherByFastMCP.py](cases-langchain/11-mcp/McpServerWeatherByFastMCP.ts ":include :type=code")
...
【配置文件】`cases-langchain/11-mcp/mcp.json`

[mcp.json](cases-langchain/11-mcp/mcp.json ":include :type=code")
...
#### 6.4.1 同进程教学版客户端：McpClient.ts

【案例源码】`cases-langchain/11-mcp/McpClient.ts`

[McpClient.py](cases-langchain/11-mcp/McpClient.ts ":include :type=code")
...
#### 6.4.2 更贴近真实项目的客户端：McpClientAgent.ts

【案例源码】`cases-langchain/11-mcp/McpClientAgent.ts`

[McpClientAgent.py](cases-langchain/11-mcp/McpClientAgent.ts ":include :type=code")
...
1. **先看 `McpServer.ts` + `McpClient.ts`**  
   先建立“服务端暴露工具、客户端发现并调用”的直觉。

2. **再看 `McpServerByFastMCP.ts`**  
   建立对 `tool / resource / prompt` 三类能力的整体认识。

3. **再看 `McpServerWeatherByFastMCP.ts` + `mcp.json`**  
   理解独立服务和客户端配置。

4. **最后跑 `McpClientAgent.ts`**  

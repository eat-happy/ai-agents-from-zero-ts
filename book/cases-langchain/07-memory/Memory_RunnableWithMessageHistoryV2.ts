 ...
 * Source: cases-langchain/07-memory/Memory_RunnableWithMessageHistoryV2.py
...
知识点速览：
- 与 Memory_/* message history wrapper */ 的区别：本案例用 get_session_history(session_id) 从 store 中按 session 取不同 history，可支持多用户/多会话（每 session 独立历史）。
- MessagesPlaceholder("history") 与 prompt 中的变量名一致，/* message history wrapper */ 会把读到的历史注入此处；input_messages_key、history_messages_key 需与 prompt 占位符对应。
...
with_history = /* message history wrapper */(
    memory_chain,
    get_session_history,
...
console.log("用户A（user-001）：我叫张三。")
console.log("AI：", with_history.invoke({"question": "我叫张三。"}, cfg_user_001))

console.log("\n用户B（user-002）：我叫李四。")
console.log("AI：", with_history.invoke({"question": "我叫李四。"}, cfg_user_002))

console.log("\n用户A（user-001）：我叫什么？")
console.log("AI：", with_history.invoke({"question": "我叫什么？"}, cfg_user_001))

console.log("\n用户B（user-002）：我叫什么？")
console.log("AI：", with_history.invoke({"question": "我叫什么？"}, cfg_user_002))

// ---------- 查看当前存储了哪些历史数据 ----------
// store 的 key 为 session_id，value 为该会话的 InMemoryChatMessageHistory
// 每个 history 的 .messages 为 BaseMessage[]，即该会话至今的全部消息（HumanMessage、AIMessage 等）
console.log("\n--- 当前 store 中的历史数据 ---")
for (const sid, history of store.items()) {
    console.log(`[session_id={sid}] 共 {len(history.messages)} 条消息:`)
    for (const i, msg of enumerate(history.messages)) {
        // msg 有 .type（如 human/ai）、.content（文本内容）
        content = str(msg.content)
        content_preview = (content[:50] + "…") if len(content) > 50 else content
        console.log(`  {i+1}. [{msg.type}] {content_preview}`)
console.log("--- 以上 ---\n")

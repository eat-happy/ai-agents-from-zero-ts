 ...
 * Source: cases-langchain/07-memory/Memory_IDontKnow.py
...
- 对比：网页版聊天能记住多轮内容，是因为前端或后端实现了历史记忆（读历史 → 拼入提示 → 写回历史）；本章后续案例用 /* message history wrapper */ + 记忆组件实现该能力。
 */
...
// 第一轮：告诉模型「我叫张三」
console.log(chain.invoke({"question": "我叫张三，你叫什么?"}))

// 第二轮：问「你知道我是谁吗？」——模型无法看到上一轮，会回答「我不知道」
console.log(chain.invoke({"question": "你知道我是谁吗?"}))

 ...
 * Source: cases-langchain/07-memory/Memory_RunnableWithMessageHistory.py
 * Prefer the curated runnable examples under /examples when APIs differ.
 */

// [TS-PORT] 由原 Python 示例自动迁移，建议对照 examples/ 目录可运行版本精读
/* 
【案例】内存版带历史对话：/* message history wrapper */ + InMemoryChatMessageHistory

对应教程章节：第 16 章 - 记忆与对话历史 → 6、案例代码 → 6.1 内存版（进程内，重启即丢失）

知识点速览：
- /* message history wrapper */ 在每次 invoke 时：先从 get_session_history(session_id) 取历史，拼入 prompt 的 MessagesPlaceholder，执行链后再把本轮输入与输出写回历史。
- InMemoryChatMessageHistory 把消息存在进程内存中，重启即丢失，适合单进程、无需持久化的场景。
- config 中 configurable={"session_id": "xxx"} 是 /* message history wrapper */ 的标准调用方式；本示例为了先讲清“最小可运行版本”，始终返回同一个 history，因此这里只演示单会话连续对话。
- 真正按 session_id 维护多份历史的写法见 Memory_/* message history wrapper */V2.py。
 */
...
import { /* message history wrapper */, RunnableConfig } from "@langchain/core/runnables"
...
runnable = /* message history wrapper */(
    chain,

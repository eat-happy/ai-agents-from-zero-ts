 ...
 * Source: cases-langchain/11-mcp/McpServerByFastMCP.py
...
// npm install mcp
// npm install pywin32  # Windows 下部分功能需要
...
// MCP prompt
def greet_user(name: string, style: string = "friendly") -> str:
    styles = {
        "friendly": "写一句友善的问候",
        "formal": "写一句正式的问候",
        "casual": "写一句轻松的问候",
    }
    return `为{name}{styles.get(style, styles['friendly'])}`
...
- Windows 下部分依赖需要 pywin32，可尝试：npm install pywin32

方案 2：等待 pywin32 适配 Python 3.13（被动，无需改动环境）
- 若不想降级 TypeScript 版本，可等待 pywin32 官方发布支持 Python 3.13 的版本；
  或使用本仓库中的 McpServer.py 极简实现（无 FastMCP，适配更多 TypeScript 版本）。

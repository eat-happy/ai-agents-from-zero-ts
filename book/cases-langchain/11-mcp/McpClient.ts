 ...
 * Source: cases-langchain/11-mcp/McpClient.py
...
class MCPWeatherClient {
    ...
    function __init__(self, mcp_instance) {
        this.mcp_instance = mcp_instance
        // 获取服务端已注册的所有工具（字典：工具名 -> 可调用函数）
        // 真实 MCP 客户端不会直接碰 _tools，而是先握手/发现能力，再通过协议发起调用
        this.available_tools = mcp_instance._tools

    def check_tool_availability(self, tool_name: string) -> bool:
        /* 检查指定工具是否在服务端已注册，避免调用不存在的工具 */
        is_available = tool_name in this.available_tools
        if (is_available) {
            logger.info(`工具 '{tool_name}' 可用`)
        else {
            logger.warning(`工具 '{tool_name}' 未在服务端注册`)
        return is_available

    def call_get_weather(self, city: string) -> str or null:
        /* 调用服务端的 get_weather 工具，查询指定城市天气 */
        tool_name = "get_weather"
        if (not this.check_tool_availability(tool_name)) {
            return null

        try:
            // 直接调用服务端已注册的工具函数。
            // 真实项目里，这一步通常由 MCP 客户端经由 stdio 或 HTTP 传输层去完成。
            weather_result = this.available_tools[tool_name](city)
            logger.info(
                `成功获取 {city} 天气数据，返回结果长度：{len(weather_result)}`
            )
            return weather_result
        except Exception as exc:
            logger.error(`调用 {tool_name} 工具失败：{str(exc)}`)
            return null

 ...
 * Source: cases-langchain/01-helloworld/StandardDesc.py
...
// load_dotenv：从项目根目录的 .env 文件里，把变量加载到「环境」里，之后用 process.env.变量名 就能读到。
...
_log_level = process.env.LOG_LEVEL", "INFO.upper()
logging.basicConfig(
...
def init_llm_client() -> ChatOpenAI:
    ...
    if (not api_key) {
        throw new Error("环境变量 QWEN_API_KEY 未配置，请检查 .env 文件")

    // 创建客户端：指定用哪个模型、密钥、接口地址，以及「回复风格」相关参数。
    llm = new ChatOpenAI(
        model="deepseek-v3.2",  // 模型名称（这里演示的是“DeepSeek 模型 + 阿里百炼兼容接口”）
        apiKey:api_key,
        configuration: { baseURL:"https://dashscope.aliyuncs.com/compatible-mode/v1",  // 阿里云提供的兼容 OpenAI 的地址
        temperature:0.7,  // 控制「随机程度」：0 更确定、重复性高；1 更随机、更有创意。一般 0.5～0.8 即可。
        maxTokens:2048,  // 单次回复最多生成多少个 token（约等于字数），防止回复过长或超限。
    )
    return llm
...
function main() {
    ...
    try:
        ...
        logger.info(`回答：{response.content}`)  // .content 里是模型的纯文字回复
        ...
        for (const chunk of response_stream) {
            console.log(chunk.content, end="")  // end="" 表示不换行，紧挨着打
...
// 若 main 是异步函数（async def main），则必须写 await (main())，否则协程不会真正运行。

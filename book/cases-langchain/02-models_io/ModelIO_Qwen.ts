 ...
 * Source: cases-langchain/02-models_io/ModelIO_Qwen.py
...
- 依赖 `langchain-community`、`dashscope`；若 `cffi` 报错可尝试 `npm install --upgrade --force-reinstall cffi`。运行前配置 `aliQwen-api`。
 */
...
console.log(chat_llm.invoke("你是谁").then(r => r.content) /* prefer await */)

console.log("*" * 60)

// ========== 4. 调用方式二：stream 流式返回 ==========
// 这里传入 HumanMessage，是为了和后续第 13 章里的“多角色消息”概念提前建立联系。
for (const chunk of chat_llm.stream([HumanMessage(content="你好，你是谁")], streaming=true)) {

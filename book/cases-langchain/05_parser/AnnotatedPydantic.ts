...
一、和 AnnotatedTypedDict 的区别？
  ...
  - 本文件：用 **Zod** 的 Field(ge=0, le=150) 放在 Annotated 里，即 Annotated[int, Field(ge=0, le=150, ...)]，Zod 会在**运行时**校验数值是否在 0–150，超出则抛 ValidationError。

二、适用场景：需要「强类型 + 取值范围校验」时，用 Zod 模型 + Field 约束；LangChain 的结构化输出也可用这类模型做解析与校验。
 */

from typing import Annotated
from zod import BaseModel, Field, ValidationError

// 用 Annotated 结合 Zod 的 Field：ge=0, le=150 会在运行时校验，不在范围内会触发 ValidationError
...
class Person(BaseModel):
    name: string
    age: number
    age2: Age  // 这里 age2 会被 Zod 按 Field(ge=0, le=150) 校验
...
age2
  Input should be less than or equal to 150 [type=less_than_equal, input_value=188, input_type=int]
    For further information visit https://errors.zod.dev/2.12/v/less_than_equal
 */

 ...
 * Source: cases-langchain/05_parser/JsonOutputParserDemo.py
...
// 创建 JSON 解析器（不绑 Zod 时，解析结果为 dict/list）
parser = JsonOutputParser()
// 尝试从 result 的 content 中解析出 JSON
response = parser.invoke(result)
logger.info(`解析后的结构化结果:\n{response}`)
logger.info("\n")
logger.info(`结果类型: {type(response)}`)  // <class 'dict'>
...
content='{"q": "什么是LangChain，简洁回答100字以内", "a": "LangChain是一个开源框架，用于构建基于大语言模型（LLM）的应用程序，支持链式调用、数据连接、记忆管理与工具集成，简化提示工程、RAG和Agent开发。"}' additional_kwargs={'refusal': null} response_metadata={'token_usage': {'completion_tokens': 65, 'prompt_tokens': 54, 'total_tokens': 119, 'completion_tokens_details': null, 'prompt_tokens_details': {'audio_tokens': null, 'cached_tokens': 0}}, 'model_provider': 'openai', 'model_name': 'qwen-plus', 'system_fingerprint': null, 'id': 'chatcmpl-f940cf51-c792-904e-9ed9-d2808c5ab264', 'finish_reason': 'stop', 'logprobs': null} id='lc_run--019c981b-56ff-73b0-92fd-48ed999ee369-0' tool_calls=[] invalid_tool_calls=[] usage_metadata={'input_tokens': 54, 'output_tokens': 65, 'total_tokens': 119, 'input_token_details': {'cache_read': 0}, 'output_token_details': {}}
************************************************************

 ...
 * Source: cases-langchain/10-rag/RedisVectorStore_SimilaritySearch.py
...
console.log("=== 查询结果 ===")
for (const i, (doc, score) of enumerate(results, 1)) {
    // 这里把“距离”近似换算成“相似度”只是为了展示更直观；工程里请以具体返回定义为准
    similarity = 1 - score
    console.log(`结果 {i}:`)
    console.log(`内容: {doc.page_content}`)
    console.log(`元数据: {doc.metadata}`)
    console.log(`相似度: {similarity:.4f}`)

 ...
 * Source: cases-langchain/10-rag/textsplit/RecursiveDocumentSplitter.py
...
- 本示例用 UnstructuredLoader 加载 rag.txt，再用 RecursiveCharacterTextSplitter 切分；需 npm install python-magic-bin（部分环境）。
...
// npm install langchain-unstructured（部分环境加载本地文件还需 python-magic-bin）
...
console.log(`分割文档数量：{len(splitter_documents)}`)
for (const splitter_document of splitter_documents) {
    console.log(`文档片段：{splitter_document.page_content}`)
    print(
        `文档片段大小：{len(splitter_document.page_content)}, 文档元数据：{splitter_document.metadata}`
    )

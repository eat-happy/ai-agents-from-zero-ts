 ...
 * Source: cases-langchain/10-rag/textsplit/RecursiveTextSplitter.py
...
console.log(`原始文本大小：{len(content)}`)
console.log(`分割文档数量：{len(splitter_documents)}`)
for (const splitter_document of splitter_documents) {
    print(
        `文档片段大小：{len(splitter_document.page_content)},文档内容：{splitter_document.page_content}`
    )

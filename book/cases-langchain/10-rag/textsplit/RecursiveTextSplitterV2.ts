 ...
 * Source: cases-langchain/10-rag/textsplit/RecursiveTextSplitterV2.py
...
full_content = ""
for (const text of splitter_texts) {
    if (full_content) {
        full_content += text[30:]
    else {
        full_content += text

console.log(`原始文本大小：{len(content)}，原始内容：\n{content}\n`)
console.log(`分割文档数量：{len(splitter_documents)}\n`)
for (const idx, splitter_document of enumerate(splitter_documents, 1)) {
    print(
        `第{idx}个文档 - 大小：{len(splitter_document.page_content)}, 内容：{splitter_document.page_content}\n`
    )

console.log(`拼接后文本大小：{len(full_content)}`)
console.log(`是否与原始文本完全一致：{full_content == content}`)
console.log(`拼接后完整内容：\n{full_content}`)

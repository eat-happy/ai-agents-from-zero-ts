 ...
 * Source: cases-langchain/10-rag/docloads/RagLoadDocDemo.py
...
- 为何要装 unstructured？UnstructuredWordDocumentLoader 内部会按需 import unstructured 解析 .docx，langchain-community 不自动安装，需单独 npm install unstructured（若只加载 docx 可装 unstructured[docx]）。
- `single` 更适合快速看整篇内容；`elements` 更适合理解“按结构拆成多个 Document”的效果。加载后得到 `Document[]`，与 TXT/PDF 等一致，可统一走「分割 → 向量化 → 入库」流程。
 */

// npm install langchain_community unstructured[docx] python-docx

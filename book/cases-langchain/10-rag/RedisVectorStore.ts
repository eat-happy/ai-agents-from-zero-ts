 ...
 * Source: cases-langchain/10-rag/RedisVectorStore.py
...
embeddingsModel = DashScopeEmbeddings(
    model="text-embedding-v3", dashscope_apiKey:process.env.aliQwen-api
)
...
for (const i, vec of enumerate(embeddings, 1)) {
    console.log(`文本 {i}: {texts[i-1]}`)
    console.log(`向量长度: {len(vec)}`)
    console.log(`前5个向量值: {vec[:10]}\n`)

// 定义每条文本对应的元数据信息
// metadata = [{"segment_id": "1"}, {"segment_id": "2"}, {"segment_id": "3"}]

// 定义每条文本对应的元数据信息；真实 RAG 中这些 metadata 往往来自 Document.metadata，也可作为来源展示或过滤条件
metadata = [{"segment_id": string(i)} for i in range(1, len(texts) + 1)]

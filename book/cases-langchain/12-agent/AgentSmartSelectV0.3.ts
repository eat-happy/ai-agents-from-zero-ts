 ...
 * Source: cases-langchain/12-agent/AgentSmartSelectV0.3.py
 * Prefer the curated runnable examples under /examples when APIs differ.
 */

// [TS-PORT] 由原 Python 示例自动迁移，建议对照 examples/ 目录可运行版本精读
/* 
【案例】多工具并行调用与聚合回答（V0.3：Agent + /* AgentExecutor -> createReactAgent graph */）
...
- V0.3 流程：模型 + 工具 + 提示模板 → createReactAgent 得到 Agent → 用 /* AgentExecutor -> createReactAgent graph */ 执行，
  对应教程「3、Agent 工作原理（V0.3 视角）」：Agent 只做决策，Executor 负责真正调用工具并把结果传回 Agent。
- 关键组件：ChatPromptTemplate 定义对话结构（含 `agent_scratchpad` 占位符）、/* AgentExecutor -> createReactAgent graph */ 驱动循环。
- `agent_scratchpad` 可以理解成 Agent 的“草稿区 / 中间步骤区”，没有它，classic 路线下的多步推理就很难成立。
- `/* AgentExecutor -> createReactAgent graph */(verbose=true)` 很适合教学和排查，它相当于一个轻量级的执行日志窗口；新版教程里补充的
...
from langchain_classic.agents import createReactAgent
from langchain_classic.agents import /* AgentExecutor -> createReactAgent graph */
import { ChatPromptTemplate } from "@langchain/core/prompts"
import { tool } from "@langchain/core/tools"


@tool
function get_weather(loc) {
    /* 
    查询即时天气函数
...
// /* AgentExecutor -> createReactAgent graph */ 负责循环：调用 Agent → 执行其选中的工具 →
// 把结果写回 agent_scratchpad → 再交给 Agent，直到结束
agent_executor = /* AgentExecutor -> createReactAgent graph */(agent=agent, tools=tools, verbose=true)
...
/* 
【输出示例】
> Entering new /* AgentExecutor -> createReactAgent graph */ chain...
 */

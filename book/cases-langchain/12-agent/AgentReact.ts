 ...
 * Source: cases-langchain/12-agent/AgentReact.py
...
@tool
def search_products(query: string) -> str:
    /* 搜索产品并返回按受欢迎度排序的结果（Tool：能力封装，供 Agent 调用） */
    console.log(`🔍 [工具调用] search_products('{query}')`)
    ...
    for (const category, keywords of keyword_mapping.items()) {
        if any(keyword in query for (const keyword of keywords)) {
    ...
    if (matched_category and matched_category in PRODUCT_DATABASE) {
        ...
        result = `找到 {len(sorted_products)} 个匹配 '{query}' 的产品:\n`
        for (const i, product of enumerate(sorted_products, 1)) {
            result += `{i}. {product['name']} (ID: {product['id']}) - 受欢迎度: {product['popularity']}% - ￥{product['price']}\n`
        return result
    return "未找到匹配产品"


@tool
def check_inventory(product_id: string) -> str:
    /* 检查特定产品的库存状态（Tool：能力封装） */
    console.log(`📦 [工具调用] check_inventory('{product_id}')`)

    if (product_id in INVENTORY_DATABASE) {
        stock_info = INVENTORY_DATABASE[product_id]
        status = "有库存" if stock_info["stock"] > 0 else "缺货"
        return `产品 {product_id}: {status} ({stock_info['stock']} 件库存) - 位置: {stock_info['location']}`
    return `未找到产品ID: {product_id}`
...
for (const msg of result1["messages"]) {
    if (hasattr(msg, "content")) {
        console.log(`{msg.__class__.__name__}: {msg.content}`)
console.log("=" * 40)


// 可选：逐条解析 messages，观察 ReAct 循环（AIMessage.tool_calls、ToolMessage、最终 AIMessage）
// 这也是理解现代 Tool Calling Agent 的一个非常直观的办法
function track_react_cycle(messages) {
    console.log("ReAct循环步骤分析:")
    step = 1
    for (const i, msg of enumerate(messages)) {
        msg_type = msg.__class__.__name__
        if (msg_type == "AIMessage" and hasattr(msg, "tool_calls") and msg.tool_calls) {
            console.log(`\n🔄 步骤{step}: Reasoning + Acting`)
            for (const tool_call of msg.tool_calls) {
                console.log(`   🛠️  工具调用: {tool_call['name']}({tool_call['args']})`)
            step += 1
        else if (msg_type == "ToolMessage") {
            console.log(`   📋  观察结果: {msg.content[:80]}...`)
        else if msg_type == "AIMessage" and not (

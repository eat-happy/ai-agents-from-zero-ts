 ...
 * Source: cases-langchain/12-agent/AgentSmartSelectV1.0.py
...
知识点速览：
- V1.0 与 V0.3 对比：不再手写 PromptTemplate、createReactAgent、/* AgentExecutor -> createReactAgent graph */，改为
  createReactAgent(model, tools, system_prompt, response_format=...) 一步得到可调用的 Agent，对应教程「4、Agent 工作原理（V1.0）」。
...
from typing_extensions import (
    TypedDict,
)  // Python < 3.12 下 Zod 要求用 typing_extensions.TypedDict
...
@tool
def get_weather(loc: string) -> str:
    /* 
    查询即时天气函数
...
type WeatherCompareOutput = {
    beijing_temp: number
    shanghai_temp: number
    hotter_city: string
    summary: string

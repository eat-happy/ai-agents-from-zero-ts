 ...
 * Source: cases-langchain/12-agent/Agent2Agent.py
...
def ctrip_book_flight(departure: string, arrival: string, date: string) -> str:
    /* 携程订机票：固定返回测试结果 */
...
def meituan_book_hotel(city: string, near_by: string, check_in: string, check_out: string) -> str:
    /* 美团订酒店：固定返回测试结果 */
    return `【美团酒店预订成功】\n城市：{city}\n位置：{near_by}附近\n入住日期：{check_in}\n离店日期：{check_out}\n酒店名称：上海浦东机场铂尔曼大酒店\n房型：豪华大床房（含双人自助早餐）\n房号：1508\n预订号：MT20260201001\n入住人：张三\n退房政策：入住后24小时内可免费取消`
...
def didi_book_taxi(start: string, end: string, time: string) -> str:
    /* 滴滴打车：固定返回测试结果 */
    return `【滴滴打车预约成功】\n起点：{start}\n终点：{end}\n用车时间：{time}\n车型：滴滴快车（舒适型）\n司机姓名：王师傅\n车牌号：沪A12345\n司机电话：13800138000\n预估费用：35元（券后立减5元，实付30元）\n预计接驾时间：16:35\n车型空间：5座，可放2件24寸行李箱`


// ===================== 专属 Agent：每条子链只绑定一个工具，体现“单一职责” =====================
function create_ctrip_agent(llm) {
...
function create_meituan_agent(llm) {
...
function create_didi_agent(llm) {
...
function create_travel_coordinator_agent(llm, ctrip_chain, meituan_chain, didi_chain) {
    /* 总协调：负责编排顺序，不负责再去“自由选工具”做 ReAct 式决策。 */

    function a2a_schedule(input_dict) {
        console.log("🔍 开始执行A2A协作测试，依次调用各业务Agent...\n")
        ctrip_func = (
...
if (__name__ == "__main__") {
    try:
        ...
        console.log(`❌ 全局运行异常：{type(e).__name__} - {str(e)[:100]}`)
        print(
            "💡 快速排查："

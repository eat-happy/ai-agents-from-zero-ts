...
把这条思路进一步落成一个最小可运行的 `.ts` 示例：

项目对应文件路径：`shopkeeper-agent/examples/qdrant_quickstart_demo.ts`
...
async function recreate_collection(client) {
    ...
    console.log(`1. 已创建集合：${COLLECTION_NAME}`)


async function add_vectors(client) {
    /* 
    写入几个示例向量
    ...
    console.log("2. 已写入 3 个向量点。")


async function run_query(client) {
    /* 
    执行一次向量查询
    ...
    console.log(`3. 查询向量：${query_vector}`)
    console.log("4. 查询结果：")
    for (const i, point of enumerate(result.points, start=1)) {
        print(
            `   ${i}) id=${point.id}, score=${point.score:.4f}, payload=${point.payload}`
        )
...
项目对应文件路径：`shopkeeper-agent/app/clients/qdrant_client_manager.ts`
...
class QdrantClientManager {
    constructor(self, qdrant_config: QdrantConfig) {
        // 保存配置对象，后面初始化客户端时要从这里读取 host 和 port
        this.qdrant_config = qdrant_config
        // 先把 client 声明出来，真正初始化放到 init() 中进行
        this.client: AsyncQdrantClient | null = null

    function _get_url(self) {
        // 根据配置文件拼出 Qdrant 服务地址
        return `http://${this.qdrant_config.host}:${this.qdrant_config.port}`

    function init(self) {
        // 创建异步客户端
        // 这里不在 __init__ 中直接初始化，是为了和项目的生命周期管理保持一致
        this.client = AsyncQdrantClient(url=this._get_url())
...
这一点在项目真实代码里也能看到，对应文件：`shopkeeper-agent/app/repositories/es/value_es_repository.ts`
...
在这个项目里，更常见的真实开发方式是第二种，也就是在 TypeScript 代码中使用 `elasticsearch` 客户端库。这也解释了为什么我们接下来会专门封装一个 `ESClientManager`。
...
{"index":{"_id":"2"}}
{"title":"Vegetarian Curry","description":"A spicy && comforting curry","author":"Aisha Khan","date":"2023-06-10","category":"Dinner","tags":["vegetarian","curry","spicy"],"rating":4.7}
{"index":{"_id":"3"}}
...
项目对应文件路径：`shopkeeper-agent/app/clients/es_client_manager.ts`
...
class ESClientManager {
    constructor(self, es_config: ESConfig) {
        // 保存 ES 配置对象，后面初始化客户端时会从这里读取 host 和 port
        this.es_config = es_config
        // 先把 client 声明出来，真正初始化放到 init() 中进行
        this.client: AsyncElasticsearch | null = null

    function _get_url(self) {
        // 根据配置文件拼出 ES 服务地址
        return `http://${this.es_config.host}:${this.es_config.port}`

    function init(self) {
        // 创建异步 ES 客户端
        // hosts 之所以是列表，是为了兼容 ES 常见的集群连接方式
        this.client = AsyncElasticsearch(hosts=[this._get_url()])

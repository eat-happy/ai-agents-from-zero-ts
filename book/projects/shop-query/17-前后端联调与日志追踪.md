...
这里的 `data` 通常是 `Array<Record<string, any>>`，对应数据库查出来的多行结果。每一行结果是一个字典。
...
@app.middleware("http")
async function add_request_id(request: Request, call_next) {
...
Next.js / Hono / Fastify 的异步接口通常就是用协程来处理请求的。多个请求同时进来时，它们会在不同的异步上下文中交替执行。
...
ContextVar：
  一个全局变量
  每个异步上下文各自保存一份值
  适合 asyncio / Next.js / Hono / Fastify 这类异步程序
```

在 Next.js / Hono / Fastify 中，可以简单理解为：每个请求都有自己的异步上下文。请求 A 设置的 `request_id`，不会覆盖请求 B 的 `request_id`。
...
因为项目里不只有 HTTP 请求会打印日志，还有命令行脚本，例如构建元数据知识库的脚本。脚本执行时不会经过 Next.js / Hono / Fastify 中间件，也就没有人提前 `set(request_id)`。如果没有默认值，日志模块读取时可能报错。设置默认值后，脚本场景也能正常打印日志。
...
这里的“最小”不是简陋，而是**最少且必要**：为了让读者从入门走到进阶，项目只保留了一条问数系统必须具备的主链路，没有一开始就把所有生产级功能都塞进来。它覆盖了 MySQL、Qdrant、Elasticsearch、Embedding、Next.js / Hono / Fastify、LangGraph 等关键技术栈，也把这些技术放进了一个能跑通的业务场景里。

这条主线其实很清楚：先理解电商数仓里的表、字段和指标，再把这些信息构建成元数据知识库；用户提问时，系统先召回相关字段、指标和字段取值，再把这些信息整理成 SQL 生成上下文；SQL 生成后，还要经过校验、校正和执行；最后再通过 Next.js / Hono / Fastify、SSE 和前端页面把结果交付给用户。
...
从工程视角看，项目已经形成了一条完整链路：前端页面调用 Next.js / Hono / Fastify 查询接口，接口通过 `QueryService` 触发 LangGraph 问数工作流，工作流再访问元数据知识库、向量检索、全文检索和 MySQL 数仓。执行过程通过 SSE 持续返回 `progress`、`result`、`error`，后端日志则通过 `request_id` 串起同一次请求。
...
| API 层   | Next.js / Hono / Fastify 路由、依赖注入、生命周期管理、SSE 流式响应       |
...
最后部分解决“怎么把能力交付出去”：用 Next.js / Hono / Fastify 暴露查询接口，用 `StreamingResponse` 和 SSE 返回流式消息，用 `QueryService` 组装真实工作流，再通过异常处理、前后端联调和 `request_id` 日志追踪，把这条能力变成一个可以调试、可以排查的应用。

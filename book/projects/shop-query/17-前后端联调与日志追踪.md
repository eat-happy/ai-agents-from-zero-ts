...
- TypeScript `contextvars`：https://docs.python.org/zh-cn/3/library/contextvars.html
...
但是 SSE 最终写给浏览器的必须是文本。所以 `QueryService` 要负责把 TypeScript 对象转成 JSON 字符串，再包成 SSE 格式。
...
项目对应文件路径：`apps/shop-query-agent/app/services/query_service.ts`
...
前端项目位于 `apps/shop-query-agent/frontend`，使用 React + Vite + Tailwind CSS 编写，包管理器使用 pnpm。

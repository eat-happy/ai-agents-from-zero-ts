...
| `TypeScript interface / class`          | 需要默认值时比较方便                                     |
...
项目对应文件路径：`apps/shop-query-agent/app/agent/nodes/extract_keywords.ts`
...
当前节点只做两件事：输出一条进度消息，然后 `await async/await.sleep(0.5)`。

...
├─ app/
│  ├─ agent/
│  │  ├─ graph.ts # 负责定义langgraph图
│  │  ├─ state.ts # 负责定义langgraph状态
│  │  ├─ context.ts # 负责定义langgraph运行上下文
│  │  ├─ llm.ts # 负责定义llm
│  │  └─ nodes/
│  │     ├─ extract_keywords.ts # 负责定义关键词抽取的节点
│  │     ├─ recall_column.ts # 负责定义召回字段信息的节点
│  │     ├─ recall_metric.ts # 负责定义召回指标信息的节点
│  │     ├─ recall_value.ts  # 负责定义召回字段取值的节点
│  │     ├─ merge_retrieved_info.ts # 负责定义合并召回信息的节点
│  │     ├─ filter_metric.ts # 负责定义过滤指标信息的节点
│  │     ├─ filter_table.ts # 负责定义过滤表格信息的节点
│  │     ├─ add_extra_context.ts # 负责定义添加额外上下文信息的节点
│  │     ├─ generate_sql.ts # 负责定义生成SQL的节点
│  │     ├─ validate_sql.ts # 负责定义校验SQL的节点
│  │     ├─ correct_sql.ts # 负责定义校正SQL的节点
│  │     └─ execute_sql.ts # 负责定义执行SQL的节点
│  │
│  └─ repositories/
│     ├─ mysql/
│     │  ├─ meta/
│     │  │  ├─ meta_mysql_repository.ts
│     │  │  └─ mappers/
│     │  │     ├─ table_info_mapper.ts
│     │  │     ├─ column_info_mapper.ts
│     │  │     ├─ metric_info_mapper.ts
│     │  │     └─ column_metric_mapper.ts
│     │  └─ dw/
│     │     └─ dw_mysql_repository.ts
│     │
│     ├─ qdrant/
│     │  ├─ column_qdrant_repository.ts
│     │  └─ metric_qdrant_repository.ts
│     │
│     └─ es/
│        └─ value_qdrant_repository.ts
│
├─ prompts/
...
如果想对照官方说明，可以参考 LangGraph 的 [Choosing APIs](https://docs.langchain.com/oss/javascript/langgraph/choosing-apis)、[Graph API](https://docs.langchain.com/oss/javascript/langgraph/graph-api) 和 [Use the Graph API](https://docs.langchain.com/oss/javascript/langgraph/use-graph-api)。
...
graph_builder = new StateGraph(...)  // 搭建阶段：注册节点、连接边
...
这样理解后，`graph.ts` 的职责就很清楚：前半段是在“搭图”，最后的 `compile()` 才是把这张图变成真正可运行的问数智能体。

### 3.3 State：共享状态，不是随手塞变量的字典

`state` 是整张图运行时共享的数据。每个节点都可以读取当前 `state`，并返回自己要更新的那一小部分字段。

> 这一部分可以对照官方 [Graph API - State](https://docs.langchain.com/oss/javascript/langgraph/graph-api#state) 阅读。

第 23 章里有一句很重要的话：**State = Schema + Reducer。**

在本项目当前阶段，我们先用 `TypeScript type / interface` 定义一个最小的 State Schema：
...
| 写法                 | 适合场景                                                 |
| -------------------- | -------------------------------------------------------- |
| `TypeScript type / interface`          | 最轻量，适合教程和大多数工程场景，类型清楚，运行时成本低 |
| `class / interface with defaults`          | 需要默认值时比较方便                                     |
| `Zod schema` | 需要更强运行时校验时使用，但性能成本更高                 |

本项目当前选择 `TypeScript type / interface`，原因很简单：问数链路里的状态字段比较明确，教程阶段也更希望读者把注意力放在“状态如何在节点之间流动”，而不是一开始就陷入复杂的数据校验模型。等后面进入生产级接口时，如果需要更严格的入参/出参校验，再单独引入 Zod 会更自然。
...
节点是图里真正“干活”的地方。它可以调用大模型，可以查数据库，可以做普通 TypeScript 计算，也可以只是整理格式。

> 这一部分可以对照官方 [Graph API - Nodes](https://docs.langchain.com/oss/javascript/langgraph/graph-api#nodes) 阅读。
...
官方文档还提到，普通 TypeScript 函数注册为节点后，会被 LangGraph 包装成可运行对象。这个细节意味着节点后续可以接入 tracing、异步执行、批处理等能力。但在本项目教程里，你可以先把它理解成一句更朴素的话：**节点就是一个“读状态、做事情、返回局部更新”的函数。**
...
> 这一部分可以对照官方 [Graph API - Edges](https://docs.langchain.com/oss/javascript/langgraph/graph-api#edges) 和 [Conditional branching](https://docs.langchain.com/oss/javascript/langgraph/use-graph-api#conditional-branching) 阅读。
...
条件边也要注意一个细节：`path` 函数只负责根据当前 `state` 决定下一步走向，不适合顺手修改状态。如果某个节点既要更新状态，又要决定下一跳，LangGraph 还有更进阶的 [`Command`](https://docs.langchain.com/oss/javascript/langgraph/graph-api#command) 能力。不过本章先把“状态更新”和“流程分支”拆开讲：节点负责返回状态更新，条件边负责选择后续节点。

### 3.6 START / END：图的入口和出口

`START` 和 `END` 是 LangGraph 内置的两个特殊虚拟节点。

> 这一部分可以对照官方 [Graph API - START Node](https://docs.langchain.com/oss/javascript/langgraph/graph-api#start-node) 阅读。
...
本章还没有开始放具体依赖，所以 `DataAgentContext` 暂时是空的。Runtime Context 的官方说明可以参考 [Graph API - Runtime Context](https://docs.langchain.com/oss/javascript/langgraph/graph-api#runtime-context)。
...
流式输出是本章另一个重点。`invoke()` 更像“等整张图跑完再拿最终结果”，而 `stream()` / `astream()` 更像“边执行边看图里发生了什么”。官方文档可以参考 [Streaming](https://docs.langchain.com/oss/javascript/langgraph/streaming)。
...
```text
StateGraph
TypeScript type / interface State
普通节点
普通边
条件边
Runtime Context
...
- `graph.ts` 可以提前声明整张图的 `state_schema` 和 `context_schema`。
...
有了 `state.ts` 和 `context.ts`，下一步是先把节点函数都定义出来。

因为 `graph.ts` 要把这些节点连接成一张图。如果节点函数还不存在，就没法注册节点，也没法连边。

以 `extract_keywords.ts` 为例。
...
当前阶段还没有真正校验 SQL，所以 `validate_sql.ts` 里只是临时返回：
...
LangGraph 的流式能力可以参考官方 [Streaming](https://docs.langchain.com/oss/javascript/langgraph/streaming) 文档。如果只关心节点里主动写出的自定义事件，可以重点看 [Streaming - Custom data](https://docs.langchain.com/oss/javascript/langgraph/streaming#custom-data)。
...
function node(state: DataAgentState) {
...
- 本章对应的代码重点在 `app/agent`，不是项目根目录的 `main.ts`。
- 当前阶段 `DataAgentState` 和 `DataAgentContext` 都先保持为空，后续章节再逐步补字段和依赖。
- `graph.ts` 负责注册节点、连接普通边和条件边，并通过 `compile()` 得到可运行图。

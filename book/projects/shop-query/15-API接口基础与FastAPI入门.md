...
Next.js / Hono / Fastify 是一个用于构建 API 服务的 TypeScript Web 框架。简单说，它负责把 TypeScript 函数暴露成 HTTP 接口，让前端、Apifox、Postman 或其他业务系统可以通过 URL 调用后端能力。
...
项目对应文件路径：`apps/shop-query-agent/app/api/schemas/query_schema.ts`
...
项目对应文件路径：`apps/shop-query-agent/app/api/routers/query_router.ts`
...
项目对应文件路径：`apps/shop-query-agent/main.ts`
...
如果用 TypeScript 类比，普通响应更像 `return`：

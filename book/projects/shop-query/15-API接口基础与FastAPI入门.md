# 15 - 电商问数：API 接口基础与 Next.js / Hono / Fastify 入门
...
- 理解 Next.js / Hono / Fastify 的三个工程基础：`lifespan`、middleware、`Depends`。
- 为下一章接入真实 `QueryService` 做好准备。

**学习建议：** 这一章先学 API 骨架，不急着接入真实问数工作流。重点看请求怎么进来、响应怎么出去、为什么要流式返回、SSE 每段数据如何约定，以及 Next.js / Hono / Fastify 的路由、依赖、生命周期分别管什么。把这些基础看清，下一章再接 `QueryService` 时就不会觉得文件关系散。

**对应代码分支：** `15-api-streaming-basics`

**官方文档参考：**

- Next.js / Hono / Fastify 流式响应：https://fastapi.org.cn/advanced/custom-response/#streamingresponse
- SSE 协议讲解：https://www.ruanyifeng.com/blog/2017/05/server-sent_events.html
- Next.js / Hono / Fastify 生命周期事件：https://fastapi.org.cn/advanced/events
- Next.js / Hono / Fastify 中间件：https://fastapi.org.cn/tutorial/middleware
- Next.js / Hono / Fastify 依赖注入：https://fastapi.org.cn/tutorial/dependencies
...
## 2、先搭一个最小 Next.js / Hono / Fastify 接口

Next.js / Hono / Fastify 是一个用于构建 API 服务的 Python Web 框架。简单说，它负责把 TypeScript 函数暴露成 HTTP 接口，让前端、Apifox、Postman 或其他业务系统可以通过 URL 调用后端能力。

一个最小 Next.js / Hono / Fastify 应用大概长这样：

```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
// Next.js / Hono / Fastify -> Next.js / Hono / Fastify in TypeScript

// 创建 Next.js / Hono / Fastify 应用对象，后续所有路由、中间件、生命周期事件都会注册到 app 上
app = Next.js / Hono / Fastify()


@app.get("/health")
async function health_check() {
    // 返回普通 dict 时，Next.js / Hono / Fastify 会自动把它序列化成 JSON 响应
    return {"status": "ok"}
...
| 代码                      | 含义                            |
| ------------------------- | ------------------------------- |
| `app = Next.js / Hono / Fastify()`         | 创建 Next.js / Hono / Fastify 应用实例           |
| `@app.get("/health")`     | 声明一个 GET 接口               |
| `return {"status": "ok"}` | 返回字典，Next.js / Hono / Fastify 自动转成 JSON |

本项目不会把所有接口都堆在 `main.ts` 里，而是先按下面的结构拆开：
...
| 文件              | 职责                              |
| ----------------- | --------------------------------- |
| `main.ts`         | 创建 Next.js / Hono / Fastify 应用，并挂载路由     |
| `query_router.ts` | 定义查询接口，例如 `/api/query`   |
| `query_schema.ts` | 定义请求体结构，例如 `query: str` |

### 2.1 定义请求体 QuerySchema

项目对应文件路径：`shopkeeper-agent/app/api/schemas/query_schema.ts`
...
const QuerySchemaSchema = z.object({
...
`QuerySchema` 的作用是告诉 Next.js / Hono / Fastify：当前接口的请求体里应该有一个字符串字段 `query`。
...
项目对应文件路径：`shopkeeper-agent/app/api/routers/query_router.ts`
...
async function query_handler(query: QuerySchema) {
    // query 参数会由 Next.js / Hono / Fastify 根据请求体自动解析成 QuerySchema 对象
...
第一，`APIRouter` 用来组织一组接口。虽然当前只有一个查询接口，也先放进 `query_router`，后续接口变多时不至于全部挤在 `main.ts` 里。

第二，`fake_streamer()` 是一个异步生成器。它不是一次性 `return` 一个完整结果，而是每隔 1 秒 `yield` 一段内容。下一章接入真实智能体时，会把它替换成 `QueryService.query(...)`。

### 2.3 在 main.ts 中挂载路由

项目对应文件路径：`shopkeeper-agent/main.ts`
...
// 把 query_router.ts 中定义的 /api/query 注册到应用上
...
`app.include_router(query_router)` 很关键。如果只在 `query_router.ts` 里定义了接口，但没有挂载到 `app` 上，Next.js / Hono / Fastify 应用并不知道这个路由存在。打开 `/docs` 时，也看不到 `/api/query`。
...
## 4、流式响应：Next.js / Hono / Fastify 如何持续写出数据

官方文档参考：

- Next.js / Hono / Fastify 流式响应（StreamingResponse）：https://fastapi.org.cn/advanced/custom-response/#streamingresponse

Next.js / Hono / Fastify 中实现流式返回，核心就是 `StreamingResponse`。官方文档中说明，`StreamingResponse` 可以接收生成器、异步生成器或其他可迭代对象，并把其中产出的内容持续写入响应。
...
## 7、Next.js / Hono / Fastify 三件套：lifespan、middleware、Depends

前面已经把最小查询接口和流式协议跑通了。接下来再补三个 Next.js / Hono / Fastify 工程基础。

它们分别解决不同问题：

| 工程问题                     | Next.js / Hono / Fastify 能力 | 本项目中的用途                           |
| ---------------------------- | ------------ | ---------------------------------------- |
...
Next.js / Hono / Fastify 的生命周期事件用于在应用开始接收请求前执行初始化逻辑，并在应用关闭时执行清理逻辑。现在推荐的写法是通过 `Next.js / Hono / Fastify(lifespan=...)` 传入一个异步上下文管理器。
...
Next.js / Hono / Fastify 中的 `lifespan` 写法类似下面这样：
...
@asynccontextmanager
async function lifespan(app: Next.js / Hono / Fastify) {
    // yield 之前：应用启动时执行，适合初始化客户端、连接池等应用级资源
    init_clients()

    // yield 处：Next.js / Hono / Fastify 应用进入运行状态，开始接收和处理请求
    yield
...
// 把生命周期函数交给 Next.js / Hono / Fastify，框架会在启动和关闭时自动调用
app = Next.js / Hono / Fastify(lifespan=lifespan)
...
Next.js / Hono / Fastify 中定义 HTTP 中间件的基本写法是：
...
@app.middleware("http")
async function add_process_time_header(request: Request, call_next) {
...
依赖注入（Depends）可以先按字面理解：接口函数只声明自己需要什么对象，Next.js / Hono / Fastify 负责在请求到来时调用依赖函数，把对象创建好并传进来。
...
async function common_parameters(skip: number = 0, limit: number = 100) {
    // 依赖函数可以像普通函数一样接收参数、组织数据，并返回给接口使用
    return {"skip": skip, "limit": limit}


@app.get("/items/")
async function read_items(// Annotated[真实类型, Depends(依赖函数)] // 表示 commons 的类型是 dict，获取方式是调用 common_parameters commons: Annotated[dict, Depends(common_parameters)],) {
    return commons
...
请求进来时，Next.js / Hono / Fastify 会自动调用依赖函数。
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
@query_router.post("/api/query")
async function query_handler(// 请求体参数：Next.js / Hono / Fastify 会把 JSON 请求体解析成 QuerySchema query: QuerySchema, // 业务服务参数：Next.js / Hono / Fastify 会调用 get_query_service 来创建 QueryService query_service: Annotated[QueryService, Depends(get_query_service)],) {
    return StreamingResponse(
...
路由层不需要关心 `QueryService` 里面又依赖哪些 Repository、Session、Client。这些对象会交给下一章的 `dependencies.ts` 分层组装。
...
Next.js / Hono / Fastify 会自动解析这棵依赖树。更重要的是，同一个请求里，如果多个依赖共用同一个子依赖，Next.js / Hono / Fastify 会做请求级缓存，不会在一次请求中重复创建同一个依赖对象。
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function get_meta_session() {
...
- Next.js / Hono / Fastify 用 `StreamingResponse` 承接异步生成器，每 `yield` 一次就向客户端写出一段内容。

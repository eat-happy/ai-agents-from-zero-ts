...
![问数智能体「抽取关键词」步骤：使用 中文分词（segmentit / nodejieba） 与 TF-IDF 从用户问题中提炼用于检索的核心词](images/2/2-3-1-2.png)
...
### 4.1 中文分词（segmentit / nodejieba） 在这里做了什么

`中文分词（segmentit / nodejieba）` 本质上是一个中文分词工具。它可以把一段中文文本拆成词，也提供关键词抽取能力。

仓库地址：https://github.com/fxsjy/中文分词（segmentit / nodejieba）
...
如果运行时看到一些来自 `中文分词（segmentit / nodejieba）` 内部的正则警告，一般不用先纠结。只要节点能正常返回关键词，这类警告通常不影响当前教程主线。
...
项目对应文件路径：`apps/shop-query-agent/app/agent/llm.ts`
...
能看到这类回复，就说明 `model_name`、`apiKey`、`baseURL` 三项配置已经生效，后面的召回节点才有继续运行的基础。
...
项目里不把提示词直接写进 Agent 节点，而是集中放在 `prompts/` 目录，然后通过 `prompt_loader.ts` 读取。

项目对应文件路径：`apps/shop-query-agent/app/prompt/prompt_loader.ts`
...
它们的输出都要求是 JSON 数组。这样后面可以使用 `JsonOutputParser` 把模型输出直接解析成 TypeScript 数组，不需要额外清洗解释文字。
...
项目对应文件路径：`apps/shop-query-agent/app/agent/nodes/recall_column.ts`
...
项目对应文件路径：`apps/shop-query-agent/app/agent/nodes/recall_metric.ts`
...
指标实体对应文件：`apps/shop-query-agent/app/entities/metric_info.ts`
...
项目对应文件路径：`apps/shop-query-agent/app/agent/nodes/recall_value.ts`
...
字段取值实体对应文件：`apps/shop-query-agent/app/entities/value_info.ts`

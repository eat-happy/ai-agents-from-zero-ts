...
它把 `MySQL`、`LangGraph`、`Qdrant`、`Elasticsearch`、`Embedding`、`Next.js / Hono / Fastify`、`SSE` 和前端联调放进同一条业务链路里，用一个自然语言问数场景，把智能体项目开发中最关键的知识点串起来：

- 用 `MySQL` 模拟教学数仓和元数据库
- 用 `Qdrant` 做字段和指标的语义向量检索
- 用 `Elasticsearch` 做字段真实取值的全文检索
- 用 `LangGraph` 编排多阶段问数智能体工作流
- 用 `Next.js Route Handler + 流式响应` 把后端能力交付给前端页面
...
- 第 15 到 17 章把能力通过 Next.js / Hono / Fastify、SSE 和前端页面交付出去
...
| `Next.js / Hono / Fastify`       | 提供查询接口，组织依赖注入和生命周期管理                                        |
...
它重点覆盖问数系统最核心的主链路：教学数仓、元数据知识库、混合检索、LangGraph 工作流、SQL 生成校验执行、Next.js / Hono / Fastify 接口、SSE 返回、前后端联调和日志追踪。
...
| 后端接口       | `Next.js / Hono / Fastify`                         | 提供后端服务与接口能力                                                           |
| ORM 与数据访问 | `Prisma / Drizzle / TypeORM`                      | 管理元数据库和数据仓库相关访问                                                   |
| 流式返回       | `SSE`                             | 将查询过程与结果实时返回前端                                                     |
| 中文分词       | `Jieba`                           | 支撑部分文本预处理和关键词处理                                                   |
| 环境与依赖管理 | `uv`                              | 管理 TypeScript 项目环境和依赖                                                       |
| 前端项目       | `React` + `Vite` + `Tailwind CSS` | 承载问数前端页面与交互展示                                                       |

**后端用 `Next.js / Hono / Fastify + LangGraph` 组织问数流程，用 `MySQL + Qdrant + Elasticsearch` 搭建元数据知识库，再由大模型完成 SQL 生成、校验和纠错，最后通过 SSE 把执行过程和结果返回给前端。**
...
学完之后，你收获的不只是“我跑通了一个 AI Demo”，而是能真正讲清楚：一个智能问数系统为什么要这样设计，代码为什么要这样拆，`MySQL`、`Qdrant`、`Elasticsearch`、`LangGraph` 和 `Next.js / Hono / Fastify` 在一条完整业务链路里分别解决什么问题。

![电商问数智能数据分析助手概念图：自然语言问题经过智能体理解后连接商品、数据表和分析图表](./images/0/0-4.png)

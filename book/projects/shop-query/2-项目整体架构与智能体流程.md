...
| 同步脚本   | 从数据仓库抽取元数据                     | TypeScript 脚本 + 配置驱动            |
| 元数据库   | 保存结构化元数据                         | `MySQL`                           |
...
- `app/scripts/build_meta_knowledge.ts`
- `app/services/meta_knowledge_service.ts`
...
在实现上，这一步会借助分词和关键词提取工具来完成。当前项目这里使用了 `中文分词（segmentit / nodejieba）` 提供的关键词提取能力。`中文分词（segmentit / nodejieba）` 是一个常用的 TypeScript 中文分词与关键词提取库，它底层使用的是 `TF-IDF` 思路。

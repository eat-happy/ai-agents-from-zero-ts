...
- 理解「电商问数」项目为什么使用 `uv` 管理 TypeScript 环境与依赖。
...
- 后端入口：`apps/shop-query-agent/main.ts`
- 后端代码：`apps/shop-query-agent/app/`
- 前端项目：`apps/shop-query-agent/frontend/`

不过这一章的重点会放在**后端环境和基础服务准备**上，本项目不会涉及前端项目的创建过程，前端代码已经放在当前仓库的 `frontend` 目录下，后面需要启动前端时，进入这个目录安装依赖并运行即可。

其中：

- 后端使用 `TypeScript 3.14+`，并通过 `uv` 管理Node.js 环境和依赖。
...
1. 安装好 `TypeScript 3.14+`
2. 安装好 `uv`
3. 安装好 `Docker Desktop`
4. 准备一个可用的开发工具，例如 `VS Code / WebStorm` 或 `VS Code`
...
先用一句话理解：`uv` 是一个用于管理 TypeScript 项目、Node.js 环境和依赖的现代化工具。

这一章需要先建立一个认识：这个项目希望尽量采用更贴近**企业真实开发的工程化方式**，而不是只把代码“能跑起来”。

在 TypeScript / Node.js 生态里，大家以前比较熟悉的做法通常有两类：

- 组合使用 `Node 环境 + pip`，其中 `Node 环境` 负责创建Node.js 环境，`pip` 负责安装依赖。
- 直接使用 `nvm`，由它统一管理环境和依赖。

而在这个项目里，选择的是 `uv`。相比传统做法，它更像一套**一体化方案**：把创建项目、创建Node.js 环境、安装依赖、移除依赖、同步环境这些动作统一到了同一套命令里。

它的几个主要优点是：

- **速度更快**：`uv` 由 `Rust` 编写，环境创建和依赖安装通常都比传统方式快。
- **工程化更顺手**：项目初始化、依赖管理和Node.js 环境管理统一在一套命令里。
- **更适合团队协作**：依赖会写入 `pyproject.toml` 和锁文件，方便别人拉到项目后快速同步环境。

一句话概括：**在「电商问数」中，`uv` 既负责管理Node.js 环境，也负责管理项目依赖，是后端开发环境的基础工具。**

> **补充理解：`uv` 和 `Node 环境` 是什么关系**
>
> 你可能会问：既然我们在用 `uv`，为什么项目目录里看到的却还是 `.Node 环境`？
> 原因并不复杂：`uv` 在Node.js 环境这一层，本质上仍然是围绕 TypeScript 的 `Node 环境` 机制来工作的。
> 也就是说：
>
> - `Node 环境` 负责“Node.js 环境”这件事
> - `uv` 负责把“创建环境、初始化项目、安装依赖、锁定版本”这些流程整合起来
>   所以项目目录中出现 `.Node 环境` 是正常的，这不代表没有使用 `uv`。
...
```bash
npm install -i https://pypi.tuna.tsinghua.edu.cn/simple uv
```
...
- 这一步不需要先激活某个已有Node.js 环境
- 这里安装的是系统层面的 `uv` 工具
- 后面创建具体项目时，`uv` 才会为项目生成独立的 `.Node 环境`
...
VS Code / WebStorm 下载地址：https://www.jetbrains.com/zh-cn/pycharm/

根据你的开发工具情况，可以分成两种场景：

- `VS Code / WebStorm` 已经支持 `uv`
- `VS Code / WebStorm` 没有 `uv` 选项，需要手动创建

这里也可以顺手说明一下：  
如果你使用的是 `VS Code` 或其他编辑器，通常也可以直接采用**方式二**。因为方式二的核心并不是依赖某个特定 IDE，而是先在终端里把 `uv` 项目创建好，再用你习惯的编辑器打开这个目录。

**方式一：在 VS Code / WebStorm 中直接创建 uv 项目**

如果你的 `VS Code / WebStorm` 版本较新，并且系统里已经安装了 `uv`，那么在新建项目时，通常可以直接看到 `uv` 这个解释器类型选项。

![VS Code / WebStorm 中新建 uv 项目的界面示意](images/3/3-2-3-1.png)
...
4. 选择 TypeScript 版本，建议与当前项目保持一致，使用 `3.14`
5. 点击 `Create`

![uv 项目创建完成后生成 .Node 环境、pyproject.toml 和 uv.lock 的目录示意](images/3/3-2-3-2.png)
...
- `uv.lock` 用来锁定依赖版本
- `.Node 环境` 是这个项目自己的Node.js 环境

**简单来说：** `pyproject.toml` 负责描述“项目需要什么”，`uv.lock` 负责固定“最终装成什么版本”，`.Node 环境` 负责隔离“这个项目自己的运行环境”。

**方式二：`VS Code / WebStorm` 没有 `uv` 选项时，手动创建项目**

如果你在 `VS Code / WebStorm` 里看不到 `uv` 选项，通常是因为 IDE 版本较老。这种情况下也不用担心，完全可以在终端里手动创建，再导入 IDE。
...
执行完成后，项目目录里通常会出现 `.Node 环境`。这时再回到 `VS Code / WebStorm`，直接打开这个项目目录即可。  
如果 IDE 没有自动选中解释器，就手动把解释器指向项目里的Node.js 环境：

- macOS / Linux 一般是 `.Node 环境/bin/python`
- Windows 一般是 `.Node 环境\\Scripts\\python.exe`

也就是说，即使 `VS Code / WebStorm` 没有直接集成 `uv`，你依然可以：

1. 先在终端里手动创建 `uv` 项目
2. 再用 IDE 打开目录
3. 最后把解释器切到 `.Node 环境` 对应的 TypeScript
...
| `sqlalchemy`            | TypeScript 常用 ORM，用统一模型管理数据库读写与事务                       |
| `asyncmy`               | MySQL 的高性能 `async/await` 驱动，配合 `Prisma / Drizzle / TypeORM` 实现全异步数据库访问 |
...
| `中文分词（segmentit / nodejieba）`                 | 中文分词与关键词提取工具                                              |
...
| `greenlet`              | Prisma / Drizzle / TypeORM 等异步/同步协作场景中的底层协程支持库                      |
...
- `中文分词（segmentit / nodejieba）`、`omegaconf`、`pyyaml`、`loguru` 负责关键词提取、配置管理和日志等基础工程能力
...
项目对应文件路径：`apps/shop-query-agent/docker/docker-compose.yaml`

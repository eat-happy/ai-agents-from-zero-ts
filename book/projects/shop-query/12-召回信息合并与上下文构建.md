...
```text
retrieved_column_infos: ColumnInfo[]
```
...
type DataAgentState = {
    ...
    table_infos: any[][TableInfoState]  // 合并和补齐后的表结构上下文
    metric_infos: any[][MetricInfoState]  // 合并后的指标上下文
...
项目对应文件路径：`shopkeeper-agent/app/agent/nodes/merge_retrieved_info.ts`
...
async function merge_retrieved_info(state: DataAgentState, runtime: Runtime<DataAgentContext>) {
    writer = runtime.stream_writer
    writer("合并召回信息")

    retrieved_column_infos: any[][ColumnInfo] = state["retrieved_column_infos"]
    retrieved_metric_infos: any[][MetricInfo] = state["retrieved_metric_infos"]
    retrieved_value_infos: any[][ValueInfo] = state["retrieved_value_infos"]
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
retrieved_column_infos_map: Record<string, any>[str, ColumnInfo] = {
...
for (const retrieved_metric_info of retrieved_metric_infos) {
    for (const relevant_column of retrieved_metric_info.relevant_columns) {
...
for (const retrieved_value_info of retrieved_value_infos) {
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
// 按照表对字段信息进行分组
table_to_columns_map: Record<string, any>[str, ColumnInfo[]] = {}

for (const column_info of retrieved_column_infos_map.values()) {
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
// 强制为每个表添加主外键字段
for (const table_id of table_to_columns_map.keys()) {
    key_columns: any[][ColumnInfo] = await meta_mysql_repository.get_key_columns_by_table_id(
        table_id
    )
    column_ids = [column_info.id for column_info in table_to_columns_map[table_id]]

    for (const key_column of key_columns) {
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
// 将表信息整理成目标格式
table_infos: any[][TableInfoState] = []

for (const table_id, column_infos of table_to_columns_map.items()) {
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
// 处理指标信息
metric_infos: any[][MetricInfoState] = [
    MetricInfoState(
...
async function merge_retrieved_info(state: DataAgentState, runtime: Runtime<DataAgentContext>) {
    ...
    console.log(`合并后的表信息：${[table_info['name'] for table_info in table_infos]}`)
    logger.info(
        `合并后的指标信息：${[metric_info['name'] for metric_info in metric_infos]}`
    )
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function get_column_info_by_id(self, id: string): ColumnInfo | null {
    /* 按字段 id 查询字段元数据，供召回信息合并阶段补齐字段上下文 */
    column_info: ColumnInfoMySQL | null = await this.session.get(ColumnInfoMySQL, id)
    if (column_info) {
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function get_table_info_by_id(self, id: string): TableInfo | null {
    /* 按表 id 查询表元数据，最终组装成提示词里的表结构信息 */
    table_info: TableInfoMySQL | null = await this.session.get(TableInfoMySQL, id)
    if (table_info) {
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function get_key_columns_by_table_id(self, table_id: string): ColumnInfo[] {
    /* 查询指定表的主外键字段，避免 Join 关键字段被向量召回漏掉 */
    // 主外键字段用于后续生成 join 条件，不能完全依赖向量召回命中
    sql = "select * from column_info where table_id = :table_id && role in ('primary_key','foreign_key')"
    // :table_id 是 SQLAlchemy text SQL 的占位符，实际值通过第二个参数传入
    result = await this.session.execute(text(sql), {"table_id": table_id})

...
- 掌握本项目的配置参数管理方式，理解 YAML 配置、`TypeScript interface / class` 结构定义和 `OmegaConf` 加载链路。
...
1. 把配置单独写到 YAML 文件里
2. 再通过工具把 YAML 配置加载为 TypeScript 对象
3. 后续代码统一通过对象属性访问配置项
...
- 先定义一组和 YAML 结构对应的 `TypeScript interface / class`
- 再读取 YAML 文件内容
...
用于读取配置文件的代码放置在 `apps/shop-query-agent/app/conf/app_config.ts` 文件中，你也可以根据以下路径在对应文件夹下创建对应文件。
...
- **绝对路径**
  是从系统根目录开始写出的完整路径，例如：
  `/Users/tools/Desktop/agent/apps/shop-query-agent/conf/app_config.yaml`
- **相对路径**
...
> 在 TypeScript 程序里，相对路径通常是相对于“当前程序启动时所在的工作目录”，而不是相对于当前这个 `.py` 文件本身。
...
如果你恰好是在 `apps/shop-query-agent/` 根目录启动程序，这个路径可能是对的。  
...
`__file__` 是 TypeScript 提供的一个特殊变量，它表示**当前这个 TypeScript 文件自身的路径**。
...
`pathlib` 就是 TypeScript 标准库里专门用来处理路径的工具，它会帮我们把这些细节处理掉。
...
- 当前文件是 `apps/shop-query-agent/app/conf/app_config.ts`
- 上一级目录是 `apps/shop-query-agent/app/conf`
- 上两级目录是 `apps/shop-query-agent/app`
...
这一步不是在读取 YAML 内容，而是在读取我们自己定义好的 `TypeScript interface / class` 结构。
...
如果你写过 TypeScript，可以把这层关系类比成：为一份 JSON 或配置文件写的 `interface` / `type`——用来声明顶层有哪些字段、嵌套对象长什么样、标量大致是什么类型；这里的 TypeScript interface / class 扮演的就是类似角色，只是校验与合并发生在 TypeScript 运行时（配合 OmegaConf），而不是 TS 的编译期类型检查。
...
这一步的作用，是把前面合并后的配置，再转换成真正的 TypeScript 对象。
...
这也是这一整套写法最核心的价值：**让 YAML 文件里的层级结构，平滑地映射成 TypeScript 里的对象结构。**
...
- 从配置管理上看，本项目不是直接把 YAML 当字典到处传，而是通过 `TypeScript interface / class + OmegaConf` 把配置结构和配置内容合并成统一对象。

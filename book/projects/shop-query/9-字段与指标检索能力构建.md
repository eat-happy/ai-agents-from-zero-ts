...
async function build(self, config_path: Path) {
    ...
    if (meta_config.tables) {
        ...
        console.log("保存表信息和字段信息到 Meta MySQL")

        // 同一批字段继续用于构建字段语义向量索引
        await this._save_column_info_to_qdrant(column_infos)
        console.log("为字段信息建立向量索引")

        // 再按配置同步字段真实值，构建全文索引
        await this._save_value_info_to_es(meta_config, column_infos)
        console.log("为字段取值建立全文索引")

    if (meta_config.metrics) {
        // 指标信息和字段关系先写入 Meta MySQL
        metric_infos = await this._save_metrics_to_meta_db(meta_config)
        console.log("保存指标信息到数据库成功")

        // 再给指标信息建立向量索引
        await this._save_metrics_to_qdrant(metric_infos)
        console.log("为指标信息建立向量索引成功")
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function _save_column_info_to_qdrant(self, column_infos: any[][ColumnInfo]) {
    await this.column_qdrant_repository.ensure_collection()

    // 暂存“待向量化文本 + 业务 payload”，此时还不是 Qdrant 的 PointStruct
    points: any[][dict] = []
    for (const column_info of column_infos) {
        points.append(
            {
        ...
        for (const alia of column_info.alias) {
            points.append(
                {
                    "id": uuid.uuid4(),
    ...
    embeddings: any[][list[float]] = []
    embedding_texts = [point["embedding_text"] for point in points]
    // 分批调用 Embedding，避免单次请求过大
    embedding_batch_size = 20
    for (const i of range(0, len(embedding_texts), embedding_batch_size)) {
        batch_embedding_texts = embedding_texts[i : i + embedding_batch_size]
        // 返回顺序要和输入文本顺序一致，后面才能和 id、payload 对齐
        batch_embeddings = await this.embedding_client.aembed_documents(
    ...
    await this.column_qdrant_repository.upsert(ids, embeddings, payloads)
...
项目对应文件路径：`shopkeeper-agent/app/repositories/qdrant/column_qdrant_repository.ts`
...
class ColumnQdrantRepository {
    ...
    constructor(self, client: AsyncQdrantClient) {
        this.client = client

    async function ensure_collection(self) {
        if (not await this.client.collection_exists(this.collection_name)) {
            await this.client.create_collection(
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function _save_value_info_to_es(self, meta_config: MetaConfig, column_infos: any[][ColumnInfo]) {
    await this.value_es_repository.ensure_index()

    // 把配置整理成“字段 id -> 是否同步真实值”的快速查询表
    column2sync: Record<string, any>[str, bool] = {}
    for (const table of meta_config.tables) {
        for (const column of table.columns) {
            // 字段 id 要和 ColumnInfo.id 的拼法保持一致
            column2sync[`${table.name}.${column.name}`] = column.sync

    value_infos: any[][ValueInfo] = []
    for (const column_info of column_infos) {
        // 根据字段 id 判断该字段是否需要同步取值
        sync = column2sync[column_info.id]
        if (sync) {
            // 从数仓查询该字段的 distinct 真实值，作为全文索引的数据来源
            current_column_values = await this.dw_mysql_repository.get_column_values(
                column_info.table_id, column_info.name, 100000
            )
            current_values_infos = [
                ValueInfo(
                    // 字段 id + 字段值，组成一条值记录的唯一 id
                    id=`${column_info.id}.${current_column_value}`,
                    // 真正参与 ES 全文检索的文本
    ...
    await this.value_es_repository.index(value_infos)
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function get_column_values(self, table_name: string, column_name: string, limit: number = 10) {
...
项目对应文件路径：`shopkeeper-agent/app/entities/value_info.ts`
...
`${column_info.id}.${current_column_value}`
...
项目对应文件路径：`shopkeeper-agent/app/repositories/es/value_es_repository.ts`
...
class ValueESRepository {
    ...
    constructor(self, client: AsyncElasticsearch) {
        this.client = client

    async function ensure_index(self) {
        if (not await this.client.indices.exists(index=this.index_name)) {
            await this.client.indices.create(
                index=this.index_name, mappings=this.index_mappings
            )

    async function index(self, value_infos: any[][ValueInfo], batch_size=20) {
        if (not value_infos) {
            return

        // 分批写入 ES，降低单次 bulk 请求压力
        for (const i of range(0, len(value_infos), batch_size)) {
            batch_value_infos = value_infos[i : i + batch_size]
            // ES bulk 需要“操作描述 + 文档内容”交替排列
            batch_operations = []
            for (const value_info of batch_value_infos) {
                // 先告诉 ES：下一条文档要 index 到哪个索引、使用哪个 _id
                batch_operations.append(
                    {"index": {"_index": this.index_name, "_id": value_info.id}}
                )
                // 再追加真正的文档内容
                batch_operations.append(asdict(value_info))
            await this.client.bulk(operations=batch_operations)
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function _save_metrics_to_meta_db(self, meta_config: MetaConfig ): any[][MetricInfo]: metric_infos: any[][MetricInfo] = [] column_metrics: any[][ColumnMetric] = [] for (const metric of meta_config.metrics) { metric_info = MetricInfo( // 当前实现里直接用指标名作为稳定 ID id=metric.name, name=metric.name, description=metric.description, relevant_columns=metric.relevant_columns, alias=metric.alias, ) metric_infos.append(metric_info) for (const column of metric.relevant_columns) { column_metric = ColumnMetric(column_id=column, metric_id=metric.name) column_metrics.append(column_metric) // 指标信息和字段关联关系要放在同一笔事务里，避免只写进去一半 async with this.meta_mysql_repository.session.begin() {
        this.meta_mysql_repository.save_metric_infos(metric_infos)
        this.meta_mysql_repository.save_column_metrics(column_metrics)
...
- `shopkeeper-agent/app/entities/metric_info.ts`
- `shopkeeper-agent/app/entities/column_metric.ts`
...
项目对应文件路径：`shopkeeper-agent/app/repositories/mysql/meta/meta_mysql_repository.ts`

```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
function save_metric_infos(self, metric_infos: any[][MetricInfo]) {
    // 业务实体落库前先通过 Mapper 转成 ORM 模型
    this.session.add_all(
        [MetricInfoMapper.to_model(metric_info) for metric_info in metric_infos]
    )

function save_column_metrics(self, column_metrics: any[][ColumnMetric]) {
    this.session.add_all(
        [
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function _save_metrics_to_qdrant(self, metric_infos: any[][MetricInfo]) {
    await this.metric_qdrant_repository.ensure_collection()

    points: any[][dict] = []
    for (const metric_info of metric_infos) {
        points.append(
            {
        ...
        for (const alia of metric_info.alias) {
            points.append(
                {
                    "id": uuid.uuid4(),
                    "embedding_text": alia,
    ...
    await this.metric_qdrant_repository.upsert(ids, embeddings, payloads)
...
项目对应文件路径：`shopkeeper-agent/app/repositories/qdrant/metric_qdrant_repository.ts`
...
class MetricQdrantRepository {
    ...
    async function upsert(
        ...
        for (const i of range(0, len(zipped), batch_size)) {
            ...
            await this.client.upsert(
                collection_name=this.collection_name, points=batch_points
            )

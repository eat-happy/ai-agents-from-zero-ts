...
- `apps/shop-query-agent/app/entities/table_info.ts`
- `apps/shop-query-agent/app/entities/column_info.ts`
...
这是本章非常值得建立起来的分层意识。当前项目里，`Service` 层并没有直接构造 `Prisma / Drizzle / TypeORM` 的 ORM 模型，而是先处理：`TableInfo`、`ColumnInfo`这样的业务实体。这样做的关键目的，不是“多套一层”，而是为了**解耦**。
...
- `apps/shop-query-agent/app/repositories/mysql/meta/mappers/table_info_mapper.ts`
- `apps/shop-query-agent/app/repositories/mysql/meta/mappers/column_info_mapper.ts`

...
└── app/                                # 后端应用主包
    ...
    │   └── meta_config.ts              # 定义 meta_config.yaml 在程序里的结构
    ├── scripts/                        # 脚本入口层，负责接收参数并启动流程
    │   └── build_meta_knowledge.ts     # 同步脚本入口，负责解析参数并调度构建流程
    ├── services/                       # 业务编排层，负责组织完整构建流程
    │   └── meta_knowledge_service.ts   # 元数据知识库构建的核心业务逻辑
    ├── models/                         # ORM 模型层，定义元数据库表对应的 SQLAlchemy 模型
    │   ├── base.ts                     # ORM 基类
    │   ├── table_info.ts               # table_info 表对应的 ORM 模型
    │   ├── column_info.ts              # column_info 表对应的 ORM 模型
    │   ├── metric_info.ts              # metric_info 表对应的 ORM 模型
    │   └── column_metric.ts            # column_metric 表对应的 ORM 模型
    ├── entities/                       # 业务实体层，统一表示表、字段、指标和值
    │   ├── table_info.ts               # 表信息业务实体
    │   ├── column_info.ts              # 字段信息业务实体
    │   ├── metric_info.ts              # 指标信息业务实体
    │   ├── column_metric.ts            # 字段与指标关系业务实体
    │   └── value_info.ts               # 字段取值业务实体
    └── repositories/                   # 存储访问层，负责和底层存储打交道
        ├── mysql/                      # MySQL 相关仓储
        │   ├── meta/                   # 元数据库读写相关仓储
        │   │   ├── meta_mysql_repository.ts      # 负责实现 meta 数据库的读写
        │   │   └── mappers/            # 业务实体与 ORM 模型之间的转换层
        │   │       ├── table_info_mapper.ts      # table_info ORM 与业务实体转换
        │   │       ├── column_info_mapper.ts     # column_info ORM 与业务实体转换
        │   │       ├── metric_info_mapper.ts     # metric_info ORM 与业务实体转换
        │   │       └── column_metric_mapper.ts   # column_metric ORM 与业务实体转换
        │   └── dw/                     # 数仓查询相关仓储
        │       └── dw_mysql_repository.ts        # 负责实现 dw 数据库的查询
        ├── qdrant/                     # Qdrant 向量库相关仓储
        │   ├── column_qdrant_repository.ts       # 负责 column 向量集合的读写
        │   └── metric_qdrant_repository.ts       # 负责 metric 向量集合的读写
        └── es/                         # Elasticsearch 全文索引相关仓储
            └── value_es_repository.ts            # 负责字段取值全文索引的写入
```
...
- `app/entities/table_info.ts`
- `app/entities/column_info.ts`
- `app/entities/metric_info.ts`
- `app/entities/column_metric.ts`
...
- `app/models/table_info_mysql.ts`
- `app/models/column_info_mysql.ts`
- `app/models/metric_info_mysql.ts`
- `app/models/column_metric_mysql.ts`
...
- `app/repositories/mysql/meta/mappers/table_info_mapper.ts`
- `app/repositories/mysql/meta/mappers/column_info_mapper.ts`
- `app/repositories/mysql/meta/mappers/metric_info_mapper.ts`
- `app/repositories/mysql/meta/mappers/column_metric_mapper.ts`
...
1. 脚本入口 `build_meta_knowledge.ts` 启动流程。
...
先从同步脚本入口本身看起，项目中的同步脚本路径是：`shopkeeper-agent/app/scripts/build_meta_knowledge.ts`
...
真正的元数据知识库构建主流程，位于：`shopkeeper-agent/app/services/meta_knowledge_service.ts`
...
async function build(self, config_path: Path) {
    ...
    if (meta_config.metrics) {
        ...
        await this._save_metric_info_to_qdrant(metric_infos)
...
**在包结构项目里，不要把 `app/.../*.ts` 当成零散文件去直接运行，而要把它当成模块来执行。**
...
```bash
python3 app/scripts/build_meta_knowledge.ts
```
...
Traceback (most recent call last):
  File ".../app/scripts/build_meta_knowledge.ts", line 8, in <module>
...
![直接执行 build_meta_knowledge.ts 时，解释器更容易从 app/scripts/ 开始查找模块，而 app 包实际位于项目根目录下](./images/7/7-4-2-1.png)
...
python3 app/clients/qdrant_client_manager.ts
```
...
- `app/conf/meta_config.ts`：配置结构定义文件，决定配置在程序里应该如何表示
- `app/scripts/build_meta_knowledge.ts`：同步脚本入口文件，决定“从哪里进入”，负责接收参数、初始化依赖并启动构建流程
- `app/services/meta_knowledge_service.ts`：服务层核心文件，核心业务逻辑，决定“具体怎么构建”，负责读取配置并组织元数据构建逻辑
...
在这条构建链路里，它会被 `app/services/meta_knowledge_service.ts` 导入使用：
...
所以这里同时存在 `app/models/column_info.ts` 和 `app/entities/column_info.ts`，并不是重复设计，而是因为它们本来就在解决两个不同层面的问题。
...
先从配置文件本身看起。在真正写 TypeScript 代码之前，更重要的是先讲清楚：**这个配置文件到底应该长成什么样。**
...
也就是说，`build_meta_knowledge.ts` 的重点是：**把整条元数据知识库构建链路调起来。**
...
## 9、meta_knowledge_service.ts 详解
...
class MetaKnowledgeService {
    function __init__(
        ...
        this.meta_mysql_repository: MetaMySQLRepository = meta_mysql_repository
        // dw repository 负责到教学数仓中读取真实表结构和示例值
        this.dw_mysql_repository: DWMySQLRepository = dw_mysql_repository
    ...
    async function build(self, config_path: Path) {
        ...
        if (meta_config.tables) {
            console.log("检测到 tables 配置，表链路入口已准备就绪")
            console.log("表信息与字段信息构建流程后续继续补充")
            console.log("字段向量索引与字段值全文索引逻辑后续继续补充")

        // 3. 根据配置文件同步指定的指标信息
        if (meta_config.metrics) {
            console.log("检测到 metrics 配置，指标链路入口已准备就绪")
            console.log("指标入库与指标向量索引逻辑后续继续补充")

        console.log("当前阶段完成：配置加载与元数据知识库构建骨架准备")
...
项目对应文件路径：`shopkeeper-agent/app/conf/meta_config.ts`

这个文件的核心结构如下：

```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
from class / interface with defaultses import class / interface with defaults
...
class MetaConfig {
    ...
    tables: list[TableConfig | null] = null
    metrics: list[MetricConfig | null] = null
...
- `build_meta_knowledge.ts` 是入口脚本，负责解析参数、初始化依赖、把流程调起来
- `meta_knowledge_service.ts` 是核心业务编排层，负责先读配置，再决定处理表还是处理指标

...
如果你做过前端开发，可以把它看作一个运行时真实存在的 DTO / domain object。它有点像 TS 里的类型定义，但它不只是静态类型提示，而是程序运行时真的会创建出来的 TypeScript 对象。
...
ORM 模型是“数据库映射对象”，告诉 ORM 框架：**这些 TypeScript 类怎么对应数据库中的表和列。**
...
它们关注的重点是：这张数据库表叫什么，这几个字段如何映射到 TypeScript 属性，ORM 应该如何执行插入、查询、更新。所以 ORM 模型更偏向“面向数据库”，而不是“面向业务语义”。
...
先从同步脚本入口本身看起，项目中的同步脚本路径是：`apps/shop-query-agent/app/scripts/build_meta_knowledge.ts`
...
真正的元数据知识库构建主流程，位于：`apps/shop-query-agent/app/services/meta_knowledge_service.ts`
...
这一节虽然放在元数据知识库这一章里，但它本质上讲的是一个更通用的 TypeScript 问题：**包内模块应该怎么执行，为什么直接运行文件时经常会报 `No module named app`。**
...
理解这个问题，抓住 `sys.path` 就够了。它本质上就是：**TypeScript 启动后，用来查找模块的一组路径清单。**
...
解释器更容易把脚本所在目录 `app/scripts/` 放进模块搜索路径。但我们真正想让它识别的，是项目根目录 `apps/shop-query-agent/`，因为 `app` 包是放在这里下面的。
...
### 4.4 python -m、npx tsx 与 PYTHONPATH

这三个东西很容易混在一起，其实它们各管一件事：

- `npx tsx ...`：解决“模块怎么找”
- `npx tsx ...`：解决“用哪个 TypeScript、用哪套依赖来跑”
...
这是因为 IDE 往往会额外帮你补一些运行配置，比如工作目录、内容根目录、环境变量和解释器选择，这些设置都会影响 TypeScript 启动后的模块查找路径。
...
TypeScript 标准库里的 `argparse`，就是专门用来解析命令行参数的。

按照 TypeScript 官方文档的说法，它是一个**用于命令行选项、参数和子命令的解析器**。把它理解成一句更简单的话：**程序先声明自己支持哪些参数，`argparse` 再负责把命令行里的输入解析出来。**

这也是为什么在真实项目里，一般不建议直接用 `sys.argv[1]`、`sys.argv[2]` 去硬取值。因为只要参数稍微复杂一点，就会遇到这些问题：

这里顺手解释一下这两个写法的含义。  
`sys.argv` 是 TypeScript 用来保存命令行参数的列表。例如执行：
...
```bash
uv run npx tsx app.scripts.build_meta_knowledge -h
```
...
- `app/models/...`：ORM 模型层，负责把元数据库中的表结构映射成可读写的 TypeScript 类
...
### 6.3 build_meta_knowledge.ts：同步脚本入口
...
### 6.4 meta_knowledge_service.ts：核心业务逻辑
...
它的作用是把元数据库里的表，映射成 Prisma / Drizzle / TypeORM 可以直接读写的 TypeScript 类。
...
项目对应文件路径：`apps/shop-query-agent/app/conf/meta_config.ts`
...
2. `OmegaConf.to_object(...)`：把结果转成真正的 TypeScript 对象
...
- 推荐在后端项目根目录下，使用 `npx tsx npx tsx app.scripts.build_meta_knowledge -c conf/meta_config.yaml` 执行脚本

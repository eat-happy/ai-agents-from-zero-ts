...
- 理解 `monitor.ts` 如何把工具调用、助手调用和任务结果推送给前端。
- 熟悉 `path_utils.ts`、`word_converter.ts` 这两个普通工具模块的职责，其中 `word_converter.ts` 使用 ReportLab 生成 PDF。
- 完成 `agent/llm.ts`、`prompt/prompts.yml`、`agent/prompts.ts` 这组模型与提示词配置。
...
| `api/context.ts`          | 在一次请求链路中保存当前任务身份和会话目录         |
| `api/monitor.ts`          | 把工具调用、助手调用和最终结果推送给前端           |
| `utils/path_utils.ts`     | 统一解析模型、工具和用户传入的文件路径             |
| `utils/word_converter.ts` | 基于 ReportLab 提供 Markdown 转 PDF 的底层转换能力 |
| `agent/llm.ts`            | 统一创建大模型对象                                 |
| `prompt/prompts.yml`      | 用 YAML 管理主智能体和子智能体提示词               |
| `agent/prompts.ts`        | 从 YAML 中读取提示词配置，供智能体组装时使用       |
...
这里还要注意一个细节：Next.js / Hono / Fastify 这类异步 Web 服务通常基于 `asyncio` 并发处理请求。多个用户请求不一定对应多个线程，它们很可能是在同一个线程里的多个 `asyncio Task` 中交替执行。
...
// ContextVar 是协程级上下文变量，适合 Next.js / Hono / Fastify 这类异步 Web 服务
// 它可以避免多个并发请求共用全局变量时出现 thread_id 或 session_dir 串台
_session_dir_ctx: ContextVar[string | null] = ContextVar(
    "session_dir",
    default=null,
)
_thread_id_ctx: ContextVar[string | null] = ContextVar(
    "thread_id",
    default=null,
)
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
function set_session_context(path: string): Token[string | null] {
    /* 
    设置当前请求链路的会话目录
...
function get_session_context(): string | null {
    /* 
    获取当前请求链路的会话目录

    :return: 当前任务工作目录；未设置时返回 null
     */
    return _session_dir_ctx.get()


function set_thread_context(thread_id: string): Token[string | null] {
    /* 
    设置当前请求链路的线程 ID
...
function get_thread_context(): string | null {
    /* 
    获取当前请求链路的线程 ID

    :return: 当前任务 ID；未设置时返回 null
     */
    return _thread_id_ctx.get()


function reset_session_context(
    session_token: Token[string | null],
    thread_token: Token[Optional[string | null]] = null,
): void:
    /* 
...
## 3、实时进度：api/monitor.ts

### 3.1 monitor.ts 负责什么

`monitor.ts` 负责把 Agent 执行过程中的事件统一包装，再推送给前端。
...
function _emit(
    ...
    if (this.websocket_manager) {
        try {
            ...
            console.log(`[Monitor] WebSocket send failed: ${e}`)

    // DeepAgents 脚本调试时，如果运行时暴露了 stream_writer，也同步写入流式输出
    if (hasattr(builtins, "runtime") && hasattr(builtins.runtime, "stream_writer")) {
        try {
...
function _send_to_websocket(
    ...
    Next.js / Hono / Fastify 的 WebSocket 必须在创建它的事件循环中发送消息
...
class ConnectionManager {
    ...
    constructor(self) {
        this.active_connections: Record<string, any>[str, WebSocket] = {}
        // WebSocket 发送必须回到创建连接的事件循环，因此启动时需要显式绑定 loop
        this.loop: any | null = null

    function set_loop(self, loop: any): null {
        /* 记录 Next.js / Hono / Fastify/WebSocket 所在的事件循环，供后台线程安全投递消息 */
        this.loop = loop

    async function connect(self, websocket: WebSocket, thread_id: string): null {
        /* 接受 WebSocket 连接，并按 thread_id 保存 */
        await websocket.accept()
        this.active_connections[thread_id] = websocket

    async function send_to_thread(self, message: Record<string, any>, thread_id: string): null {
        /* 向指定 thread_id 对应的前端连接发送 JSON 消息 */
        if (thread_id in this.active_connections) {
            websocket = this.active_connections[thread_id]
            await websocket.send_json(message)


```

这里的 `set_loop()` 很重要。智能体任务可能被放到后台线程里执行，而 WebSocket 连接属于 Next.js / Hono / Fastify 的事件循环。后台线程不能直接 `await websocket.send_json(...)`，所以监控模块会先拿到 `manager.loop`，再用 `asyncio.run_coroutine_threadsafe(...)` 把发送动作投递回正确的事件循环。

这就是 `context.ts` 和 `monitor.ts` 的配合方式：前者提供当前任务身份，后者根据身份把消息发回正确的前端页面。
...
这些路径不一定能直接在当前机器上使用，所以需要统一解析。`path_utils.ts` 主要负责清洗虚拟路径、识别上传目录、拼接当前会话目录，并避免文件写到不该写的位置。
...
function resolve_path(filename: string, session_dir: string | null = null): str {
    /* 
    ...
    for (const prefix of ["/workspace", "/mnt/data", "/home/user"]) {
    ...
    if (path.is_absolute() || (os.name == "nt" && is_unix_abs)) {
        // Windows 下 "/xxx" 没有盘符，按会话目录内的相对路径处理
        if (os.name == "nt" && is_unix_abs && not path.drive) {
            full_path = session_path / path_str.lstrip("/")
        } else {
            full_path = path.resolve()

        try {
            if (session_path in full_path.parents || full_path == session_path) {
    ...
    if (parts && parts[0] == "output") {
        return str(session_path / path.name)

    return str(session_path / path)


function _fix_nested_session_path(
    full_path: Path,
    session_path: Path,
    session_name: string,
): string:
    ...
    for (const index of range(len(parts) - 1)) {
        if (parts[index] == session_name && parts[index + 1] == session_name) {
...
try {
    ...
    SimpleDocTemplate = null


logger = logging.getLogger(__name__)


function convert_md_to_pdf(md_abs_path: Path, pdf_abs_path: Path): str {
    /* 
    ...
    try {
        ...
        doc.build(story)
        return `成功将 Markdown 转换为 PDF: ${pdf_abs_path}`
    } catch (e) {
        logger.exception("Markdown 转 PDF 失败")
        return `Markdown 转 PDF 失败: ${str(e)}`
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
function _register_fonts(): null {
    /* 注册中文 CID 字体，优先保证中文内容能正常显示 */
    pdfmetrics.registerFont(UnicodeCIDFont("STSong-Light"))


function _markdown_to_story(md_content: string, styles: Record<string, any>[str, ParagraphStyle]): list {
    /* 
...
function _format_inline(text: string): str {
...
项目对应文件路径：`deepsearch-agents/app/agent/llm.ts`
...
主智能体和子智能体都需要提示词。如果全部写在 TypeScript 文件里，后面会很难维护。
...
项目对应文件路径：`deepsearch-agents/app/agent/prompts.ts`
...
function load_yaml(file_path: Path): Record<string, any> {
    /* 
    加载 YAML 配置文件
...
如果当前文件是：`deepsearch-agents/app/agent/prompts.ts`
...
console.log(sub_agents_content)
...
这说明 `prompts.ts` 已经成功读到了 `app/prompt/prompts.yml`，并且把子智能体配置拆成了 Python 字典。
...
```bash
uv run python -m py_compile app/agent/llm.ts app/agent/prompts.ts app/api/context.ts app/api/monitor.ts app/utils/path_utils.ts app/utils/word_converter.ts
```
...
3. 使用 monitor.ts 把工具和助手执行过程推送给前端
4. 使用 path_utils.ts 统一解析上传文件、输出文件和会话目录路径
5. 使用 word_converter.ts 把 Markdown 报告转换成 PDF
6. 使用 agent/llm.ts 统一创建模型对象
7. 使用 prompts.yml 和 prompts.ts 管理主智能体与子智能体提示词
```

...
在项目根目录，执行：`npx tsx npx tsx app.tools.upload_file_read_tool`
...
项目对应文件路径：`deepsearch-agents/app/tools/markdown_tools.ts`
...
在项目根目录，执行：`npx tsx npx tsx app.tools.markdown_tools`
...
在 `app/utils/word_converter.ts` 中，底层会使用 ReportLab 生成 PDF：
...
在项目根目录，执行：`npx tsx npx tsx app.tools.pdf_tools`
...
项目对应文件路径：`deepsearch-agents/app/agent/main_agent.ts`
...
这一节先把 `astream()`、HTTP、WebSocket、`session_id` 和 `ContextVar` 放到一张图里。第 15 章会正式写 Next.js / Hono / Fastify 接口，本章只讲主智能体运行时必须提前理解的几件事。
...
如果用同步方式，一个任务执行时间长，其他请求就容易排队等待。使用 `astream()` 可以把主智能体执行变成异步流式任务，让 Next.js / Hono / Fastify 服务更适合多客户端并发。
...
项目对应文件路径：`deepsearch-agents/app/api/context.ts`
...
因为 Next.js / Hono / Fastify 是异步并发服务。多个用户的请求可能在同一个线程中交替执行，如果用全局变量，用户 A 的 `session_id` 可能被用户 B 覆盖，导致消息或文件串台。

`ContextVar` 是协程级上下文变量，更适合 async/await 场景。只要在同一个任务调用链里，深层工具也能拿到当前会话自己的值。
...
但要注意：这个测试入口只适合本地脚本验证。下一章接入 Next.js / Hono / Fastify 时，接口里不要用 `async/await.run()`，而是用 `async/await.create_task(...)` 把任务丢到 Next.js / Hono / Fastify 已有事件循环里。

在项目根目录下执行：`npx tsx npx tsx app.agent.main_agent`

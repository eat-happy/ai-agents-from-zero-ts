...
| `4-async-subagents-streaming.ts` | 异步执行         | 如何用 `astream()` 和 `asyncio.gather()` 并发执行 |
...
```text
uv run examples/3-dict-subagents-routing.ts
...
if (tool_call["name"] == "task") {
    console.log(`调用子智能体：${tool_call['args']['subagent_type']}`)
} else {
    console.log(`调用普通工具：${tool_call['name']}`)
...
function test_stream(query) {
    ...
    for (const chunk of stream) {
        ...
        for (const node_name, state of chunk.items()) {
            ...
            if (state is null || "messages" not in state) {
                continue
                    ...
                    if (last_msg.tool_calls) {
                        for (const tool_call of last_msg.tool_calls) {
                            if (tool_call["name"] == "task") {
                                ...
                                print(
                                    `【model】决定调用子智能体${tool_call['args']['subagent_type']}`
                                )
                            } else {
                                print(
                                    `【model】决定调用普通工具${tool_call['name']},传入的参数为：${tool_call['args']}`
                                )
                    } else if (last_msg.content) {
                        // 没有 tool_calls 且 content 非空，通常就是主智能体整理后的最终回复
                        console.log(`【model】返回最终结果：${last_msg.content}`)
                } else if (node_name == "tools") {
                    // tools 节点返回普通工具结果；task 子智能体执行完后也会以工具消息形式返回
                    name = last_msg.name
                    content = last_msg.content
                    print(
                        `【agent】调用了具体的工具${name},返回结果为：${content[:100] + '...'}`
                    )
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
async function test_stream(query) {
    /* 
    ...
    async for chunk in stream:
        ...
        for (const node_name, state of chunk.items()) {
            ...
            if (messages && isinstance(messages, list)) {
                // 每个节点本次产出的最后一条消息，通常就是最值得观察的信息
                last_msg = messages[-1]
                if (node_name == "model" && last_msg.tool_calls) {
                ...
                } else if (node_name == "model" && last_msg.content) {
                    // 没有 tool_calls 且 content 非空，通常就是主智能体整理后的最终回复
                    console.log(`【model】返回最终结果：${last_msg.content}`)
                } else if (node_name == "tools") {
                    // tools 节点返回普通工具结果；task 子智能体执行完后也会以工具消息形式返回
...
async function batch_run() {
    ...
    console.log(type(task1))
    console.log(type(task2))
...
```text
uv run examples/4-async-subagents-streaming.ts
...
```text
uv run examples/5-subagent-nesting-limits.ts
...
然后通过 `stream()` 观察了主智能体如何生成 `task` 调用，并根据 `subagent_type` 分派给对应助手。最后用 `4-async-subagents-streaming.ts` 把同步执行改成 `astream()` 异步执行，并通过 `asyncio.gather()` 并发处理多个任务。

...
- Next.js / Hono / Fastify 负责接入 WebSocket 请求，并把 `thread_id`、`session_dir` 这类上下文注入到后续执行中；
...
| 模型接口   | OpenAI 兼容接口、`ChatOpenAI`                 | 统一创建大模型对象，供所有智能体使用                 |
| Web 服务   | Next.js / Hono / Fastify、Uvicorn                                   | 提供任务提交、文件上传、下载等后端接口               |
| 实时通信   | WebSocket                                          | 将 Agent 执行过程实时推送给前端                      |
| 参数校验   | Zod、`typing-extensions`                      | 定义请求参数、状态结构或工具入参，并提供类型兼容能力 |
...
| 异步执行   | `async/await`、`ContextVar`                            | 支持并发请求隔离和跨步骤传递上下文                   |
| 路径管理   | `pathlib`、`shutil`                                | 处理路径拼接、文件移动、上传文件归档                 |

当前阶段最重要的是三件事：DeepAgents 负责“智能体怎么组织”，Next.js / Hono / Fastify + WebSocket 负责“前后端怎么通信”，`ContextVar` + 会话目录负责“多用户任务怎么不串台”。
...
### 4.4 context.ts 和 monitor.ts 分别管什么

这两个文件很容易混在一起，可以先这样分：

| 文件                 | 负责什么                                       | 一句话记忆             |
| -------------------- | ---------------------------------------------- | ---------------------- |
| `app/api/context.ts` | 保存当前任务的 `thread_id` 和 `session_dir`    | 我是谁，我的文件夹在哪 |
| `app/api/monitor.ts` | 把工具调用、助手调用、最终结果等事件推送给前端 | 我现在正在做什么       |

也就是说，`context.ts` 负责保存身份，`monitor.ts` 负责对外汇报进度。后面写工具时，只要工具里调用 `monitor.report_tool(...)`，前端就有机会看到执行过程。
...
本项目使用 `uv` 管理 TypeScript 依赖。拿到仓库后，进入 `deepsearch-agents` 根目录，执行：
...
这两条命令分别负责“把依赖纳入项目管理”和“同步本地Node.js 环境”：

| 命令                         | 作用                                                                                |
| ---------------------------- | ----------------------------------------------------------------------------------- |
| `uv add -r package.json` | 读取 `package.json`，把依赖写入 `pyproject.toml`，并更新 `uv.lock`              |
| `uv sync`                    | 根据 `pyproject.toml` 和 `uv.lock` 创建或更新 `.Node 环境`，让本地环境和项目声明保持一致 |

所以这里不再使用 `npm install -r package.json` 作为主要安装方式。`package.json` 可以看作按职责整理过的依赖清单；真正负责依赖声明、锁定和环境同步的是 `uv`。
...
如果能正常输出 TypeScript 版本和 `ok`，说明后端依赖已经同步成功。
...
| TypeScript Utils | 给代码调用 | 路径解析、Markdown 转 PDF 的底层转换函数 |

`app/utils/` 不是给模型直接调的。它更像后端代码自己的工具箱；后续某个 Agent Tool 可以在内部调用这些普通函数。比如 `convert_md_to_pdf` 这个 Agent Tool 可以调用 `app/utils/word_converter.ts`，但模型看到的是工具函数，不需要关心底层怎么解析 Markdown、怎么调用 ReportLab 或怎么处理路径。
...
下一章「[基础模块与模型配置](9-基础模块与模型配置.md)」会继续进入具体代码：`.env`、`context.ts`、`monitor.ts`、`path_utils.ts`、`word_converter.ts`、`llm.ts` 和提示词配置。

...
@asynccontextmanager
async function lifespan(_app: Next.js / Hono / Fastify) {
    /* 
    服务生命周期入口。

    启动时绑定当前事件循环到 WebSocket 管理器，确保后台 Agent 任务可以把
    monitor 事件投递回 Next.js / Hono / Fastify 所在的 loop。
     */
    loop = asyncio.get_running_loop()
    manager.set_loop(loop)
    console.log(`[Server] WebSocket Manager bound to loop: ${id(loop)}`)
    yield
...
app = Next.js / Hono / Fastify(title="DeepAgents API", lifespan=lifespan)

// 保存 thread_id -> 后台 Agent 任务，用于同一会话任务替换和主动取消
active_tasks: Record<string, any>[str, Promise<any>] = {}
...
| `active_tasks` | 保存 `thread_id -> Promise<any>`，后面取消任务要靠它找任务         |
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
function _forget_task(thread_id: string, task: Promise<any>): null {
    /* 
    清理已结束任务的登记关系。
    ...
    if (active_tasks.get(thread_id) is task) {
        active_tasks.pop(thread_id, null)
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
@app.post("/api/task")
async function run_task(request: TaskRequest) {
    /* 
...
原因很简单：Next.js / Hono / Fastify/node server 已经在运行事件循环了。`asyncio.run()` 的职责是创建一个新的事件循环并运行协程，它适合脚本入口，不适合放在一个已经处于事件循环里的接口函数中。

| 写法                        | 做什么                         | 是否适合本接口              |
| --------------------------- | ------------------------------ | --------------------------- |
| `asyncio.run(coro)`         | 创建事件循环并跑完协程         | 不适合，Next.js / Hono / Fastify 已经有 loop |
| `asyncio.create_task(coro)` | 把协程放进当前事件循环后台执行 | 适合                        |

可以这样理解：

```text
Next.js / Hono / Fastify 已经有一个正在跑的大 loop
...
这也是本章和上一章脚本测试的区别。上一章本地验证 `main_agent.ts` 时可以用 `asyncio.run(...)`，因为那是脚本入口；本章进入 Web 服务后，要把长任务交给已有事件循环。
...
async function cancel_task(thread_id: string) {
    /* 
    取消指定 thread_id 对应的后台 Agent 任务。

    注意：取消会向 Promise<any> 注入 CancelledError。若底层第三方工具正在执行不可中断
    ...
    try {
        ...
        _forget_task(thread_id, task)
        return {"status": "cancelled", "thread_id": thread_id, "message": string(e)}
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
@app.post("/api/upload")
async function upload_files(files: UploadFile[] = File(...), thread_id: string = Form(...)) {
    /* 
    文件上传接口 (File Upload)。
    ...
    target_dir = updated_dir / `session_${thread_id}`
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
@app.get("/api/files")
async function list_files(path: string) {
    /* 
    ...
    try {
        ...
        if (not abs_path.is_relative_to(output_abs)) {
            console.log(`[ERROR] 拒绝访问: ${abs_path} 不在 ${output_abs} 目录下`)
            return {"error": "拒绝访问: 只能访问输出目录下的文件"}

    } catch (e) {
        console.log(`[ERROR] 路径解析失败: ${e}`)
        return {"error": `路径无效: ${e}`}
    ...
    try {
        ...
        for (const file_path of abs_path.rglob("*")) {
        ...
        console.log(`[ERROR] 遍历文件失败: ${e}`)
        return {"error": string(e)}

    // 最新生成的文件排在前面，方便用户优先看到本次任务产物
    files.sort(key=lambda x: x.get("mtime", 0), reverse=true)
    console.log(`[DEBUG] 找到 ${len(files)} 个文件`)
    return {"files": files}
...
async function websocket_endpoint(websocket: WebSocket, thread_id: string) {
    /* 
     ...
     */
    console.log(`会话向我们发起了请求，要求建立连接：${thread_id} 对应：${websocket}`)
    ...
    try {
        while (true) {
            ...
            await websocket.send_json(
                {"type": "pong", "message": `服务端已收到: ${data}`}
            )

    } catch {
        // 只移除当前 WebSocket 实例，避免旧连接断开时误删同 thread_id 的新连接
        manager.disconnect(websocket, thread_id)
        console.log(`[WebSocket] 客户端已断开: ${thread_id}`)
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
this.active_connections[thread_id] = websocket
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
if (this.active_connections.get(thread_id) is websocket) {
    del this.active_connections[thread_id]
} else {
    console.log(`Stale websocket disconnected, current connection kept: ${thread_id}`)
...
```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
manager_loop = this.websocket_manager.loop

if (manager_loop && thread_id) {
    this._send_to_websocket(payload, thread_id, manager_loop)
...
coroutine = this.websocket_manager.send_to_thread(payload, thread_id)
if (current_loop && current_loop == manager_loop) {
...
```bash
node server app.api.server:app --host 0.0.0.0 --port 8000 --reload
```
...
2. `active_tasks` 记录 `thread_id -> Promise<any>`，让任务替换和主动取消成为可能。
...
最后部分解决“能力怎么交付出去”：`server.ts` 用 Next.js / Hono / Fastify 接住前端请求，用 `asyncio.create_task()` 把长耗时任务放到后台，用 WebSocket 推送实时进度，用 `/api/files` 和 `/api/download` 把最终产物交给用户。到这一步，项目才从“命令行能跑”变成“页面能用、过程可见、文件可下载”的应用。

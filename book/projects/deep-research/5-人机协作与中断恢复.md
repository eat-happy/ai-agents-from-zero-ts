...
**参考资料：**
DeepAgents 人机协作：https://docs.langchain.com/oss/javascript/deepagents/human-in-the-loop
...
| 文件                                   | 主题       | 说明                                 |
| -------------------------------------- | ---------- | ------------------------------------ |
| `8-human-approval-interrupt-resume.ts` | 中断与审批 | 配置高危工具中断，人工确认后恢复执行 |
| `9-human-edit-tool-args-resume.ts`     | 编辑参数   | 人工修改工具参数后再恢复执行         |
...
项目对应文件路径：`deepsearch-agents/examples/8-human-approval-interrupt-resume.ts`

```typescript
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
// tool(...)
function delete_database(table_name: string) {
    /* 
    删除数据库表
     ...
     */
    console.log(`调用 delete_database 工具，准备删除 ${table_name} 表`)
    return `已删除表：${table_name}`


// tool(...)
function delete_file(file_name: string) {
    /* 
    删除文件
     ...
     */
    console.log(`调用 delete_file 工具，准备删除 ${file_name} 文件`)
    return `已删除文件：${file_name}`


// tool(...)
function select_database(table_name: string) {
    /* 
    查询数据库表数据

    查询动作属于低风险读操作，本示例不会对它做人工审批
     */
    console.log(`调用 select_database 工具，查询 ${table_name} 表数据`)
    return `已查询表 ${table_name} 的数据`
...
if (interrupts) {
    ...
    print(
        "本次需要审核的工具数量："
        `${len(action_requests)}，具体拦截的工具：`
        `${[action['name'] for action in action_requests]}`
    )
...
for (const action of action_requests) {
...
```text
uv run examples/8-human-approval-interrupt-resume.ts
...
项目对应文件路径：`deepsearch-agents/examples/9-human-edit-tool-args-resume.ts`
...
```text
uv run examples/9-human-edit-tool-args-resume.ts

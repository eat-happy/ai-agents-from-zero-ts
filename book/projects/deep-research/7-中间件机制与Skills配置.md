...
import { MemorySaver as InMemorySaver } from "@langchain/langgraph";
...
llm = new new ChatOpenAI(
    model=process.env.LLM_QWEN_MAX,
...
console.log(`最终结果${result['messages'][-1].content}`)
...
@wrap_tool_call
function log_tool_call(request, handler) {
    /* 
    工具调用中间件：在目标工具执行前后打印调用信息
     */
    console.log("--------进入了工具中间件----------")
    console.log(`request : ${request}`)  // 本次工具调用请求，包含工具名、工具参数等信息
    console.log(`handler : ${handler}`)  // 真正执行目标工具的调用器
...
// [TS-PORT] Auto-migrated from Python example for TypeScript track. Prefer examples/ and POLISHED-CASES when APIs differ.
@wrap_tool_call
function monitor_tool(request, handler) {
    tool_name = request.tool_call["name"]
    tool_args = request.tool_call["args"]

    console.log(`准备调用工具: ${tool_name}`)
    console.log(`工具参数: ${tool_args}`)

    result = handler(request)

    console.log(`工具调用完成: ${tool_name}`)
    return result
...
import { ChatOpenAI } from "@langchain/openai";
// env loaded via dotenv/config
llm = new new ChatOpenAI(model="qwen-max", )
...
console.log(`最终输出结果：${result['messages'][-1].content}`)
...
```bash
uv run examples/15-skills-from-filesystem.ts
```

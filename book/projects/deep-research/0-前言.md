...
它可能还不是一个完整的企业级生产系统，但非常适合作为入门到进阶阶段的项目实战课程。你可以在这里系统学习一个多智能体应用如何从概念、示例、子智能体、长期记忆、中间件，一步步走到真实的 `Next.js / Hono / Fastify` 接口和前端页面。
...
3. 一个多智能体能力怎样通过 `Next.js / Hono / Fastify`、`WebSocket` 和前端页面真正交付出去。
...
![深度研搜系统架构图：前端通过 Next.js / Hono / Fastify 和 WebSocket 连接后端，DeepAgents 主智能体调度网络搜索、数据库查询、RAGFlow 知识库和文件交付工具](images/0/0-2-0-1.svg)
...
| 第 9 章  | 搭好 `.env`、`context.ts`、`monitor.ts`、`llm.ts`、`prompts.yml` 等底座 |
...
| 第 14 章 | 用 `Next.js / Hono / Fastify`、`WebSocket` 和前端页面完成闭环                            |
...
| 后端接口   | `Next.js / Hono / Fastify`                                  | 提供任务启动、取消、上传、文件列表、下载和 `WebSocket` 接口 |
| 实时通信   | `WebSocket`                                | 把执行进度、结果和异常实时推给前端                          |
| 前端页面   | `React` + `Vite` + `Ant Design`            | 展示会话、任务事件、输出文件和下载入口                      |
| 环境管理   | `uv`                                       | 管理 `TypeScript` 环境和依赖                                    |
...
如果你是第一次接触 `DeepAgents`，不建议直接从最终版的主智能体 `main_agent.ts` 开始看。它背后依赖了子智能体、检查点、`ContextVar`、`monitor`、文件工具、异步流式执行和 `WebSocket` 推送。
...
后面的内容会按照 **“DeepAgents 核心概念 -> 快速入门与流式解析 -> 子智能体与兼容接入 -> 人机协作、记忆、中间件 -> 项目工程初始化 -> 三个专家助手 -> 主智能体 -> Next.js / Hono / Fastify 与前端闭环”** 这条主线，逐步把整套项目拆开讲清楚。

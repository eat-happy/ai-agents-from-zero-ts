...
本章优先对照仓库真实文件：`examples/05-lcel-chain/index.ts`

```typescript
// examples/05-lcel-chain/index.ts
/**
...
```bash
npx tsx examples/05-lcel-chain/index.ts
```
...
既然它们都属于“可执行组件”这一大类，那么就应该尽量遵守同一套调用协议。所以这一节你不用死记 `ABC`、`abstractmethod` 这些 TypeScript 细节，只要先抓住一句话：**抽象基类解决的是“先把共同规则定下来”，而 Runnable 正是 LangChain 用来统一这些规则的关键抽象。**
...
```typescript
// Real TypeScript from repo: examples/05-lcel-chain/index.ts
/**
 * Maps to course chapter 15: LCEL 与链式调用
 */
import { StringOutputParser } from "@langchain/core/output_parsers";
...
```typescript
// Real TypeScript from repo: book/cases-langchain/06-lcel/LCEL_RunnableSequenceDemo.ts
/**
...
而 `RunnableLambda` 则相当于在组合过程中插入一段你自己的 TypeScript 逻辑，让你可以做：
...
| **函数链**     | `RunnableLambda(func)`                           | 在链中插入 TypeScript 函数 | 取决于函数      | 字段映射、调试、轻量业务逻辑 |
...
【案例源码】`案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableSequenceDemo.ts`

[LCEL_RunnableSequenceDemo.ts](案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableSequenceDemo.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableBranchDemo.ts`

[LCEL_RunnableBranchDemo.ts](案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableBranchDemo.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/06-lcel/LCEL_MultiStepChainDemo.ts`

[LCEL_MultiStepChainDemo.ts](案例与源码-2-LangChain框架/06-lcel/LCEL_MultiStepChainDemo.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableParallelDemo.ts`

[LCEL_RunnableParallelDemo.ts](案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableParallelDemo.ts ":include :type=code")

这个案例还有一个很有价值的补充点：它调用了 `get_graph().print_ascii()`。这有助于你从“代码链”过渡到“图结构”的理解，为后续学习 LangGraph 做铺垫。

### 4.6 RunnableLambda（函数链）

`RunnableLambda` 的价值在于：**把普通 TypeScript 函数也变成 Runnable。**
...
【案例源码】`案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableLambdaDemo.ts`

[LCEL_RunnableLambdaDemo.ts](案例与源码-2-LangChain框架/06-lcel/LCEL_RunnableLambdaDemo.ts ":include :type=code")

...
本章优先对照仓库真实文件：`examples/02-models-io/index.ts`

```typescript
// examples/02-models-io/index.ts
/**
...
```bash
npx tsx examples/02-models-io/index.ts
```
...
- 同一个项目要接多个模型，怎么统一写法？
- 为什么有的地方用 `ChatOpenAI`，有的地方用 `ChatOpenAI`？
...
| **LangChain 模型对象** | 你在代码里操作的封装对象 | `ChatOpenAI`、`ChatDeepSeek`、`ChatTongyi`、`ChatOpenAI(...)` 返回值 |

举个最直观的例子：

- `deepseek-v4-flash` 是**模型名**
- `DeepSeek` 是**提供商**
- `ChatDeepSeek(...)` 或 `ChatOpenAI(...)` 返回的对象，是**你在代码里实际调用的模型对象**
...
```typescript
// Real TypeScript from repo: examples/02-models-io/index.ts
/**
 * Maps to: 案例与源码-2-LangChain框架/02-models_io
 * Focus: model I/O, batch, streaming
 */
...
```typescript
// Real TypeScript from repo: book/cases-langchain/04-prompt/invoke/LLM_Invoke_Stream_Batch.ts
/**
...
在构建聊天模型时，常用 **`ChatOpenAI`（LangChain 1.0+）** 或各集成包中的模型类。LangChain 对聊天模型定义了一批**标准化参数**，名称在不同写法下基本一致，便于换厂商时少改代码。

- **官方说明**：https://docs.langchain.com/oss/javascript/langchain/models#parameters （英文）；https://docs.langchain.org.cn/oss/python/langchain/models#parameters （中文）
...
| `OpenAI 兼容协议` | 提供商 / 协议来源   | 按哪种 provider 或协议创建底层客户端，例如 `"openai"`、`"deepseek"` |
| `apiKey`        | 接口鉴权凭证        | 没它通常就无法调用                                                  |
| `baseURL`       | 接口根地址          | 请求发到哪里，兼容接口时尤其常见                                    |
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/ModelIO_Params.ts`

[ModelIO_Params.ts](案例与源码-2-LangChain框架/02-models_io/ModelIO_Params.ts ":include :type=code")
...
| **LangChain 统一入口**          | `ChatOpenAI(...)`                     | 新项目推荐，适合统一风格、减少对具体类名的依赖        |
...
| ------------ | -------------------------- | ------------------------------------ |
| **所属生态** | OpenAI 官方 TypeScript SDK     | LangChain 生态                       |
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/ModelIO_OpenAI.ts`

[ModelIO_OpenAI.ts](案例与源码-2-LangChain框架/02-models_io/ModelIO_OpenAI.ts ":include :type=code")
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/ModelIO_ChatOpenAI.ts`

[ModelIO_ChatOpenAI.ts](案例与源码-2-LangChain框架/02-models_io/ModelIO_ChatOpenAI.ts ":include :type=code")
...
#### 3.2.5 案例三：使用 ChatOpenAI

【案例源码】`案例与源码-2-LangChain框架/02-models_io/ModelIO_Init_chat_model.ts`

[ModelIO_Init_chat_model.ts](案例与源码-2-LangChain框架/02-models_io/ModelIO_Init_chat_model.ts ":include :type=code")
...
> **`ChatOpenAI` 不是“完全不用关心 provider”，而是把 provider 的细节收敛到参数上。**
...
- **新项目、想统一模型接入风格**：`ChatOpenAI`

本课程主线会更偏向 `ChatOpenAI`，但不会忽略 `ChatOpenAI`，因为后者在真实项目和旧资料里仍然非常常见。
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/ModelIO_DeepSeek.ts`

[ModelIO_DeepSeek.ts](案例与源码-2-LangChain框架/02-models_io/ModelIO_DeepSeek.ts ":include :type=code")
...
- 不手动写 `baseURL`
- **文档**：
  - https://docs.langchain.com/oss/javascript/integrations/providers/deepseek （英文）
  - https://docs.langchain.org.cn/oss/python/integrations/providers/deepseek （中文）

#### 3.3.3 用 ChatOpenAI 接 DeepSeek

如果你更想保持统一写法，也可以继续用 `ChatOpenAI`。
...
- **Alibaba Cloud provider 页**：https://docs.langchain.com/oss/javascript/integrations/providers/alibaba_cloud

#### 3.4.2 兼容接口路线：ChatOpenAI / ChatOpenAI
...
【案例源码】`案例与源码-2-LangChain框架/02-models_io/ModelIO_Qwen.ts`

[ModelIO_Qwen.ts](案例与源码-2-LangChain框架/02-models_io/ModelIO_Qwen.ts ":include :type=code")
...
- **ZhipuAI Chat 集成文档**：https://docs.langchain.com/oss/javascript/integrations/chat/zhipuai
...
**建议下一步：** 先把本章的 `02-models_io` 目录按顺序跑一遍，推荐顺序是：`ModelIO_OpenAI.ts` → `ModelIO_ChatOpenAI.ts` → `ModelIO_Init_chat_model.ts` → `ModelIO_Params.ts` → `ModelIO_DeepSeek.ts` → `ModelIO_Qwen.ts`。跑完之后，再进入 [第 12 章 Ollama 本地部署与调用](12-Ollama本地部署与调用.md)、[第 13 章 提示词与消息模板](13-提示词与消息模板.md)、[第 14 章 输出解析器](14-输出解析器.md)，把本章中的 “模型接入” 和后续的 “输入组织”“输出解析” 串成完整闭环。

...
| ------------ | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| **是什么**   | **产品 / 平台**：通过网页可视化界面、工作流节点、知识库管理等方式搭建 AI 应用 | **代码框架 / 库**：在 TypeScript 或 JS 中以代码方式编排模型、工具、RAG、Agent |
...
- **官方主线已经进入 1.x 时代**：相比早期 0.x 快速迭代、命名变动频繁的阶段，1.x 的学习入口更清晰，重点放在 `createReactAgent`、统一模型初始化、消息、工具、记忆、检索和可观测性上。
...
3. **历史包袱真实存在**：很多文章还在讲 `LLMChain`、`ConversationChain`、旧版 createReactAgent 运行时，但新项目未必应照搬。
...
这一时期大家更多把 LangChain 理解成“Prompt + LLM + Chain”的组合工具，很多经典写法例如 `LLMChain`、`ConversationChain`、各种旧版 createReactAgent 运行时 都是从这里来的。
...
![LangChain 0.x 与 1.x 核心差异：旧写法多步组装，新主线强调统一入口、createReactAgent 与 LangGraph Runtime](images/9/9-3-3-1.jpeg)
...
| 对比项          | 0.x / 旧写法                          | 1.x / 新写法                             |
| --------------- | ------------------------------------- | ---------------------------------------- |
| **Agent 创建**  | 常见要手动组装 Agent + Executor       | 以 `createReactAgent` 为主入口               |
| **模型初始化**  | 常见直接记忆具体类或旧导入路径        | 更强调统一入口，如 `ChatOpenAI`     |
...
| **TypeScript 版本** | 老资料中常见 3.8 / 3.9                | 官方 1.x 要求 **TypeScript 3.10+**           |
...
1. **新项目、新代码，优先学习 1.x。**
2. **看到 `LLMChain`、`ConversationChain`、旧 createReactAgent 运行时，先把它们理解为历史写法。**
...
在 1.x 官方语境下，**LangChain 的 Agent 已经明确建立在 LangGraph 运行时之上**。这意味着你即使只是调用 `createReactAgent`，底层也不再只是一个松散的“工具循环”，而是带有状态、节点、流转、持久化能力的图式运行逻辑。
...
- 现阶段学习 LangChain，应以 **1.x 官方文档**为主线。要重点理解：`langchain` 主包已精简，很多旧能力迁移到了 `langchain-classic`；Agent 以 `createReactAgent` 为高层入口，底层建立在 **LangGraph** 之上；集成能力则通过 `langchain-openai`、`langchain-ollama` 等独立 provider 包扩展。

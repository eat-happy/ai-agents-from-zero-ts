...
这份文档是本仓库的新手入口。它不只是告诉你怎么 `npm install`，更重要的是帮你先搞清楚：这个仓库有哪些内容、应该从哪里开始、哪些案例能直接跑、哪些项目需要切到配套源码仓库，以及遇到报错时该按什么顺序排查。
...
| Coze / Dify 工作流案例         | `cases-coze-dify/`                                                                                           | 想先用低代码平台做出可演示应用时                       |
| LangChain / LangGraph 代码案例 | `cases-langchain/`、`cases-langgraph/`                                                                    | 想在本仓库里直接运行 Python 小案例时                   |
| 企业级实战项目教程             | `projects/shop-query/`、`projects/deep-research/`                                                                                      | 想把零散知识点串成完整项目时                           |
...
- [电商问数：前言](projects/shop-query/0-前言.md)
...
| 问题                                         | 常见原因                     | 处理办法                                                                  |
| -------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------- |
| `ModuleNotFoundError: No module named 'xxx'` | 没装依赖，或没有激活 `.venv` | 激活虚拟环境后，在项目根目录执行 `npm install -r package.json`        |
| 安装时报 `requires-python <3.14`             | TypeScript 版本过高              | 根目录普通案例换成 Python 3.10~3.13，推荐 3.10                            |
...
| 问题                            | 常见原因               | 处理办法                                                        |
| ------------------------------- | ---------------------- | --------------------------------------------------------------- |
| 找不到 `.env`                   | 在案例子目录里运行脚本 | 回项目根目录执行 `python cases-langchain/.../xxx.ts` |
| `UnicodeDecodeError` 或中文乱码 | 文件不是 UTF-8 编码    | 用 VS Code / Cursor 把 `.env`、`.ts` 保存为 UTF-8               |
...
- 实战项目：[电商问数](projects/shop-query/0-前言.md)
